...
\begin{defn}
Given a set $\La$ of \define{labels}, an \define{$\La$-graph} is a graph $s,t\maps E\to N$
...
but following previous work \cite{BCR} we call an open $\La$-graph an \define{$\La$-circuit}.
...
\define{$\La$-circuit} is a cospan in $\Graph_\La$ of this form:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(S)$};
\node (B) at (1,0) {$G$};
