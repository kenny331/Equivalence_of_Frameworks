...
{\chris This is not a hundred percent true, because we want the corresponding F via the MonGroth to do so. This is the introduction though so we could be simpler, but more accurate perhaps?}
From the accompanying left adjoint $L \maps \A \to \X$, we  construct a symmetric monoidal double category ${}_L \lCsp(\X)$ of structured cospans.  Finally, in \cref{thm:equiv} we prove that this structured cospan double category ${}_L \lCsp(\X)$ is isomorphic to the decorated cospan double category $F \lCsp$.   In fact, they are isomorphic as symmetric monoidal double categories. %%%
{\chris Perhaps some ending statement saying something like ``this provides a passage between those two approaches, which we see in action in some of the main applications of the cospan frameworks'' or something? Right now it feels a bit like the paper finishes with theorem 3.2.}
...
\end{tikzpicture}
\]
%%% Kenny: Commenting out previous equation.
\begin{comment}
\[
\begin{tikzpicture}[scale=1.5]
\node (G) at (-1,0.5) {$a$};
