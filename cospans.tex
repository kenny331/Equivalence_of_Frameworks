...
The category of objects $F\lCsp_0$ is simply $\A$. The category of arrows $F\lCsp_1$ has $F$-decorated cospans for objects and maps of $F$-decorated cospans as morphisms. Given two vertically composable $F$-decorated cospans:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0.5) {$a$};
\node (A') at (0,-0.5) {$a'$};
...
\begin{thm}\label{DC}
Let $\A$ be a category with finite colimits and let $F \maps (\A,+) \to (\Cat,\times)$ a symmetric lax monoidal pseudofunctor. Then the double category $F\lCsp$ of Theorem \ref{thm:decorated_cospans} is symmetric monoidal, where
\begin{enumerate}
