...
\begin{cor}\label{cor:fcocMonGroth}
 Suppose $\A$ has finite colimits and $F\maps(\A,+)\to(\Cat,\times)$ is a lax monoidal pseudofunctor. If $F\maps\A\to\Cat$ as a pseudofunctor factors through $\Rex$, then $\inta F$ has all finite colimits and the induced opfibration $U\maps\inta F\to\A$ preserves them.
...
{\chris Decide on the specific assumptions of \cref{cor:fcocMonGroth} and \cref{thm:equiv} after discussing with John and Kenny}

Notice that by \cref{lem:MonGroth}, since $\A$ is cocartesian monoidal, the initial lax monoidal pseudofunc\-tor structure $(F,\phi,\phi_0)\maps(\A,+)\to(\Cat,\times)$ gives rise to a specific monoidal structure on the fibers $F(a)$ in terms of $\phi,\phi_0$, given explicitly by \cref{eq:explicitstructure1}. If this fiberwise monoidal structure happens to itself be cocartesian, namely \cref{eq:explicitstructure1} gives coproducts and an initial object in each $F(a)$, 
%e.g. when the induced total category is cocartesian
those are clearly isomorphic to the arbitrary choice of finite coproducts in $F(a)$ coming from the above assumption that as a pseudofunctor, $F\colon \A\to\Rex$. 
For example, in that case, the coproducts induced in the Grothendieck category from the fiberwise ones as in \cref{eq:fibrewisecoprod} are equivalently given by \cref{eq:explicitstructure2}.
