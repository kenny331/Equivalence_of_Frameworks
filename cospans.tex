...
Under $\lE$ this is mapped to the underlying $\A$-cospan $\cspn{a}{m+_bn}{b}{}{}$ with decoration $F(v_m)x+F(v_n)y$, since a pushout over an initial object is really a coproduct. That decoration is isomorphic to that of \cref{eq:1} due to the following diagram 
...
\end{align*}
The cospan part is equal, whereas we have an isomorphism between the decorations $F(\iota_m)x+F(\iota_{m'})\cong \phi_{m,m'}(x,x')$ in an analogous way to \cref{eq:isomorphismdiag} as follows
\begin{displaymath}
\begin{tikzcd}
...
F(m+m')\times F(m+m')\ar[r,"\phi_{m+m',m+m'}"']\ar[rr,dashed,bend right=15] & F\left((m+m')+(m+m')\right)\ar[r,"F(\nabla)"'] & F(m+m')
 \end{tikzcd}
\end{displaymath}
Notice that this isomorphism is in fact unique, since both sides express a coproduct in the Grothendieck category $\inta F$ by formulas \cref{eq:fibrewisecoprod,eq:explicitstructure2}. {\chris IS THIS CLEAR?}
