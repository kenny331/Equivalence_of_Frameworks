...
This sets the stage for structured cospans: \cref{SC} gives us a symmetric monoidal double category ${}_L \lCsp(\Graph_\La)$ where a horizontal 1-cell is an \define{open $\La$-graph}, also called an \define{$\La$-circuit}: that is, a cospan in $\Graph_\La$ of this form:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(X)$};
\node (B) at (1,0) {$G$};
...
\begin{itemize}
\item objects being \define{$\La$-graph structures} on $N$: that is, $\La$-graphs where the set of nodes is $N$,
