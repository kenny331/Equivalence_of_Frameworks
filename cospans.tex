...
\item a horizontal 1-cell is an \define{$F$-decorated cospan}: that is, 
a cospan in $\A$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
...
\item a 2-morphism is a \define{map of $F$-decorated cospans}: that is, 
...
\end{tikzpicture}
\]
together with the composite morphism of decorations $\tau' F(\tau) \colon F(h'h)(x) \to x'$ in $F(m'')$.
% this was:
...
\begin{gather}
\textrm{lax monoidal pseudofunctors }F\maps(\A,+,0)\to(\bicat{Cat},\times) \notag\\
\Updownarrow \notag\\
