...
We thank Daniel Cicala, Brendan Fong, and Joe Moeller for helpful conversations.
...
With these definitions, $\lE$ is indeed a double functor as in \cref{def:doublefun}: both $\lE_0$ and $\lE_1$ are functors that appropriately commute with sources and targets, and there exist globular 2-isomorphisms $\lE(M) \odot \lE(N) \simrightarrow\lE(M \odot N)$, $U_{\lE m} \simrightarrow\lE(U_m)$
