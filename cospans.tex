...
\end{align*}
%%% Kenny: I think we should call this \alpha something else like \kappa, since it's only a component used in building the associator for horizontal composition (which we are calling \alpha), not the entire thing on its own.
\[
\begin{tikzpicture}[scale=1.7]
\node (A) at (3.75,0) {$\scriptstyle{\cong \tau_a}$};
