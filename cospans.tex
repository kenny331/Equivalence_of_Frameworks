...
\end{equation}
describes the construction of the adjunction from the original $F$. Explicitly, the left adjoint picks the initial object in the fiber $L(a)=(a,\bot_a)$, also expressed as $F(!_a)\circ\phi_0(*)$ according to \cref{eq:explicitstructure1} for $(\phi,\phi_0)$ the monoidal structure of $F$. %There are many laris, one for each choice of initial object. Having fixed our choice of colimits, there is really only one lari, correct? Also, is that ``also...'' part used anywhere?
...
F(m+m')\times F(m+m')\ar[r,"\phi_{m+m',m+m'}"']\ar[rr,dashed,bend right=15] & F\left((m+m')+(m+m')\right)\ar[r,"F(\nabla)"'] & F(m+m')
 \end{tikzcd}
\end{displaymath}
Notice that both sides above express a coproduct in the Grothendieck category $\inta F$ by formulas \cref{eq:fibrewisecoprod,eq:explicitstructure2},  so this isomorphism is in the canonical unique one that commutes with the respective cocones. {\chris IS THIS CLEAR? OR HELPFUL? Could omit.}

{\chris Kenny, I am a bit confused with all the units and monoidal units around. Could you go through in detail, and phrase correctly, the following part? Following the definition of a monoidal double functors and related structures precisely? For example, shouldn't we start from the monoidal unit of structured cospans? How do you write that?}
Moreover, the monoidal unit for the category of arrows of $F\lCsp$ is $0_\A\xrightarrow{\raisebox{-3pt}[0pt][0pt]{$=$}}0_\A\xleftarrow{\raisebox{-3pt}[0pt][0pt]{$=$}}0_\A$ with the trivial decoration $\phi_0\maps \one\to F(0_A)\to F(0_A)$,
...
again by \cref{eq:explicitstructure2}. It can be verified that with the above structure, $\lE_1$ is also a symmetric monoidal functor, and the rest of the axioms of \cref{defn:monoidal_double_functor} hold.

\textbf{Kenny: This is what I would say regarding the monoidal units:} Moreover, the monoidal unit $I_{F\lCsp}$ for the category of arrows of $F\lCsp$ is given by the identity cospan $0_\A \rightarrow 0_\A \leftarrow 0_\A$ on the monoidal unit of $\A$ together with the trivial decoration $\bot_{0_\A} = \phi_0 \colon 1 \to F(0_\A)$ on $0_\A$, and the monoidal unit $I_{{_L \lCsp(\int F)}}$ for the category of arrows of ${ _L \lCsp(\int F)}$ is given by $(0_\A,\bot_{0_\A}) \rightarrow (0_\A,\bot_{0_\A}) \leftarrow (0_\A,\bot_{0_\A})$, with $\lE(I_{{ _L \lCsp(\int F)}})$ also being given by the identity cospan on $0_\A$ with the trivial decoration on its apex so that the transformation $\theta_U \colon I_{F\lCsp} \to \lE \circ I_{{_L \lCsp (\int F)}}$ is an identity.
\end{proof}
...
\noindent
Unpacking this definition, one obtains a number of diagrams which can be found after \cite[Definition 2.14]{HS}.
