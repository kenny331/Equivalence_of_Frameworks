...
As noted for example by Shulman \cite{Shulman2010},
%%%Christina: this should be a much older, very well-known fact. I suggest for removal of reference completely (given that we describe this thing from scratch) or instead insert a much older reference than that.
any double category $\lX$ has a 
{\chris How come we changed from D to X as our basic letter for double cats?}
...
A double category is `fibrant' if every vertical 1-morphism has a `companion' and a `conjoint'---concepts explained in \cref{def:companion}. Shulman (\cref{Shulhorizontalbicat}) proved that when a double category $\lX$ is fibrant, any symmetrical monoidal structure on $\lX$ gives one on $\bX$.    We can apply this to decorated cospans as follows:
...
All this so far is boringly familiar.  More interesting is the symmetric monoidal double
...
%{\chris The rest is not done yet...} %%%Christina I have no clue where this comment came from!

We now show that open dynamical systems of this nature are an example of open systems that are able to be characterized by decorated cospans but not structured cospans.
...
%%% Give reference describing these conditions!!!
In particular, pseudonaturality of the monoidal structure means that it comes with isomorphisms (natural in $f,g$)
...
\item{for every composable pair of horizontal 1-cells $M$ and $N$ in $\lA$, a natural transformation $\phi_{M,N} \colon \lF(N) \odot \lF(M) \to \lF(N \odot M)$ called the \define{composite comparison} and for every object $a$ in $\lA$, a natural transformation $\phi_a \colon U_{\lF_0(a)} \to \lF_1(U_a)$ called the \define{unit comparison}. The components of each of these natural transformations are globular isomorphisms which satisfy coherence axioms analogous to those of a monoidal functor.}
\end{itemize}
\end{defn}
...
\begin{thm}{\cite[Theorem 1.2]{Shulman2010}}
...
\end{tikzpicture}
\]
{\chris Chai's in the definition of a monoidal double category have a different direction. Since it is an isomorphism it doesn't really change anything, but consider changing to keep them right}
