...
\begin{proof}
The unit structure functor $U \maps F\lCsp_0 \to F\lCsp_1$ is defined on objects as: 
\[
\begin{tikzpicture}[scale=1.5]
\node (E) at (-1,0) {$a$};
\node (F) at (-.5,0) {$\mapsto$};
...
\end{tikzpicture}
\]
together with the morphism $\tau_{!_f} = F(f) F(!) \phi \maps 1 \to F(a')$. We also have source and target structure functors $S, T \maps F\lCsp_1 \to F\lCsp_0$ where the source of the horizontal 1-cell
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,0) {$m$};
