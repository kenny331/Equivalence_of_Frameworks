...
\end{equation}
This can always be achieved by replacing $F$ by an equivalent pseudofunctor that strictly preserves identity morphisms, but we do this merely to reduce the size of diagrams to come; the reader can reinsert the morphism $F(!_0)$ where needed. %%% see if you like this!  I'd like to add a ref that says any 2-functors is equivalent to a normalized one. - jb
The associator is formed as follows: for three objects
...
Moreover, the monoidal unit $I_1$ for $F\lCsp_1$, given by \cref{eq:monoidalunitFCsp}, is mapped under $\lE_1$ to the identity cospan on $0_\A$ with the trivial decoration on its apex, so $\lE_1$ strictly preserves the monoidal unit.   %%% Under our new assumptions it's really true! - jb
...
\begin{itemize}
\item monoidal structures on both $\lD_0$ and $\lD_1$ (each with tensor product denoted $\otimes$, associator $a$, left unitor $\ell$ and right unitor $r$), such that $U \maps \lD_0 \to \lD_1$ strictly preserves the unit object and $S,T \maps \lD_1 \to \lD_0$ are strict monoidal,    %%% got rid of footnote saying that "isomorphic is okay" - jb
