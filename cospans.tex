...
\begin{tikzcd}[row sep=.35in]
 ...
 F(b)\times F(b)\ar[r,"\phi_{a,b}"'] & F(b+b)\ar[r,"F(\nabla)"'] & F(b)
\end{tikzcd}
\end{equation}
\end{proof}
...
\end{tikzpicture}
\]
%%%Christina: if I am not mistaken, John had some comments on how these cospans are written and whether our convention is clear.
\[
\begin{tikzpicture}[scale=1.5]
\node (G) at (-0.75,0.5) {$a$};
\node (H) at (-0.75,-0.5)  {$b$};
...
\end{tikzpicture}
\]
%since the right hand sides of the above two equations are given respectively by the 2-morphisms $U_f$ and $1_{\hat{f}}$. 
...
%%% Christina: that's nice and clear, but could we also pinpoint which assumption from the Theorem fails exactly? Because having an adjoint is a RESULT of the initial conditions. We don't need to elaborate, just point it out - it is presumably that F does not factor through Rex?
