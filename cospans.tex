...
\end{comment}
\begin{displaymath}
I_1=\left(0\xrightarrow{!}0\xleftarrow{!}0, 1\xrightarrow{\phi_0}F(0)\right)
\end{displaymath}
...
a_2+a_1\ar[rr,"i_2+i_1"'] && m_2+m_1 && b_2+b_1\ar[ll,"o_2+o_1"] &&1\ar[r,"x_2\times x_1"] & F(m_2)\times F(m_1)\ar[r,"\phi_{m_2,m_1}"] & F(m_2+m_1)
 \end{tikzcd}
\end{displaymath}
...
1\ar[ddrr,dashed,"{(x_1\oplus y_1)\odot(x_2\oplus y_2)}"']\ar[r,"x_1{\times}y_1{\times}x_2{\times}y_2"] \& F(m_1){\times}F(n_1){\times}F(m_2){\times}F(n_2)\ar[r,"\phi_{m_1,n_1}\times\phi_{m_2,n_2}"] \& F(m_1{+}n_1){\times}F(m_2{+}n_2) \ar[d,"\phi_{m_1+n_1,m_2+n_2}"] \\
...
1\ar[ddrr,dashed,"{(x_1\odot x_2)\oplus(y_1\odot y_2)}"']\ar[r,"x_1{\times}x_2{\times}y_1{\times}y_2"] \& F(m_1){\times}F(m_2){\times}F(n_1){\times}F(n_2)\ar[r,"\phi_{m_1,m_2}\times\phi_{n_1,n_2}"] \& F(m_1{+}m_2){\times}F(n_1{+}n_2) \ar[d, "F\psi\times F\psi"] \\
...
\& (b,y)\ar[ul,"{\begin{cases}f\maps b\to a &\textrm{in }\A \\k\maps F(f)y\to x &\textrm{in }F(a)\end{cases}}"description]\ar[ur,"{\begin{cases}g\maps b\to c &\textrm{in }\A \\\ell\maps F(g)y\to z &\textrm{in }F(c)\end{cases}}"description] \&
 \end{tikzcd}
\end{equation}
which is the pushout $u_a\colon a\to a+_bc\leftarrow c:u_c$ in $\A$ on the first variable, and $w$ is the pushout in the fibre $F(a+_bc)$
\begin{displaymath}
\begin{tikzcd}
