...
We have given conditions under which a decorated cospan double category is isomorphic to a structured cospan double category, in \cref{thm:equiv}. The converse question is also interesting: is every structured cospan double category equivalent to a decorated cospan double category? The answer is similar to the previous one: affirmative, under certain conditions that essentially allow us to make the passage from an appropriate functor $L\colon \A\to \X$ to an appropriate pseudofunctor $F\colon \A\to\Cat$, namely in the reverse direction than the earlier passage \cref{eq:FtoL}.
...
\begin{displaymath}
\begin{tikzpicture}
\node (A) { \{Lax monoidal pseudofunctors $(\A,+)\to(\Cat,\times)$\} };
\node[below right=1.5 and 1.5 of A] (C)  { \{Symmetric monoidal double categories\} };
\node[below=1  of A] (D)  { \{Special opfibrations\} };
\node[below=2  of A] (E)  { \{Special laris\} };
\node[below=3 of A] (B)  { \{ Finitely cocomplete functors $A\to\X$\} };
\node[below=.05 of D] {$\cong$};
\path[->,font=\scriptsize]
(A) edge[bend left] node[above,sloped]{$F\mapsto F\lCsp$}  (C)
(D) edge (A)
(E) edge (B)
(B) edge[bend right] node[below,sloped]{$L\mapsto {}_{L}\lCsp$} (C);
\end{tikzpicture}
\end{displaymath}
with an actual natural isomorphism in the middle. This would imply that starting from an appropriate middle ground, employing the structured or the decorated cospan formalism is actually the same. We leave such considerations for future work.
