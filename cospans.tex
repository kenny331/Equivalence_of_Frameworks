...
\& (b,y)\ar[ul,"{\begin{cases}f\maps b\to a &\textrm{in }\A \\k\maps F(f)y\to x &\textrm{in }F(a)\end{cases}}"description]\ar[ur,"{\begin{cases}g\maps b\to c &\textrm{in }\A \\\ell\maps F(g)y\to z &\textrm{in }F(c)\end{cases}}"description] \&
 \end{tikzcd}
\end{equation}
which on the first variable is given by the pushout in $\A$ on the left below, and the object $w$ is formed as the pushout in the fiber $F(a+_bc)$ on the right below:
\begin{displaymath}
\begin{tikzcd}
