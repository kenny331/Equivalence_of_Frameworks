...
\begin{itemize}
\item {\chris Notation between \cref{Equiv} and \cref{thm:decoratedcospans} right now does not match! Need to fix. Slightly prefer a,b,c for objects of A, then $a'$ and $b'$ for other objects, and x an object of the category F(c) namely the decoration. The previously used `d' is slightly confusing around all these a's, b's and c's that are in a different category. Also I prefer $a$ and $b$ rather than $a_1$ and $a_2$.}

{\chris After a brief discussion in John's office, for \cref{Equiv} I used $a,b,c$ for the feet and $m,n$ for the apices so we can serially compose. Of course can change as we like.}
...
%Changed Defn. to Definition
In this paper, `double category' means `pseudo double category', as in Definition \ref{defn:double_category}.   Following Shulman \cite{Shul2010}, vertical composition 
...
Let $\mathsf{A}$ be a category with finite colimits and $F \maps \mathsf{A} \to \Cat$ a lax monoidal pseudofunctor. Then there exists a (pseudo) double category $F\mathbb{C}$sp which has:
\begin{enumerate}
\item{objects as those of $\mathsf{A}$,}
...
\end{tikzpicture}
\end{equation}
together with a morphism $\iota \maps F(h)(m) \to m^\prime$ in $F(m^\prime)$.}
\end{enumerate}
\end{thm}
\textbf{Kenny: We should try to make this theorem shorter.} {\chris I agree!}
\textbf{Kenny: On second thought, I'm not sure what we can cut out of this theorem. We're already saying "the standard monoidal category diagrams are satisfied" rather than displaying them.}
\begin{proof}
...
\end{tikzpicture}
\]
together with the morphism $\iota_{!_f} = F(f) F(!) \phi \maps 1 \to F(a^\prime)$. We also have source and target structure functors $S, T \maps F\mathbb{C}\textnormal{sp}_1 \to F\mathbb{C}\textnormal{sp}_0$ where the source of the horizontal 1-cell
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,0) {$m$};
...
together with the isomorphism $\iota_\sigma \maps F(\sigma)((x \odot y) \odot z) \to x \odot (y \odot z)$. Note that the map $\sigma \maps (m_1+_{b} m_2)+_{c}m_3 \to m_1+_{b}(m_2+_{c}m_3)$ is the universal map between two colimits of the same diagram. We also have left and right unitors where given a horizontal 1-cell $M$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,0) {$m$};
...
\end{tikzpicture}
\]
where $r \maps m+_{b} b \xrightarrow{\sim} m$ is a universal map together with the isomorphism $\iota_r \maps F(r)(x \odot !_b) \to x$. The left unitor is similar. The source and target functor applied to the left and right unitors and associators yield identities, and the left and right unitors together with the associator satisfy the standard pentagon and triangle identities of a monoidal category or bicategory. Finally, for the interchange law, given four 2-morphisms $\alpha, \beta, \alpha^\prime$ and $\beta^\prime$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0.5) {$a$};
\node (A') at (0,-0.5) {$a'$};
...
\node (L) at (1,-1) {$\iota_\alpha \maps F(h_1)(x) \to x'$};
\node (M) at (5,-1) {$\iota_\beta \maps F(h_2)(y) \to y'$};
...
\node (L'') at (1,-3) {$\iota_{\alpha^\prime} \maps F(h_1^\prime)(x') \to x''$};
\node (M'') at (5,-3) {$\iota_{\beta^\prime} \maps F(h_2^\prime)(y') \to y''$};
\path[->,font=\scriptsize,>=angle 90]
...
\node (G) at (1.5,-1) {$\iota_{\alpha \odot \beta} \maps F(h_1 +_g h_2)(x \odot y) \to x' \odot y'$};
\node (A'') at (0,-1.5) {$a^\prime$};
\node (A''') at (0,-2.5) {$a''$};
...
\node (G'') at (1.5,-3) {$\iota_{\alpha^\prime \odot \beta^\prime} \maps F(h_1^\prime +_{g^\prime} h_2^\prime)(x' \odot y') \to x'' \odot y''.$};
\path[->,font=\scriptsize,>=angle 90]
...
\end{tikzpicture}
\]
These two 2-morphisms $\iota_\alpha$ and $\iota_\beta$ are two 2-morphisms in the monoidal 2-category $(\Cat,\times,1)$ and so we can tensor them which results in:
\[
\begin{tikzpicture}[scale=1.5]
...
\end{tikzpicture}
\]
where the middle square commutes since $F$ is a lax monoidal pseudofunctor and the right square commutes as the underlying diagram commutes. The decorations $x \odot y$ and $x' \odot y'$ are given respectively by top and bottom composite of arrows and the morphism of decorations $\iota_{\alpha \odot \beta}$ is given by composing $\iota_\alpha \times \iota_\beta$ with the two commuting squares, which can equivalently be viewed as a morphism in $F(m' +_{b'} n')$. 
...
\node (G) at (2,-1) {$\iota_{(a^\prime \odot \beta^\prime)(\alpha \odot \beta)} \maps F((h_1^\prime +_{g^\prime} h_2^\prime)(h_1 +_g h_2))(x \odot y) \to x'' \odot y''.$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$j \psi_{m} i_1$} (B)
(C) edge node[above]{$j \psi_{n} o_2$} (B)
...
\node (L) at (1,-1) {$\iota_{\alpha^\prime \alpha} \maps F(h_1^\prime h_1)(x) \to x''$};
\node (M) at (5,-1) {$\iota_{\beta^\prime \beta} \maps F(h_2^\prime h_2)(y) \to y''$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$i_1$} (B)
(C) edge node[above]{$o_1$} (B)
(A) edge node[left]{$f^\prime f$} (A')
...
\node (E) at (5.5,0.5) {$x \odot y \in F(m+_{b} n)$};
\node (F) at (5.5,-0.5) {$x'' \odot y'' \in F(m'' +_{b''} n'')$};
\node (G) at (2,-1) {$\iota_{(\alpha^\prime \alpha) \odot (\beta^\prime \beta)} \maps F((h_1^\prime h_1)+_{g^\prime g} (h_2^\prime h_2))(x \odot y) \to x'' \odot y''.$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$j \psi_{m} i_1$} (B)
(C) edge node[above]{$j \psi_{n} o_2$} (B)
(A) edge node[left]{$f^\prime f$} (A')
...
%\end{tikzpicture}
%\]
As is usual concerning the interchange law of double categories of this nature, only the `interior' of the two composites appears different, but the two morphisms $(h_1^\prime +_{g^\prime} h_2^\prime)(h_1 +_g h_2) \maps m+_{b} n \to m'' +_{b''} n''$ and $(h_1^\prime h_1) +_{g^\prime g} (h_2^\prime h_2) \maps m+_{b} n \to m'' +_{b''}n''$ are the same universal map realized in two different ways. The two morphisms of decorations $\iota_{(\alpha^\prime \odot \beta^\prime)(\alpha \odot \beta)}$ and $\iota_{(\alpha^\prime \alpha) \odot (\beta^\prime \beta)}$ are obtained as two different compositions of four 2-morphisms in $\Cat$, namely horizontally then vertically and vertically then horizontally. As $\Cat$ is a 2-category, the interchange law for these 2-morphisms already holds, and as a result, the morphisms $$\iota_{(\alpha' \odot \beta')(\alpha \odot \beta)} \maps F((h_1^\prime +_{g^\prime} h_2^\prime)(h_1 +_g h_2))(x \odot y) \to x'' \odot y''$$ and $$\iota_{(\alpha^\prime \alpha)\odot(\beta^\prime \beta)} \maps F((h_1^\prime h_1)+_{g^\prime g} (h_2^\prime h_2))(x \odot y) \to x'' \odot y''$$ are also the same. Thus the interchange law for 2-morphisms holds and $F\mathbb{C}$sp is a double category.
\end{proof}
...
\end{thm}
\textbf{Kenny: We should try to make this theorem shorter.} {\chris Yes!}
\begin{proof}
...
\end{tikzpicture}
\]
together with a morphism $\iota \maps F(h)(x) \to x^\prime$.
}
\end{enumerate}
...
\end{tikzpicture}
\]
where $\ell$ is the left unitor of $(\mathsf{A},+,0)$, together with the isomorphism $\iota_{\lambda} \maps F(\ell)(!_0 + x) \to x$. The right unitor is similar.
...
\end{tikzpicture}
\]
together with the isomorphism $\iota_a \maps F(a)((x_1+x_2)+x_3) \to x_1+(x_2+x_3)$. These associators and left and right unitors together satisfy the pentagon and triangle identities of a monoidal category. \textbf{Kenny: Probably cut stuff out below this.} If we denote the above associator simply as $a$ and the left and right unitors as $\lambda$ and $\rho$, respectively, then given four objects in $F\mathbb{C}\textnormal{sp}_1$, say $M_1, M_2, M_3$ and $M_4$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a_1$};
\node (B) at (1,0) {$m_1$};
...
\end{tikzpicture}
\]
$$\iota_{\beta_{M_1,M_2}} \maps F(\beta_{m_1,m_2})(x_1+x_2) \xrightarrow{\sim} x_2+x_1$$ where the vertical 1-morphisms are given by braidings in $(\mathsf{A},+,0)$. This braiding makes the following triangle of underlying cospans commute:
\[
\begin{tikzpicture}[scale=1.5]
...
\end{tikzpicture}
\]
$$\iota_{\hat{\chi}} \maps F(\hat{\chi})((x_1+y_1)\odot(x_2+y_2)) \to (x_1 \odot x_2)+(y_1 \odot y_2)$$
...
\end{tikzpicture}
\]
$$\iota_{a,b} \maps !_{a+b} \xrightarrow{\sim} !_a + !_b$$
...
(b6) edge node[right]{$\iota_6$} (b7)
		...
		\end{tikzpicture}
	\]
Here all of the vertical 1-morphisms on the left and right are associators or identities, the middle vertical 1-morphisms labeled on the left are the 2-morphisms from the previous commutative diagram, and the horizontal vertical 1-morphisms pointing towards the middle are natural maps into each colimit, all of which are naturally isomorphic to each other as all the middle objects are colimits of the same diagram, namely the previous collection of cospans, taken in various ways. The above diagram of maps of cospans can then be visualized as a hexagonal prism in which all the faces commute by identifying the top and the bottom as the same. As for the morphisms of decorations, which are labeled on the right of the interior vertical 1-morphisms, each isomorphism $\iota_n$ goes from the domain under the image of the functor $F$ applied to natural isomorphism adjacent to it to the codomain as written, meaning that, for example: $$\iota_1 \maps F(a \odot a)(((x_1+y_1)+z_1) \odot ((x_2+y_2)+z_2)) \to (x_1+(y_1+z_1)) \odot (x_2+(y_2+z_2)).$$  The following diagram commutes in the category $F((m_1+_b m_2) + ((n_1+_{b'} n_2) + (p_1+_{b''} p_2)))$:
\[
\begin{tikzpicture}[scale=1.5]
...
\node (M) at (5,-1) {$\iota_f \maps F(f)(!_c) \to !_{c^\prime}$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$f$} (B)
(C) edge node[above]{$1$} (B)
(A) edge node[left]{$f$} (A')
...
\node (Q) at (1,-1) {$\iota_f \maps F(f)(!_c) \to !_{c^\prime}$};
\node (L) at (1,-1.5) {$\iota_{c^\prime} = 1_{!_{c^\prime}}$};
\node (M) at (5,-0.5) {$\iota_f \maps F(f)(!_c) \to !_{c^\prime}$};
\node (R) at (3,0.5) {$=$};
\path[->,font=\scriptsize,>=angle 90]
...
\node (L') at (-1,-1.5) {$\iota_{f} \maps F(f)(!_c) \to !_{c^\prime}$};
...
\end{tikzpicture}
\]
together with a morphism $\iota \maps F(h)(d) \to d^\prime$ in $F(c^\prime)$.}
\end{enumerate}
\end{thm}
...
\end{tikzpicture}
\]
and there exists an isomorphism $\iota \maps F(f)(d) \to d^\prime$ in $F(c^\prime)$.}
\end{enumerate}
\end{cor}
...
\node (E) at (3,0.5) {$d \in F(c)$};
\node (F) at (3,-0.5) {$d^\prime \in F(c^\prime)$};
\node (G) at (1,-1) {$\iota \maps F(h)(d) \to d^\prime$};
...
\node (G'') at (5.25,-1) {$E(\iota) \maps F^\prime(H(h))(\theta_c E(d)\phi) \to (\theta_{c'}E(d^\prime)\phi)$};
...
\begin{comment}
Recall that we can think of the object $d \in F(c)$ as a morphism $d \maps 1 \to F(c)$ and the functor $\iota \maps F(h)(d) \to d^\prime$ of $F(c^\prime)$ as a natural transformation  in $\Cat$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,-0.5) {$1$};
\node (B) at (1,0) {$F(c)$};
...
\end{tikzpicture}
\]
which results in a 2-morphism $E(\iota) \maps F^\prime(H(h))(\theta_c E(d)\phi) \to (\theta_{c'} E(d^\prime)\phi)$ in $F^\prime(H(c^\prime))$. To check that the above recipe is functorial, given two vertically composable 2-morphisms in $F\mathbb{C}\textnormal{sp}$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0.5) {$a$};
\node (A') at (0,-0.5) {$a^\prime$};
...
\node (G) at (1.5,-1) {$\iota \maps F(h)(d) \to d^\prime$};
\node (A'') at (0,-1.5) {$a^\prime$};
\node (A''') at (0,-2.5) {$a''$};
...
\node (G'') at (1.5,-3) {$\iota^\prime \maps F(h^\prime)(d^\prime) \to d''$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$$} (B)
(C) edge node[above]{$$} (B)
(A) edge node[left]{$f$} (A')
...
\node (G) at (1.5,-1) {$\iota ^\prime \iota \maps F(h^\prime h)(d) \to d''$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$$} (B)
(C) edge node[above]{$$} (B)
(A) edge node[left]{$f^\prime f$} (A')
...
\node (G) at (1.5,-1) {$E(\iota ^\prime \iota) \maps F^\prime(H(h^\prime h))(\theta_c E(d)\phi) \to (\theta_{c''}E(d'')\phi).$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$$} (B)
(C) edge node[above]{$$} (B)
...
\node (G) at (1.5,-1) {$E(\iota) \maps F^\prime(H(h))(\theta_c E(d)\phi) \to (\theta_{c'} E(d^\prime)\phi)$};
...
\node (G'') at (1.5,-3) {$E(\iota^\prime) \maps F^\prime(H(h^\prime))(\theta_{c'} E(d^\prime)\phi) \to (\theta_{c''}E(d'')\phi)$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$$} (B)
(C) edge node[above]{$$} (B)
(A) edge node[left]{$H(f)$} (A')
...
\node (G) at (2.5,-1) {$\iota_{\kappa^{-1}} \maps F^\prime(\kappa^{-1})(d_{\mathbb{H}(M) \odot \mathbb{H}(N)}) \to d_{\mathbb{H}(M \odot N)}.$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$\Psi j_{H(c_1)} H(i_1)$} (B)
(C) edge node[above]{$\Psi j_{H(c_2)} H(o_2)$} (B)
(A) edge node[left]{$1$} (A')
...
\end{tikzpicture}
\]
where the morphism of decorations is the morphism $\iota \maps !_{H(c)} \to (\theta_c E(!_c) \phi)$ in $F'(H(c))$. These comparison constrains satisfy the coherence axioms of a monoidal category, namely:
\[
\begin{tikzpicture}[scale=1.5]
...
\end{tikzpicture}
\]
$$\iota_\kappa \maps F^\prime(\kappa)(d_{\mathbb{H}(M_1) \otimes \mathbb{H}(M_2)}) \to d_{\mathbb{H}(M_1 \otimes M_2)}$$
...
\end{tikzpicture}
\]
together with the morphism $\iota_\mu \maps F^\prime(\kappa)(!_{1_\mathsf{A^\prime}}) \to (\theta_{1_\mathsf{A}} E(!_{1_\mathsf{A}})\phi)$ in $F^\prime(H(1_\mathsf{A}))$. The following square then commutes for any horizontal 1-cell $M$ of $F \mathbb{C}\textnormal{sp}$: 
\[
\begin{tikzpicture}[scale=1.5]
...
\begin{displaymath}
 ...
 (a,\bot_a)\ar[r,"{\begin{cases}i\maps a\to m &\textrm{in }\A \\!\maps F(i)(\bot_a)\to x &\textrm{in }F(m)\end{cases}}"]\ar[d,"{\begin{cases}f\maps a\to a' &\textrm{in }\A \\\chi_a\maps F(f)(\bot_a)\cong \bot_{a'} &\textrm{in }F(a')\end{cases}}"description] \& (m,x) \ar[d,"{\begin{cases}k\maps m\to m' &\textrm{in }\A \\ \iota\maps F(k)(x)\to x' &\textrm{in }F(m')\end{cases}}"description] \& (b,\bot_b)\ar[l,"{\begin{cases}o\maps b\to m &\textrm{in }\A \\!\maps F(o)(\bot_b)\to x &\textrm{in }F(m)\end{cases}}"']\ar[d,"{\begin{cases}g\maps b\to b' &\textrm{in }\A \\\chi_b\maps F(g)(\bot_b)\cong \bot_{b'} &\textrm{in }F(b')\end{cases}}"description] \\
...
a'\ar[r,"i'"']& m' & b'\ar[l,"o'"]
 \end{tikzcd}
\end{displaymath}
 in $\A$ along with the map $\iota\maps Fk(x)\to x'$ in $F(m')$, namely a decorated cospan 2-morphism as per \cref{eq:FCsp2morph}. Once again, this is a bijective correspondence since, as discussed above, the commutativity \cref{eq:Grothcommutativity} holds automatically by initiality of the domain.
 {\chris Christina commented out big chunks of proof. They are right below if we want to revive parts of them, if we think the reader needs deeper level of detail. The idea is that the above description suffices as fully faithful and essentially surjective evidence, but perhaps we would like to spell that out more. On to strongness next.}
...
\end{tikzpicture}
\]
together with a morphism $\iota_{M' M} \maps F(R(\alpha' \alpha))(x) \to x''$ in $F(R(x''))$. On the other hand, the individual images $\mathbb{E}_1(M)$ and $\mathbb{E}_1(M')$ are given by:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$c_1$};
\node (B) at (1.5,0) {$R(x)$};
...
\end{tikzpicture}
\]
together with a morphism $\iota \maps F(R(\alpha))(x) \to x^\prime$ of $F(R(x^\prime))$.
Thus the set $$_{\mathbb{E}(f)} {F\mathbb{C}\textnormal{sp}}_{\mathbb{E}(g)}(\mathbb{E}(M),\mathbb{E}(N))$$ consists of 4-tuples $$(f,R(\alpha),g,\iota)$$ rendering the above diagram commutative and where $f,g$ and $R(\alpha)$ are morphisms of $\mathsf{A}$ and $\iota$ is a morphism in $F(R(x'))$. The morphisms $R(\alpha) \maps R(x) \to R(x^\prime)$ and $\iota \maps F(R(\alpha))(x) \to x^\prime$ together determine the morphism $\alpha \maps x \to x^\prime$ in $\mathrm{X}$ and conversely; given two objects $x=(c,x \in F(c))$ and $x^\prime=(c^\prime,x^\prime \in F(c^\prime))$ of $\X=\inta{F}$, a morphism from $\alpha \maps x \to x^\prime$ is a pair $$(h \maps c \to c^\prime, \iota \maps F(h)(x) \to x^\prime)$$ where $h \maps c \to c^\prime$ is given by $R(\alpha) \maps R(x) \to R(x^\prime)$. This shows that $\mathbb{E}$ is fully faithful. \textbf{At least in my favorite example...}
\end{comment}
...
On the other hand, first composing $M$ and $N$ in the Grothendieck category $\scsp{L_F}{\inta F}$ using fibrewise pushouts constructed using \cref{lem:fibrewiselimits} ({\chris I could make this specific construction into a lemma earlier if you want, I have it in my notes}) produces 
\begin{displaymath}
 \begin{tikzcd}
 ...
 \end{tikzcd}
\end{displaymath}
which is mapped, under $\mathbb{E}$, to precisely the cospan $\cspn{a}{m+_bn}{b}{}{}$ as above, and the decorations are the same {\chris isomorphic would be enough but they look equal to me!} due to the following commutative diagram
\begin{displaymath}
 \begin{tikzcd}
...
{\chris Below, up to my next comment, is Kenny's earlier proof on the same thing.}
...
\end{tikzpicture}
\]
$$\iota \maps F(\eta_c)(!_c) \xrightarrow{!} !_{R(L(c))}$$
...
\end{tikzpicture}
\]
Lastly, this map of cospans comes with an isomorphism $$\iota \maps F(\sigma)(d_{\mathbb{E}(M) \odot \mathbb{E}(N)}) \to d_{\mathbb{E}(M \odot N)}$$ in $F(R(x+_{L(c_2)}x^\prime))$. This shows that $\mathbb{E}$ is strong, and so 

{\chris Up to here was Kenny's strong functoriality.}
...
{\chris In fact, Christina and Kenny believe it is an isomorphism of double categories! Bijective on all levels of objects and morphisms...should we say?}

{\chris Have not edited below, but everything should be fine up to notation changes. Happy to do if needed in the future.}
...
\end{tikzpicture}
\]
$$\iota_{{\eta_{1_\mathsf{A}}}} \maps F({\eta_{1_\mathsf{A}}})(!_{1_\mathsf{A}})) \to !_{R(L(1_\mathsf{A}))}$$
...
\node (E) at (2,-1.5) {$\iota_\mu \maps F(\kappa)(d_{\mathbb{E}(M) \odot \mathbb{E}(N)}) \to d_{\mathbb{E}(M \odot N)}$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$R(i)\eta_{c_1} + R(i^\prime)\eta_{c_1^\prime}$} (B)
...
The isomorphisms $\delta$ and $\mu$ satisfy the left and right unitality squares, associativity hexagon and braiding square. \textbf{Kenny: I think this might be a good place to cut the monoidal stuff.}{\chris Deal! They are right below.}
...
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$F(R(\lambda_{\X})(\mu_{L(1_\mathsf{A}),x_1}))(\iota_2)$} (C)
(A) edge node[left]{$\iota_1$} (D)
(E) edge node[above]{$\iota_4$} (D)
(C) edge node[right]{$F(R(\lambda_\X))(\iota_3)$} (E);
\end{tikzpicture}
\]
...
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$F((R(a_\X))(\kappa))(\iota_1)$} (B)
(B) edge node[right]{$F(R(a_\X))(\iota_2)$} (C)
(C) edge node[right]{$\iota_3$} (D)
...
(A) edge node[left]{$F(R(\beta_\X))(\iota_3)$} (D)
...
\end{tikzpicture}
\]
where $!_{c} \maps L(c) \to x$ is the unique morphism from the trival decoration on $c$ given by $$!_{c} \mapseqq 1 \xrightarrow{\phi} F(0) \xrightarrow{F(!)} F(c)$$ to $x \in F(c)$. In other words, the trivial decoraction $!_{c}$ is initial in $F(c)$. Similarly, given a 2-morphism $(f,h,g,\iota) \maps M \to M^\prime$ of $F\mathbb{C}\textnormal{sp}$:
\[
\begin{tikzpicture}[scale=1.5]
\node (D) at (1,0.5) {$x \in F(c)$};
...
\end{tikzpicture}
\]
$$\iota \maps F(h)(x) \to x^\prime$$
the image $\mathbb{G}(f,h,g,\iota)$ in $_L \mathbb{C}\textnormal{sp}(\X)$ is given by the 2-morphism:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(c_1)$};
\node (B) at (1.5,0) {$x$};
...
\end{tikzpicture}
\]
where $\alpha \maps x \to x^\prime$ is the morphism in the Grothendieck construction of $F$ given by $\alpha = (h \maps c \to c^\prime, \iota \maps F(h)(x) \to x^\prime)$. 
...
\end{tikzpicture}
\]
together with a graph morphism $\iota \maps F(h)(G_1) \to G_2$ in $F(P_2)$.}
\end{enumerate}
...
\end{tikzpicture}
\]
together with a morphism of $k$-graphs $\iota \maps F(h)(K_1) \to K_2$ in $F(V_2)$.}
\end{enumerate}
...
\end{tikzpicture}
\]
together with a map of Markov processes $\iota \maps F(h)(M_1) \to M_2$ in $F(N_2)$, where a map between two Markov processes to be given by a pair of functions $(g,h)$ that make the following diagram commute:
\[
\begin{tikzpicture}[scale=1.5]
...
\end{tikzpicture}
\]
together with a morphism of Petri nets $\iota \maps F(h)(P_1) \to P_2$ in $F(Z_2)$.}
\end{enumerate}
...
{\chris Here, I basically rearranged things to make them come in a certain order, and also added a few things for monoidal bicats, maybe more is needed or not. Didn't change Kenny's notation or writings almost at all}
...
{\chris Hiding various details under the rug - e.g. what a symmetric monoidal bicategory is. Should be fine? Also, not sure if we need monoidal transformations?}
...
{\chris Pretty sure the def below can be incorporated above, however got a bit lost in different notation. Kenny Fix?}
\begin{defn}
