...
\]
We might hope to do this using symmetric lax monoidal functor $F \maps (\Fin\Set, +) \to (\Set, \times)$ assiging to each finite set $m$ the set of all graphs with $m$ as their set of nodes    But this hope is doomed, for reasons painstakingly explained in \cite[Sec.\ 5]{BC}.  There is really a \emph{category} of graphs with $m$ as their set of nodes, and trying to treat it as a mere set does not work.
...
a pseudofunctor, the opfibration $R \maps \X \to \A$ is also a right adjoint.  From the accompanying left adjoint $L \maps \A \to \X$, we  construct a symmetric monoidal double category ${}_L \lCsp(\X)$ of structured cospans.  Finally, in \cref{Equiv} we prove that this structured cospan double category ${}_L \lCsp(\X)$ is equivalent to the decorated cospan double category $F \lCospan$. 
...
In this section we build the symmetric monoidal double category $F \lCsp$ mentioned in the introduction. Then we obtain an underlying symmetric monoidal bicategory $F \mathbf{Csp}$ using a result of Shulman \cite{Shul} and finally decategorify this to obtain a symmetric monoidal category $F\mathsf{Csp}$ which is a generalization of Fong's original decorated cospan category \cite{Fong}.  The definition of a lax monoidal pseudofunctor is recalled in \cref{sec:2cats}, and the basics of double categories are recalled in \cref{sec:doublecats}.
...
Let $\A$ be a category with chosen finite colimits and $F \maps \A \to \Cat$ a lax monoidal pseudofunctor. Then there exists a double category $F\lCsp$ with:
\begin{itemize}
\item{objects of $\A$ as objects,}
\item{morphisms of $\A$ as vertical 1-morphisms,}
\item{$F$-decorated cospans as horizontal 1-cells, where an \define{$F$-decorated cospan} is a pair given by a cospan in $\A$ together with a decoration on the cospan's apex:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$(a$};
\node (B) at (1,0) {$m$};
...
\end{tikzpicture}
\]}
\item{maps of $F$-decorated cospans as 2-morphisms, where a \define{map of $F$-decorated cospans} is a pair given by a map of cospans in $\A$:
...
\end{tikzpicture}
\end{equation}
together with a \define{morphism of decorations} $\tau \maps F(h)(x) \to x'$ in $F(m')$.}
\end{itemize}
\end{thm}
...
\end{tikzpicture}
\]
together with the composite morphism of decorations $\tau' \tau \colon F(h'h)(x) \to x'$ in $F(m'')$. This completes the descriptions of the category of objects $F\lCsp_0 = \A$ and the category of arrows $F\lCsp_1$.
...
\end{tikzpicture}
\]
where $!_a \in F(a)$ is the \define{trivial decoration} on $a$ given by the composite of the unique map $F(!) \maps F(0) \to F(a)$ and the morphism $\phi \maps 1 \to F(0)$  which comes from the structure of the lax monoidal pseudofunctor $F \maps \A \to \Cat$. For morphisms, the functor $U$ is defined as:
\[
\begin{tikzpicture}[scale=1.5]
\node (G) at (-1,0.5) {$a$};
\node (G') at (-1,-0.5) {$a'$};
...
Next we define the composite functor $\odot \colon F\lCsp_1 \times_{F\lCsp_0} F\lCsp_1 \to F\lCsp_1$, which amounts to defining how we compose two composable $F$-decorated cospans or two maps of $F$-decorated cospans with a common source and target. Given two composable $F$-decorated cospans which we respectively denote by $M$ and $N$, the capitals of their apices:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,0) {$m$};
...
(D) edge node [above] {$j'$} (F)
(F) edge node [left] {$\psi$} (G)
(A) edge[bend left] node [left] {$\psi j i$} (G)
(E) edge[bend right] node [right] {$\psi j' o'$} (G);
\end{tikzpicture}
\end{equation}
...
\begin{proof}
First we note that the category of objects $F\lCsp_0=\A$ is symmetric monoidal under binary coproducts and the left and right unitors, associators and braidings are given as natural maps. The category of arrows $F\lCsp_1$ has:
\begin{enumerate}
...
\end{tikzpicture}
\]
together with a morphism $\tau \maps F(h)(x) \to x'$.
}
\end{enumerate}
Given two objects $M_1$ and $M_2$ of $F\lCsp_1$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a_1$};
\node (B) at (1,0) {$m_1$};
...
Given three objects $M_1, M_2$ and $M_3$ in $F\lCsp_1$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a_1$};
\node (B) at (1,0) {$m_1$};
...
\begin{proof}
Let $f \maps c \to c'$ be a vertical 1-morphism in $F\lCsp$. We can lift $f$ to the companion horizontal 1-cell $\hat{f}$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$c$};
\node (B) at (1,0) {$c'$};
...
\begin{thm}
Let $\A$ be a category with finite colimits and $F \maps (\A, +) \to \Cat$ a symmetric lax monoidal pseudofunctor. Then there exists a symmetric monoidal bicategory $F \mathbf{Csp} \mapseqq H(F\lCsp)$ which has:
\begin{enumerate}
\item{objects as those of $\A$,}
...
\begin{proof}
This follows immediately by applying \cref{Shul} to the fibrant symmetric monoidal double category $F\lCsp$.
\end{proof}

This symmetric monoidal bicategory $F\mathbf{Csp}$ is a superior version of the symmetric monoidal bicategory $F$Cospan$(\A)$ constructed earlier by the second author \cite{Courser} in that there is greater flexibility in what 2-morphisms are allowed. 
...
\end{tikzpicture}
\]
these two $F$-decorated cospans are in the same isomorphism class if there exists an isomorphism $f \maps c \to c'$ such that following diagram commutes:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B') at (1,0.5) {$c$};
...
\begin{thm}\label{SC}
Given a category $\X$ with finite colimits and a category $\A$ with finite coproducts and a finite coproduct preserving functor $L \maps \A \to \X$ with $\A$ and $\X$, there exists a symmetric monoidal double category $_L \lCsp(\X)$ which has:
\begin{itemize}
...
Using a result of Shulman \cite{Shul}, each of the isofibrant symmetric monoidal double categories $F\lCsp$ and $_L \lCsp(\mathrm{X})$ give rise to underlying symmetric monoidal bicategories, namely $F\lCsp$ induces a symmetric monoidal bicategory $F\mathbf{Csp} \mapseqq H(F\lCsp)$ which has:
\begin{enumerate}
\item{objects as those of $\A$,}
\item{morphisms as horizontal 1-cells of $F\lCsp$, and}
\item{2-morphisms as globular 2-morphisms of $F\lCsp$.}
\end{enumerate} 
Likewise, $_L \lCsp(\X)$ induces a symmetric monoidal bicategory $H( {_L \lCsp(\X)})$ which has:
\begin{enumerate}
\item{objects as those of $\A$,}
\item{morphisms as horizontal 1-cells of $_L \lCsp(\X)$, and}
\item{2-morphisms as globular 2-morphisms of $_L \lCsp(\X)$.}
\end{enumerate}
...
\end{enumerate}
These two double categories $_L \lCsp(\mathsf{Graph}_k)$ and $F\lCsp$ are equivalent by Theorem \cref{Equiv}.
...
\end{enumerate}
The two double categories $F\lCsp$ and $_L \mathbb{C} \textnormal{sp}(\mathsf{Mark})$ are equivalent by Theorem \cref{Equiv}.
...
%\begin{defn}
%Let $P \maps \ca{X} \to \ca{A}$ be a functor. A morphism $f \maps d_1 \to d_{2}$ in the category $\mathrm{D}$ is \textbf{cocartesian (with respect to the functor P)} if for any object $d'$ in $\mathrm{D}$ and morphism $g \maps d_1 \to d'$ and every $p \maps P(d_{2}) \to P(d')$ such that $P(g)=p P(f)$, there exists a unique $h \maps d_{2} \to d'$ such that $g=hf$ and $p=P(h)$.
%\[
%\begin{tikzpicture}[scale=1.5]
%\node (A) at (0,0) {$d'$};
%\node (B) at (0,-1) {$d_{2}$};
...
%\end{defn}
%\begin{defn}
%A functor $P \maps \mathrm{D} \to \mathrm{A}$ is a \textbf{(Grothendieck) opfibration} if for any object $d$ in $\mathbf{D}$ and morphism $f \maps c %\to P(d)$ there exists a cocartesian morphism $\phi \maps d' \to d$ such that $P(\phi)=f$.
%\end{defn}
%\begin{defn}
%Let $P \maps \mathrm{D} \to \mathrm{A}$ and $P' \maps \mathrm{D'} \to \mathrm{A}$ be opfibrations over a category $\mathrm{A}$. A \define{morphism of opfibrations} is a functor $F \maps \mathrm{D} \to \mathrm{D}'$ such that $F(p)$ is cocartesian with respect to $P'$ if $p$ is cocartesian with respect to $P$ and $P=P' F$.
%\end{defn}
