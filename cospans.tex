...
Notice a subtlety in the above formulation: \cref{lem:MonGroth} provides a specifically induced monoidal structure on the fibres which becomes \cref{eq:explicitstructure1} in the cocartesian base case. If that fibrewise monoidal structure happens to itself be cocartesian, then it is necessarily isomorphic to \emph{any} other cocartesian structure on the fibres, by universal property of colimits. Therefore, we can simplify the above formulation to the one of the very \cref{thm:equiv} without having to specify that the structures came from monoidality of the pseudofunctor. DOES THIS MAKE SENSE TO ALL?? 
