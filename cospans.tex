...
Petri nets are widely used as models of systems in computer science, chemistry, population biology and epidemiology \cite{GiraultValk,Peterson}.   Structured cospans have been used to define a symmetric monoidal double category of `open Petri nets' \cite{BM}, which lets us build large Petri nets out of smaller pieces.  We can also use decorated cospans to create a double category of open Petri nets.  Again this example is very similar to the example of open graphs.
...
We now show that while open dynamical systems are morphisms in a decorated cospan category, they do not arise from a structured cospan category via \cref{thm:equiv}.  To do this, recall that in this result we start with the data required for a decorated cospan category, namely a symmetric lax monoidal pseudofunctor $F \maps (\A,+) \to (\Cat,\times)$, and show the resulting opfibration $U \maps \inta F \to \A$ has a left adjoint $L \maps \A \to \X$.   
We shall show that in the case at hand, where $F = D$, the opfibration $U \maps \inta F \to \A$ does \emph{not} have a left adjoint.
...
\node[below=.01 of D] {\rotatebox{90}{$\simeq$}};
...
\end{tikzpicture}
\]
The morphisms $\tau_1$ and $\tau_2$ may be composed as $$\tau_2 F(\hat{\chi})(\tau_1) \colon F(\hat{\chi})F(\beta \odot \beta)((x_1 \oplus y_1) \odot (x_2 \oplus y_2)) \to (y_1 \odot y_2) \oplus (x_1 \odot x_2)$$ which is given by the diagram:
\[
\begin{tikzpicture}[scale=1.7]
...
\node () at (7.875,-1) {$\scriptstyle{\cong}$};
...
(B') edge node [above] {$F(\psi)$} (C')
(C) edge node [fill=white] {$F(\beta \odot \beta)$} (C')
...
\end{tikzpicture}
\]
and the morphisms $\tau_3$ and $\tau_4$ may be composed as $$\tau_4 F(\beta)(\tau_3) \colon F(\beta) F(\hat{\chi}) \colon F(\beta)F(\hat{\chi})((x_1 \oplus y_1)\odot(x_2 \oplus y_2)) \to (y_1 \odot y_2) \oplus (x_1 \odot x_2)$$ which is given by the diagram:
\[
\begin{tikzpicture}[scale=1.7]
%\node (A) at (3.85,0) {$\scriptstyle{\cong}$};
%\node (A') at (3.85,-1) {$\scriptstyle{\cong}$};
...
\node () at (7.875,0) {$\scriptstyle{\cong}$};
