...
\end{tikzpicture}
\]
where given a decoration $x \colon 1 \to F(m)$ on $m$, we obtain a decoration on $H(m)$ via the composite $$1 \xrightarrow{\phi} E(1) \xrightarrow{E(x)} E(F(m)) \xrightarrow{\theta_m} F'(H(m)).$$}
