...
\]
We might hope to do this using a symmetric lax monoidal functor $F \maps (\Fin\Set, +) \to (\Set, \times)$ assigning to each finite set $m$ the set of all graphs with $m$ as their set of nodes. But this hope is doomed, for reasons painstakingly explained in \cite[Section 5]{BC2}. 
...
\]
\end{itemize}
Many of the flawed applications of decorated cospans have been fixed using structured cospans \cite[Section 6]{BC2}, but not all decorated cospans can handled using structured cospans.
...
\end{thm}
%%% Christina: note to self, now our functor factors through that specific Rex means that each category F(a) which later becomes the fiber comes with a specific choice of colimits. We do not know what this choice is, but of course such colimits in the fibers will be isomorphic to any other we may stumble upon!
...
\end{equation}
Moreover, each fiber category $\X_a=F(a)$ obtains a monoidal structure via 
...
Moreover in this situation the $\J$-colimits in $\X$ can be chosen so that $U$ strictly preserves them.  
\end{enumerate}
\end{lem}
...
\& (b,y)\ar[ul,"{\begin{cases}f\maps b\to a &\textrm{in }\A \\k\maps F(f)y\to x &\textrm{in }F(a)\end{cases}}"description]\ar[ur,"{\begin{cases}g\maps b\to c &\textrm{in }\A \\\ell\maps F(g)y\to z &\textrm{in }F(c)\end{cases}}"description] \&
 \end{tikzcd}
\end{equation}
which on the first variable is given by the pushout in $\A$ on the left below, and the object $w$ is formed as the pushout in the fiber category $F(a+_bc)$ on the right below:
\begin{displaymath}
\begin{tikzcd}
...
\begin{tikzcd}
 ...
 \end{tikzcd}
\end{displaymath}
In a similar way we can construct the initial object and coproducts in a Grothendieck category from those in the fibers, namely
...
\end{equation}
where $\iota_a:a\to a+b\leftarrow b:\iota_b$ are the sum inclusions in $\A$ and the sum on the second variable is in the fiber $F(a+b)$.
...
On a highly related note, in what follows we are also interested in the existence of a left adjoint $L$ to an induced monoidal opfibration.  The following result provides sufficient conditions for that.  Following Gray \cite{Gray}, let us define a `right adjoint left inverse' or  \define{rali} to be a right adjoint in an adjunction for which the unit is the identity.  

\begin{lem}[\textbf{Gray}] \label{prop:opfibtolari}
Let $U \maps\X \to \A$ be an opfibration. Then to make $U$ into a rali, it suffices to equip
its fibers with initial objects that are preserved up to isomorphism by the reindexing functors.
\end{lem}

\begin{proof}
 This is \cite[Proposition~4.4]{Gray}.    Suppose each fiber $\X_a$ of the opfibration $U$ is equipped with an initial object $\bot_a$, and these objects are preserved up to isomorphism by the reindexing functors.   Define $L\maps\A\to\X$ on objects $a \in \A$ by $L(a) = \bot_a$.   Given a morphism $f\maps a\to a'$ in $\A$, define $L(f)$ to be the composite
 \begin{equation}\label{eq:Lonarrows}
  \bot_a\xrightarrow{\mathrm{Cocart}(f,\bot_a)}f_!(\bot_a)\xrightarrow{\;\chi_a\;}\bot_{a'}
 \end{equation}
where $\mathrm{Cocart}(f,\bot_a)$ is the cocartesian lifting of $f$ to $\bot_a$  and $\chi_a$ is the unique isomorphism between two initial objects in the fiber above $a'$.   One can then make $U$ into a right adjoint of $L$ with unit $\iota_a \maps a \to U(L(a))$ being the identity, using the fact that $U(L(a)) = U(\bot_a) = a$.
\end{proof}
...
\end{equation}
with $x\in F(m)$; recall that the reindexing functors $F(i),F(o)$ preserve all finite colimits across the fiber categories.
...
\end{gather}
in the fiber category $F(m')$, by composition in the Grothendieck category \cref{eq:compGrothcat}. Since both morphisms emanate from the mapped initial object in $F(m')$ under the finite-colimit-preserving reindexing functors, they are the unique such into $x'$ therefore this gives no extra conditions; similarly for the equality including $o,o'$.
...
Structured and decorated cospans are a powerful tool for studying categories where the morphisms are electrical circuits---see \cite[Section 6.1]{BC2} and \cite{BCR,BF}.  The key idea is to use open graphs with labeled edges to describe circuits, where the labels can stand for resistors with any chosen resistance, capacitors with any chosen capacitance, or other circuit elements.   The whole theory of open graphs discussed in the previous section can be recapitulated for labeled graphs.  Since the abstract formalism works the same way, we can be brief.   Concrete applications of this formalism are discussed in the references.
...
Even though for both \cref{thm:equiv} and the above result the conditions required are only sufficient, they suggest that with work we could establish a functorial picture
\begin{displaymath}
\begin{tikzpicture}
...
\node[below left=2 and 1 of A,draw] (E)  {Special ralis};
...
(D) edge (A)
(E) edge (B)
(B) edge node[below,sloped]{$L\mapsto {}_{L}\lCsp(\X)$} (C);
\end{tikzpicture}
\end{displaymath}
with an actual natural isomorphism in the middle.  
This would imply that starting from an appropriate middle ground, employing the structured or the decorated cospan formalism is essentially the same. We leave such considerations for future work.
...
\end{displaymath}
The category $\X$ is called the \textbf{total} category and $\A$ is called the \textbf{base} category of the opfibration. For any $a\in\A$, the \textbf{fiber category} $\X_a$ consists of all objects that map to $a$ and \define{vertical} morphisms between them, i.e., morphisms mapping to $1_a$.   

Assuming the axiom of choice, we may select a cocartesian lifting of each morphism $f\maps a\to b$ in $\A$ to each $x\in\X _a$, denoted by $\mathrm{Cocart}(f,x)\maps x\to f_!(x)$, rendering $U$ a so-called \textbf{cloven} opfibration. This choice induces \textbf{reindexing functors} $f_!\maps\X _a\to\X _b$ between the fiber categories, which by the lifting's universal property come equipped with natural isomorphisms $(1_a)_!\cong 1_{\X _a}$ and $(f\circ g)_!\cong f_!\circ g_!$.   With the help of these, any cloven opfibration $U \maps \X \to \A$ gives a pseudofunctor $F \maps \A \to \Cat$, where $\A$ viewed as a 2-category with trivial 2-cells, $F(a) = \X_a$ for each object $a \in \A$, and $F(f) = f_!$ for each morphism $f$ in $\A$.
...
\end{equation}
This is an opfibered category over $\A$ via the obvious forgetful functor, with fiber categories $(\inta F)_a=F(a)$ and reindexing functors $f_!=F(f)$.
\end{defn}
...
For double categories we follow the notation of our paper on structured cospans \cite{BC2}, which in turn follows that of Hansen and Shulman \cite{HS,Shulman2010}. 
...
obeying a list of equations that can be found after \cite[Definition 2.10]{HS} and also \cite[Definition A.5]{BC2}.
\end{itemize}
...
\[
		\begin{tikzpicture}
			\node (a) at (-4,0) {$a+a'$};
			\node (b) at (1,0) {$(m_2+n_2) +_{(b+b')} (m_1+n_1)$};
			\node (c) at (6,0) {$c+c'$};
			\node (a2) at (-4,1) {$a'+a$};
			\node (b2) at (1,1) {$(n_2+m_2) +_{(b'+b)} (n_1+m_1)$};
			\node (c2) at (6,1) {$c'+c$};
            \node (a3) at (-4,2) {$a'+a$};
			\node (b3) at (1,2) {$(n_2 +_{b'} n_1) + (m_2 +_b m_1)$};
			\node (c3) at (6,2) {$c'+c$};
            \node (a5) at (-4,-1) {$a+a'$};
			\node (b5) at (1,-1) {$(m_2+_b m_1) + (n_2+_{b'} n_1)$};
			\node (c5) at (6,-1) {$c+c'$};
            \node (a6) at (-4,-2) {$a'+a$};
			\node (b6) at (1,-2) {$(n_2 +_{b'} n_1) + (m_2 +_b m_1)$};
			\node (c6) at (6,-2) {$c'+c$};
			\path[->,font=\scriptsize,>=angle 90]
			(a) edge node[above]{$$} (b)
			(c) edge node[above]{$$} (b)
...
and the inwards pointing morphisms are natural maps from the each cospan's feet to its apex.
...
%			\node (a) at (-4,0) {$a+a'$};
			\node (b) at (1,0) {$(m_2+n_2) +_{(b+b')} (m_1+n_1)$};
			\node (c) at (8,0) {$ (x_2\oplus y_2)\odot (x_1\oplus y_1)  \in F((m_2+n_2)+_{b+b'}(m_1+n_1))$};
%			\node (a2) at (-4,1) {$a'+a$};
			\node (b2) at (1,1) {$(n_2+m_2) +_{(b'+b)} (n_1+m_1)$};
			\node (c2) at (8,1) {$(y_2\oplus x_2)\odot(y_1\oplus x_1) \in F((n_2+m_2)+_{b'+b}(n_1+m_1))$};
%                            \node (a3) at (-4,2) {$a'+a$};
			\node (b3) at (1,2) {$(n_2 +_{b'} n_1) + (m_2 +_b m_1)$};
			\node (c3) at (8,2) {$(y_2 \odot y_1) \oplus (x_2 \odot x_1) \in F((n_2+_{b'} n_1)+(m_2+_b m_1))$};
%                                \node (a5) at (-4,-1) {$a+a'$};
			\node (b5) at (1,-1) {$(m_2+_b m_1) + (n_2+_{b'} n_1)$};
			\node (c5) at (8,-1) {$(x_2 \odot x_1) \oplus (y_2 \odot y_1) \in F((m_2+_b m_1)+(n_2+_{b'} n_1))$};
%                                \node (a6) at (-4,-2) {$a'+a$};
			\node (b6) at (1,-2) {$(n_2 +_{b'} n_1) + (m_2 +_b m_1)$};
			\node (c6) at (8,-2) {$(y_2 \odot y_1) \oplus (x_2 \odot x_1) \in F((n_2+_{b'} n_1)+(m_2+_b m_1))$};
			\path[->,font=\scriptsize,>=angle 90]
%			(a) edge node[above]{$$} (b)
%			(c) edge node[above]{$$} (b)
%                                (a2) edge node[above]{$$} (b2)
...
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\tau_1 \colon F(\beta +_\beta \beta)((x_2\oplus y_2)\odot(x_1\oplus y_1)) \to (y_2\oplus x_2)\odot(y_1\oplus x_1)$};
\node (B) at (0,-.5) {$\tau_2 \colon F(\hat{\chi})((y_2 \oplus x_2)\odot(y_1 \oplus x_1)) \to (y_2\odot y_1) \oplus (x_2\odot x_1)$};
\node (C) at (0,-1) {$\tau_3 \colon F(\hat{\chi})((x_2 \oplus y_2)\odot(x_1 \oplus y_1)) \to (x_2\odot x_1) \oplus (y_2\odot y_1)$};
\node (D) at (0,-1.5) {$\tau_4 \colon F(\beta)((x_2\odot x_1)\oplus(y_2\odot y_1)) \to (y_2\odot y_1)\oplus(x_2\odot x_1)$};
\path[->,font=\scriptsize,>=angle 90]
...
\begin{displaymath}
\scalebox{.8}{$F(\beta\hat{\chi})((x_2 \oplus y_2)\odot(x_1 \oplus y_1))\cong F(\beta)(F\hat{\chi})((x_2 \oplus y_2)\odot(x_1 \oplus y_1))\xrightarrow{F(\beta)(\tau_3)}F(\beta)((x_2\odot x_1) \oplus (y_2\odot y_1))\xrightarrow{\tau_4}(y_2 \odot y_1) \oplus (x_2 \odot x_1)$}
\end{displaymath}
...
\node (E) at (6.75,0.5) {$\scriptstyle{F(m_2 + n_2) \times F(m_1+n_1)}$};
...
\node (E') at (6.75,-0.5) {$\scriptstyle{F(m_2+m_1) \times F(n_2+n_1)}$};
\node (B) at (9,0.5) {$\scriptstyle{F((m_2+n_2)+(m_1+n_1))}$};
\node (B') at (9,-0.5) {$\scriptstyle{F(m_2+_b m_1)\times F(n_2+_{b'}n_1)}$};
\node (C) at (11.5,0.5) {$\scriptstyle{F((m_2+n_2)+_{b+b'}(m_1+n_1))}$};
\node (C') at (11.5,-0.5) {$\scriptstyle{F((m_2+_b m_1)+ (n_2+_{b'} n_1))}$};
\node (G) at (4.25,0.5) {$\scriptstyle{F(m_2) \times F(n_2) \times F(m_1) \times F(n_1)}$};
\node (G') at (4.25,-0.5) {$\scriptstyle{F(m_2) \times F(m_1) \times F(n_2) \times F(n_1)}$};
\node (G'') at (4.25,-1.5) {$\scriptstyle{F(n_2) \times F(n_1) \times F(m_2) \times F(m_1)}$};
\node (E'') at (6.75,-1.5) {$\scriptstyle{F(n_2+n_1) \times F(m_2+m_1)}$};
\node (B'') at (9,-1.5) {$\scriptstyle{F(n_2+_{b'}n_1) \times F(m_2+_b m_1)}$};
\node (C'') at (11.5,-1.5) {$\scriptstyle{F((n_2+_{b'}n_1)+(m_2+_b m_1))}$};
\node (X) at (3.4,0.2) {$\scriptscriptstyle{x_2 \times y_2 \times x_1 \times y_1}$};
\node (X') at (3.4,-0.35) {$\scriptscriptstyle{x_2 \times x_1 \times y_2 \times y_1}$};
\node (X'') at (3.4,-1.2) {$\scriptscriptstyle{y_2 \times y_1 \times x_2 \times x_1}$};
\path[->,font=\scriptsize,>=angle 90]
(B') edge node [fill=white] {$\beta$} (B'')
(E) edge node [above] {$\phi_{m_2+n_2,m_1+n_1}$} (B)
(E'') edge node [below] {$F\psi\times F\psi$} (B'')
(E') edge node [above] {$F\psi\times F\psi$} (B')
(B) edge node [above] {$F(\psi)$} (C)
(B'') edge node [below] {$\phi_{n_2+_{b'}n_1,m_2+_b m_1}$} (C'')
(B') edge node [above] {$\phi_{m_2+_b m_1,n_2+_{b'}n_1}$} (C')
(C) edge node [fill=white] {$F(\hat{\chi})$} (C')
(C') edge node [fill=white] {$F(\beta)$} (C'')
(G) edge node [above] {$\phi_{m_2,n_2} {\times} \phi_{m_1,n_1}$} (E)
(G'') edge node [below] {$\phi_{n_2,n_1} {\times} \phi_{m_2,m_1}$} (E'')
(G') edge node [above] {$\phi_{m_2,m_1} \times \phi_{n_2,n_1}$} (E')
...
\node (E') at (6.75,-0.5) {$\scriptstyle{F(m_2+m_1) \times F(n_2+n_1)}$};
\node (B) at (9,0.5) {$\scriptstyle{F((m_2+n_2)+(m_1+n_1))}$};
\node (B') at (9,-0.5) {$\scriptstyle{F((m_2+ m_1)+(n_2+n_1))}$};
\node (C) at (11.5,0.5) {$\scriptstyle{F((m_2+n_2)+_{b+b'}(m_1+n_1))}$};
\node (C') at (11.5,-0.5) {$\scriptstyle{F((m_2+_b m_1)+ (n_2+_{b'} n_1))}$};
\node (G) at (4.25,0.5) {$\scriptstyle{F(m_2) \times F(n_2) \times F(m_1) \times F(n_1)}$};
\node (G') at (4.25,-0.5) {$\scriptstyle{F(m_2) \times F(m_1) \times F(n_2) \times F(n_1)}$};
\node (G'') at (4.25,-1.5) {$\scriptstyle{F(n_2) \times F(n_1) \times F(m_2) \times F(m_1)}$};
\node (E'') at (6.75,-1.5) {$\scriptstyle{F(n_2+n_1) \times F(m_2+m_1)}$};
\node (B'') at (9,-1.5) {$\scriptstyle{F((n_2+n_1)+(m_2+ m_1))}$};
\node (C'') at (11.5,-1.5) {$\scriptstyle{F((n_2+_{b'}n_1)+(m_2+_b m_1))}$};
\node (X) at (3.4,0.2) {$\scriptscriptstyle{x_2 \times y_2 \times x_1 \times y_1}$};
\node (X') at (3.4,-0.35) {$\scriptscriptstyle{x_2 \times x_1 \times y_2 \times y_1}$};
\node (X'') at (3.4,-1.2) {$\scriptscriptstyle{y_2 \times y_1 \times x_2 \times x_1}$};
...
(E) edge node [above] {$\phi_{m_2+n_2,m_1+n_1}$} (B)
(E'') edge node [above] {$\phi_{n_2+n_1,m_2+m_1}$} (B'')
(E') edge node [above] {$\phi_{m_2+m_1,n_2+n_1}$} (B')
(B) edge node [above] {$F(\psi)$} (C)
...
(C') edge node [fill=white] {$F(\beta)$} (C'')
(G) edge node [above] {$\phi_{m_2,n_2} {\times} \phi_{m_1,n_1}$} (E)
(G'') edge node [above] {$\phi_{n_2,n_1} {\times} \phi_{m_2,m_1}$} (E'')
(G') edge node [above] {$\phi_{m_2,m_1} \times \phi_{n_2,n_1}$} (E')
(G) edge node [fill=white] {$1 \times \beta \times 1$} (G')
(G') edge node [fill=white] {$\beta$} (G'')
(D) edge[out=90,in=180] node [left,above] {$$} (G)
...
\begin{displaymath}
\scalebox{.8}{$F(\hat{\chi}(\beta +_\beta \beta))( (x_2 \oplus y_2)\odot (x_1 \oplus y_1))\cong F(\hat{\chi})F(\beta +_\beta \beta)( (x_2 \oplus y_2)\odot (x_1 \oplus y_1)) \xrightarrow{F(\hat{\chi})(\tau_1)}F(\hat{\chi})(y_2\oplus x_2)\odot(y_1\oplus x_1)\xrightarrow{\tau_2}(y_2 \odot y_1) \oplus (x_2 \odot x_1)$}
\end{displaymath}
...
\node () at (12,-1) {$\scriptstyle\cong$};
\node (E') at (6.75,-0.5) {$\scriptstyle{F(n_2+m_2) \times F(n_1+m_1)}$};
\node (B) at (9,0.5) {$\scriptstyle{F((m_2+n_2)+(m_1+n_1))}$};
\node (B') at (9,-0.5) {$\scriptstyle{F((n_2+m_2)+(n_1+m_1))}$};
\node (C) at (11.5,0.5) {$\scriptstyle{F((m_2+n_2)+_{b+b'}(m_1+n_1))}$};
\node (C') at (11.5,-0.5) {$\scriptstyle{F((n_2+m_2)+_{b'+b}(n_1+m_1))}$};
\node (G) at (4.25,0.5) {$\scriptstyle{F(m_2) \times F(n_2) \times F(m_1) \times F(n_1)}$};
\node (G') at (4.25,-0.5) {$\scriptstyle{F(n_2) \times F(m_2) \times F(n_1) \times F(m_1)}$};
\node (G'') at (4.25,-1.5) {$\scriptstyle{F(n_2) \times F(n_1) \times F(m_2) \times F(m_1)}$};
\node (E'') at (6.75,-1.5) {$\scriptstyle{F(n_2+n_1) \times F(m_2+m_1)}$};
\node (B'') at (9,-1.5) {$\scriptstyle{F(n_2+_{b'}n_1)\times F(m_2+_b m_1)}$};
\node (C'') at (11.5,-1.5) {$\scriptstyle{F((n_2+_{b'}n_1)+(m_2+_b m_1))}$};
\node (X) at (3.4,0.2) {$\scriptscriptstyle{x_2 \times y_2 \times x_1 \times y_1}$};
\node (X') at (3.4,-0.35) {$\scriptscriptstyle{y_2 \times x_2 \times y_1 \times x_1}$};
\node (X'') at (3.4,-1.2) {$\scriptscriptstyle{y_2 \times y_1 \times x_2 \times x_1}$};
\path[->,font=\scriptsize,>=angle 90]
...
(E) edge node [above] {$\phi_{m_2+n_2,m_1+n_1}$} (B)
(E'') edge node [below] {$F\psi\times F\psi)$} (B'')
(E') edge node [above] {$\phi_{n_2+m_2,n_1+m_1}$} (B')
(B) edge node [above] {$F(\psi)$} (C)
(B'') edge node [below] {$\phi_{n_2+_{b'}n_1,m_2+_{b}m_1}$} (C'')
(B') edge node [above] {$F(\psi)$} (C')
(C) edge node [fill=white] {$F(\beta +_\beta \beta)$} (C')
(C') edge node [fill=white] {$F(\hat{\chi})$} (C'')
(G) edge node [above] {$\phi_{m_2,n_2} {\times} \phi_{m_1,n_1}$} (E)
(G'') edge node [below] {$\phi_{n_2,n_1} {\times} \phi_{m_2,m_1}$} (E'')
(G') edge node [above] {$\phi_{n_2,m_2} \times \phi_{n_1,m_1}$} (E')
