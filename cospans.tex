...
\begin{thm}\label{DC}
Let $\A$ be a category with finite colimits and let $F \maps (\A,+) \to (\Cat,\times)$ a symmetric lax monoidal pseudofunctor. Then the double category $F\lCsp$ is symmetric monoidal, where the tensor product
\begin{itemize}
...
\end{tikzcd}\qquad1\xrightarrow{x_1\times x_2}F(m_1)\times F(m_2)\xrightarrow{\phi_{m_1,m_2}}F(m_1+m_2)
\end{equation}
%$M_1$ and $M_2$:
%\[
%\begin{tikzpicture}[scale=1.5]
...
A \define{monoidal double category} is a pseudomonoid in $\Dbl$. Explicitly, it is a double category $\lD$ with:
\begin{itemize}
\item monoidal structures on both $\lD_0$
and $\lD_1$, each with tensor product denoted $\otimes$, associator $a$, left unitor $\ell$ and right unitor $r$, such that the unit object of $\lD_0$ is preserved by the unit functor $U$, and the source and target functors $S,T \maps \lD_1 \to \lD_0$ are strict monoidal, 
...
\end{defn}
%This definition neatly packages a large quantity of information.   When we unpack it, we see 
