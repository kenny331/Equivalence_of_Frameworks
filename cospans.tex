...
Similarly the right and left unitors can be constructed: for example, for a horizontal 1-cell $M=(a\to m \leftarrow b,x\in F(m))$, the isomorphism $M\odot U_a\cong M$ is given by the canonical map of cospans $\kappa\colon a+_am\cong m$ along with the isomorphism between decorations
\begin{displaymath}
 \scalebox{.9}
...
\&\& F(0+m)\ar[drr]\ar[ur,"F(!_a\times1)"'] \&\&  \\
1\ar[rrrr,"x"] \&\phantom{A}\&\&\& F(m)
 \end{tikzcd}}
\end{displaymath}
All data can be verified to satisfy the axioms of a double category---essentially breaking down to those of the cospan double category of $\A$ and axioms of the lax monoidal pseudofunctor $F$.
\end{proof}
...
since the right hand side of the first equation is given by $U_f$ and the right hand side of the second equation is given as in Definition \ref{def:companion}.
