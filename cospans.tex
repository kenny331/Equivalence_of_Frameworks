...
\begin{proof}
 ...
 \end{equation}
where $\mathrm{Cocart}(f,\bot_a)$ is the cocartesian lifting of $f$ to $\bot_a$  and $\chi_a$ is the unique isomorphism between these two initial objects in the fiber above $a'$.   One can then make $U$ into a right adjoint of $L$ with unit $\iota_a \maps a \to U(L(a))$ being the identity, using the fact that $U(L(a)) = U(\bot_a) = a$.
\end{proof}
