...
Chemists sometimes use Petri nets where the places represent `chemical species'---types of molecules, ions and the like---and the transitions represent chemical reactions.   In these Petri nets, which we shall call `Petri nets with rates', the transitions are labelled with positive real numbers called `rate constants'.  This allows one to systematically construct a dynamical system from any Petri net with rates, and mathematical chemists have proved deep theorems relating the topology of Petri nets with rates to the qualitative behavior of their dynamical systems.    Pollard and the first author showed how to construct an `open' dynamical system from an `open' Petri net with rates---meaning one where chemicals can flow in and out at certain places \cite{BP}.  We used the formalism of decorated cospans to do this, and to define a functor from a category with open Petri nets with rates as morphisms to one with open dynamical systems as morphisms.  Later, the first two authors used structured cospans to promote the first of these categories to a double category \cite[Sec.\ 6.16]{BC}. Here we show the second, the category with open dynamical systems as morphism, \emph{cannot} be made into a double category using structured cospans---but we can do it using decorated cospans. 
...
\item objects are finite sets,
\item vertical 1-morphisms are functions,
\item horizontal 1-cells are \define{open dynamical systems}:
\[
\begin{tikzpicture}[scale=1.5]
\node (D) at (-3,0) {$X$};
\node (E) at (-2,0) {$S$};
...
(D) edge node [above] {$i$} (E)
(F) edge node [above] {$o$} (E);
\end{tikzpicture}, \qquad v \in D(S) 
\]
\item 2-morphisms are....
\[
\begin{tikzpicture}[scale=1.5]
\node (E) at (3,0) {$L(X_1)$};
\node (F) at (5,0) {$L(Y_1)$};
