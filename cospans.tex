...
\author{John\ C.\ Baez$^{*,\bullet}$, Kenny Courser$^*$, and Christina Vasilakopoulou$^\square$}
...
From this symmetric lax monoidal functor $F \maps \textsf{A} \to \mathsf{Set}$, Fong then constructs a symmetric monoidal category $F\textnormal{Cospan}(\mathsf{A})$ which has:
\begin{enumerate}
...
The definition of a lax monoidal pseudofunctor is recalled in \cref{sec:2cats}, and the basics of double categories are recalled in \cref{sec:doublecats}.

\begin{thm}\label{thm:decoratedcospans}
Let $\mathsf{A}$ be a category with finite colimits and $F \maps \mathsf{A} \to \mathbf{Cat}$ a lax monoidal pseudofunctor. Then there exists a (pseudo) double category $F\mathbb{C}$sp which has:
\begin{enumerate}
\item{objects as those of $\mathsf{A}$,}
...
\end{tikzpicture}
\]
where $!_a \in F(a)$ is the trivial decoration on $a$ given by the composite of the unique map $F(!) \maps F(0) \to F(a)$ and the morphism $\phi \maps 1 \to F(0)$  which comes from the structure of the lax monoidal pseudofunctor $F \maps \mathsf{A} \to \mathbf{Cat}$. For morphisms, the structure functor $U$ is defined as:
\[
\begin{tikzpicture}[scale=1.5]
\node (G) at (-1,0.5) {$a$};
...
where $\psi \maps m + n \to m+_{b} n$ is the natural map from the coproduct to the pushout and $\phi_{m,n} \maps F(m) \times F(n) \to F(m+n)$ is the natural transformation coming from the structure of the lax monoidal pseudofunctor $F \maps \mathsf{A} \to \mathbf{Cat}$. Denoting the first and second of these horizontal 1-cells as $M$ and $N$, respectively, the source and target structure functors satisfy the equations $S(N \odot M)=S(M)$ and $T(N \odot M)=T(N)$.
...
%\end{tikzpicture}
%\]
As is usual concerning the interchange law of double categories of this nature, only the `interior' of the two composites appears different, but the two morphisms $(h_1^\prime +_{g^\prime} h_2^\prime)(h_1 +_g h_2) \maps m+_{b} n \to m'' +_{b''} n''$ and $(h_1^\prime h_1) +_{g^\prime g} (h_2^\prime h_2) \maps m+_{b} n \to m'' +_{b''}n''$ are the same universal map realized in two different ways. The two morphisms of decorations $\iota_{(\alpha^\prime \odot \beta^\prime)(\alpha \odot \beta)}$ and $\iota_{(\alpha^\prime \alpha) \odot (\beta^\prime \beta)}$ are obtained as two different compositions of four 2-morphisms in $\mathbf{Cat}$, namely horizontally then vertically and vertically then horizontally. As $\mathbf{Cat}$ is a 2-category, the interchange law for these 2-morphisms already holds, and as a result, the morphisms $$\iota_{(\alpha' \odot \beta')(\alpha \odot \beta)} \maps F((h_1^\prime +_{g^\prime} h_2^\prime)(h_1 +_g h_2))(x \odot y) \to x'' \odot y''$$ and $$\iota_{(\alpha^\prime \alpha)\odot(\beta^\prime \beta)} \maps F((h_1^\prime h_1)+_{g^\prime g} (h_2^\prime h_2))(x \odot y) \to x'' \odot y''$$ are also the same. Thus the interchange law for 2-morphisms holds and $F\mathbb{C}$sp is a double category.
\end{proof}

\begin{thm}\label{DC}
Let $\mathsf{A}$ be a category with finite colimits and $F \maps \mathsf{A} \to \mathbf{Cat}$ a symmetric lax monoidal pseudofunctor. Then the double category $F\mathbb{C}$sp is symmetric monoidal.
\end{thm}
...
\end{tikzpicture}
\]
where $0$ is the monoidal unit of $\mathsf{A}$ and $!_0 \maps 1 \to F(0)$ is the morphism which is part of the structure of the symmetric lax monoidal pseudofunctor $F \maps \mathsf{A} \to \mathbf{Cat}$. Tensoring an object with the monoidal unit, say, on the left:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$0$};
\node (B) at (1,0) {$0$};
...
\end{tikzpicture}
\]
since $$F(aa)(((x_1+x_2)+x_3)+x_4)=F((1 \otimes a)a(a \otimes 1))(((x_1+x_2)+x_3)+x_4)$$ as the corresponding pentagon in the symmetric monoidal category $(\mathsf{A},+,0)$ commutes, and then appliying the pseudofunctor $F$ to said pentagon yields a commutative pentagon in $\mathbf{Cat}$.
...
\end{tikzpicture}
\]
since $$F(\rho \otimes 1)((x_1+0)+x_2)=F((1 \otimes \lambda)a)((x_1+0)+x_2)$$ as the corresponding triangle in the symmetric monoidal category $(\mathsf{A},+,0)$ commutes and applying the pseudofunctor $F$ to this commutative triangle results in a commutative triangle in $\mathbf{Cat}$.
...
as the above underlying diagram of maps of cospans commutes and then applying the pseudofunctor $F$ yields a commutative diagram in $\mathbf{Cat}$.
...
as the above underlying diagram of maps of cospans commutes and then applying the pseudofunctor $F$ to this diagram yields a commutative diagram in $\mathbf{Cat}$. The other diagrams are shown to commute similarly.
\end{proof}
...
\begin{cor}
Let $(\mathsf{C},+,0)$ be a category with finite colimts and $F \maps \mathsf{C} \to \mathbf{Cat}$ a symmetric lax monoidal pseudofunctor. Then there exists a symmetric monoidal bicategory $F \mathbf{Csp} \mapseqq H(F\mathbb{C}\textnormal{sp})$ which has:
\begin{enumerate}
\item{objects as those of $\mathsf{A}$,}
...
\begin{cor}
Given a symmetric lax monoidal pseudofunctor $F \maps \mathsf{A} \to \mathbf{Cat}$ where $\mathsf{A}$ is a category with finite colimits and whose monoidal structure is given by binary coproducts, there exists a symmetric monoidal category $F\textnormal{Csp} \mapseqq D(F\mathbf{Csp})$ which has:
\begin{enumerate}
\item{objects as those of $\mathsf{A}$ and}
...
%Let $F \maps \mathsf{A} \to \mathbf{Cat}$ be a symmetric lax monoidal pseudofunctor. Then by Theorem \ref{DC} of the previous section, we get a symmetric monoidal double category $F\mathbb{C}\textnormal{sp}$. This symmetric monoidal double category has:
%\begin{enumerate}
%\item{objects as those of $\mathsf{A}$,}
%\item{vertical 1-morphisms as morphisms of $\mathsf{A}$,}
...
Regarding maps between symmetric monoidal double categories, consider another symmetric lax monoidal pseudofunctor $F^\prime \maps \mathsf{A^\prime} \to \mathbf{Cat}$%, we can obtain another symmetric monoidal double category $F^\prime \mathbb{C}\textnormal{sp}$.
...
\end{tikzpicture}
\]
where $E \maps \mathbf{Cat} \to \mathbf{Cat}$ is a symmetric lax monoidal pseudofuctor such that the following diagram commutes up to isomorphism $\theta \maps EF \Rightarrow F'H$:
\[
\begin{tikzpicture}[scale=1.5]
...
\begin{comment}
Recall that we can think of the object $d \in F(c)$ as a morphism $d \maps 1 \to F(c)$ and the functor $\iota \maps F(h)(d) \to d^\prime$ of $F(c^\prime)$ as a natural transformation  in $\mathbf{Cat}$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,-0.5) {$1$};
\node (B) at (1,0) {$F(c)$};
...
\end{tikzpicture}
\]
Applying the symmetric lax monoidal pseudofunctor $E \maps \mathbf{Cat} \to \mathbf{Cat}$ to this diagram yields:
\[
\begin{tikzpicture}[scale=1.5]
...
\begin{thm}
Given two finitely cocomplete categories $\mathsf{A}$ and $\mathsf{A}'$, two symmetric lax monoidal pseudofunctors $F \maps \mathsf{A} \to \mathbf{Cat}$ and $F' \maps \mathsf{A}' \to \mathbf{Cat}$, a finite colimit preserving functor $H \maps \mathsf{A} \to \mathsf{A}'$, a symmetric lax monoidal pseudofunctor $E \maps \mathbf{Cat} \to \mathbf{Cat}$ and a 2-isomorphism $\theta \maps EF \Rightarrow F^\prime H$ as in the following diagram, the triple $(H,E,\theta)$ induces a symmetric monoidal (strong) double functor $\mathbb{H} \maps F\mathbb{C}\textnormal{sp} \to F'\mathbb{C}\textnormal{sp}$ as defined above.
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\mathsf{A}$};
\node (B) at (1,0) {$\mathbf{Cat}$};
...
\end{tikzpicture}
\]
where $!_{1_{\mathsf{A}}} = \phi \maps 1 \to F(1_\mathsf{A})$ is the trivial decoration which comes from the structure of the symmetric lax monoidal pseudofunctor $F \maps \mathsf{A} \to \mathbf{Cat}$. The horizontal 1-cell $U_{1_{_L \mathbb{C}\textnormal{sp}(\mathsf{X})}}$ is given by:
\[
\begin{tikzpicture}[scale=1.5]
...
%\end{defn}
%\begin{defn}
%Given a pseudofunctor $F \maps \mathrm{A} \to \mathbf{Cat}$, the \define{Grothendieck construction of} $F$ is given by the `strict 2-pullback' of the following cospan:
% \[
%\begin{tikzpicture}[scale=1.5]
%\node (D) at (1,1) {$\inta F$};
...
%\end{tikzpicture}
%\]
%where $P \maps \mathbf{Cat}_{\textnormal{lax},\star} \to \mathbf{Cat}$ is the forgetful functor. 
...
%: $$\inta F \maps [ \mathrm{A}, \mathbf{Cat} ] \to \textnormal{Opfib}(\mathrm{A}) \hookrightarrow \mathbf{Cat} / \mathrm{A}.$$
