...
Suppose $\A$ has finite colimits and $F \maps (\A,+) \to (\Cat,\times)$ is a symmetric lax monoidal pseudofunctor. If its corresponding ordinary pseudofunctor (via the monoidal Grothendieck construction) {\chris SHOULD THIS PARENTHESIS STAY OR NOT?} $F\colon \A\to\SMC$ factors through $\Rex$, then the symmetric monoidal double categories $F\lCsp$ of decorated cospans and $_L\lCsp(\inta F)$ of structured cospans are isomorphic, where $L \maps \A \to \inta F$ is a left adjoint of the induced Grothendieck opfibration $U\maps \inta F \to \A$.
\end{thm}
...
\begin{enumerate}
 \item all fibers have $\J$-colimits, and the reindexing functors strictly preserve them;
...
\begin{cor}\label{cor:fcocMonGroth}
 Suppose $\A$ has finite colimits and $F \maps (\A,+) \to (\Cat,\times)$ is a symmetric lax monoidal pseudofunctor. If $F  \maps \A \to \Cat$ as a pseudofunctor factors through $\Rex$, then $\inta F$ has all finite colimits and the induced opfibration $U\maps \inta F \to \A$ preserves them.
...
Notice that by \cref{lem:MonGroth}, since $\A$ is cocartesian monoidal, the initial lax monoidal pseudofunc\-tor structure $(F,\phi,\phi_0) \maps (\A,+) \to (\Cat,\times)$ gives rise to a specific symmetric monoidal structure on the fibers $F(a)$ in terms of $\phi,\phi_0$, given explicitly by \cref{eq:explicitstructure1}. If this fiberwise monoidal structure happens to itself be cocartesian, namely \cref{eq:explicitstructure1} gives coproducts and an initial object in each $F(a)$, 
