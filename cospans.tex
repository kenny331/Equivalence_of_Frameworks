...
%%% Combining \cref{lem:MonGroth,lem:fibrewiselimits}, we obtain the following corollary that sets up the statement of \cref{thm:equiv}.

\begin{cor}\label{cor:fcocMonGroth}
 Suppose $\A$ has finite colimits and $F \maps (\A,+) \to (\Cat,\times)$ is a symmetric lax monoidal pseudofunctor. If the corresponding pseudofunctor $F  \maps \A \to \SMC$ from \cref{lem:MonGroth} factors through $\Rex$, then $\inta F$ has all finite colimits 
 %%% which can be chosen in a way that the induced opfibration strictly preserves them.
 and the induced opfibration $U\maps \inta F \to \A$ preserves them.
\end{cor}
...
$U$ strictly preserves these chosen colimits (which were already chosen in $A$).  
%%% By \cref{cor:fcocMonGroth}, since the corresponding pseudofunctor $F\maps\A\to\SMC$ factors through $\Rex$, the Gro\-the\-ndieck construction gives rise to a category $\inta F$ where we can choose finite colimits in a way that the corresponding opfibration $U\maps\inta{F}\to\A$ strictly preserves them.
