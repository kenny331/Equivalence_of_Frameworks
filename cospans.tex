...
In this section we build the symmetric monoidal double category $F \lCsp$ mentioned in the introduction. Then we obtain an underlying symmetric monoidal bicategory $F \mathbf{Csp}$ using a result of Shulman \cite{Shul} 
%!!!Christina: John suggested we might move all the horizontal bicategory and decategorified 1-category results together somewhere later/in the end.
and finally decategorify this to obtain a symmetric monoidal category $F\mathsf{Csp}$ which is a generalization of Fong's original decorated cospan category \cite{Fong}.  The definition of a lax monoidal pseudofunctor is recalled in \cref{sec:2cats}, and the basics of double categories are recalled in \cref{sec:doublecats}.
...
\item an object is an object of $\A$,
\item a vertical morphism is a morphism of $\A$,
\item a horizontal 1-cell $a\tickar b$ is a cospan in $\X$ of the form
...
\end{tikzpicture}
\end{equation}
\item a 2-morphism is a map of cospans of $\X$ of the form
