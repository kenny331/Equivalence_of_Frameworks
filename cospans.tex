...
In \cite{MV}, the classical Grothendieck construction is generalized to the monoidal setting.  Suppose $\A$ is a monoidal category.     Then there is a bijection between:
\begin{itemize}
\item lax monoidal structures on a pseudofunctor $F\maps\A\to\Cat$, and
...
\begin{cor}\label{cor:fcocMonGroth}
 Suppose $\A$ is a finitely cocomplete category and $F\maps(\A,+)\to(\Cat,\times)$ is a lax monoidal pseudofunctor. If the corresponding pseudofunctor $\A\to\bicat{MonCat}$ given in \cref{monGroth} factors through $\Rex$, then $\inta F$ has all finite colimits and the induced opfibration $U_F\maps\inta F\to\A$ preserves them. 
\end{cor}
