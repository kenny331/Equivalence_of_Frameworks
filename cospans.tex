...
By \cref{cor:fcocMonGroth}, since the corresponding pseudofunctor $F\maps\A\to\SMC$ factors through $\Rex$, the Gro\-the\-ndieck construction gives rise to a category $\inta F$ with finite colimits such that the corresponding opfibration $U\maps\inta{F}\to\A$ preserves them.  Using \cref{lem:fibrewiselimits} we choose finite colimits in $\inta{F}$ such that
$U$ strictly preserves these chosen colimits (which were already chosen in $A$).  By \cref{prop:opfibtolari}, $U$ has a left adjoint $L\maps\A\to\inta F$ with $UL=1_\A$. Diagrammatically,
...
Moreover, the monoidal unit $I_1$ for ${}_L \int \lCsp(\int F)$ is mapped under $\lE_1$ to the decorated cospan given in \cref{eq:monoidalunitFCsp}, so $\lE_1$ strictly preserves the monoidal unit.   %%% I think I have it right now. - jb 
