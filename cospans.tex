...
As a left adjoint, this induced $L$ preserves all colimits that exist between the finitely cocomplete categories $\A$ and $\inta F$, so we can construct the double category of structured cospans $\scsp{L}{\inta F}$ of \cref{SC}. Its objects and vertical morphisms are those of the category $\A$ (as for $\dcsp{F}$ earlier), whereas horizontal 1-cells are now cospans of the form $\cspn{L(a)}{v}{L(b)}{}{}$ in the Grothendieck category $\inta F$ (\cref{def:GrothCat}). Explicitly, they consist of two pairs of morphisms 
...
Building up to an equivalence of the above described double categories, we now define a double functor
\begin{displaymath}
\lE=(\lE_0,\lE_1)\maps\scsp{L}{\inta F} \longrightarrow\dcsp{F}
\end{displaymath}
which on vertical categories acts as the identity $\lE_0 = \id_{\A}$. Given a horizontal 1-cell in $\scsp{L}{\inta F}$ as in \cref{eq:scsphor1cell}, $\lE_1$ maps it to the decorated cospan $i\maps a\to m\leftarrow b:o$ with decoration $x\in F(m)$,
%Notice that this really is a bijective correspondence since the unique maps from the initial objects in the fibres provide no extra information.
...
% Once again, this is a bijective correspondence since, as discussed above, the commutativity \cref{eq:Grothcommutativity} holds automatically by initiality of the domain.
...
for any pair of composable horizontal 1-cells $M=\cspn{(a,\bot_a)}{(m,x)}{(b,\bot_b)}{}{}$ and $N=\cspn{(b,\bot_b)}{(n,y)}{(c,\bot_c)}{}{}$ and any object $m$ of $\scsp{L}{\inta F}$. Explicitly, $\mathbb{E}(M)\odot \mathbb{E}(N)$ is given as in \cref{eq:dcospanscomposition} via the pushout cospan and decoration
...
\end{equation}
whereas first composing $M$ and $N$ in $\scsp{L}{\inta F}$ using pushouts in $\inta F$ (constructed by \cref{lem:fibrewiselimits}) %!!! Christina: would you like me to write how global pushouts are constructed from fiberwise ones and incorporate it earlier say at \cref{rmk:important} where we do so for initial objects?
produces 
\begin{displaymath}
...
Under $\lE$ this is mapped to the underlying $\A$-cospan $\cspn{a}{m+_bn}{b}{}{}$ with decoration $F(\sigma_m)x+F(\sigma_n)y$ -- since a pushout over an initial object is really a coproduct -- which is isomorphic to \cref{eq:1} due to the following diagram 
\begin{displaymath}
 \begin{tikzcd}
...
Regarding identities, $U_m$ is $\cspn{(m,\bot_m)}{(m,\bot_m)}{(m,\bot_m)}{}{}$ with $1_m$ as the $A$-components and isomorphisms between initial objects in the fibres. So $\lE(U_m)$ is the identity cospan in $A$ together with the `initial decoration' $\bot_m\in F(m)$, and so is $U_{\mathbb{E}(m)}$ as in \cref{eq:UFCsp}.
...
We denote by $[\bA,\bB]_\pse$ the 2-category 
