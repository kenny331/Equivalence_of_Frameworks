...
In \cite{BC2}, the first two authors introduced the symmetric monoidal double category of \emph{structured cospans} as a formalism to describe open networks. One of the main goals of this paper is to provide a monoidal double isomorphism between this double category and that of \emph{decorated cospans}, described in \cref{DecCospansDoubleCat}.  We first recall the double category of structured cospans.
...
The following theorem establishes the isomorphism between structured and decorated cospan double categories under certain conditions.  We denote by $\Rex$ the 2-category of finitely cocomplete categories, finitely cocontinuous functors and natural transformations.
...
\end{gather}
The second equivalence was also earlier observed by Shulman \cite{Shulman2008}. In more detail, if $(\phi,\phi_0)$ is the lax monoidal structure of the pseudofunctor $F$ as recalled in \cref{sec:2cats}, each fibre category $\X_a=F(a)$ obtains a monoidal structure via
...
$U \maps (\X,+)\to(\A,+)$ that strictly preserve chosen coproducts and initial object
...
Finally, we can further restrict to the setting of opfibrations that also preserve pushouts, and thus all finite colimits, thanks to the following result.
...
\begin{proof}  See \cite[Corollary~4.9]{Hermida1999}.   \end{proof}
...
categories it says that $\X$ is finitely cocomplete and $U$ preserves all finite colimits.  Combining this with \cref{lem:MonGroth}, we deduce the following.
...
\end{displaymath}
describes the construction of the adjunction from the original $F$. Explicitly, the left adjoint picks the initial object in the finitely cocomplete fibre $L(a)=(a,\bot_a)$, also expressed as $F(u_a)\circ\\phi_0(*)$ according to \cref{eq:initialinfibre} for $(\phi,\phi_0)$ the monoidal structure of $F$.
...
\end{equation}
whereas first composing $M$ and $N$ in $_L\lCsp(\inta F)$ using pushouts in $\inta F$ (constructed by \cref{lem:fibrewiselimits}) %!!! Christina: would you like me to write how global pushouts are constructed from fiberwise ones and incorporate it earlier say at \cref{rmk:important} where we do so for initial objects?
produces 
\begin{displaymath}
...
\end{tikzpicture}
\]
We then have a 2-isomorphism $\phi_{M,N} \maps E(M) \otimes E(N) \xrightarrow{\sim} E(M \otimes N)$ in $F\lCsp$ given by:
\[
\begin{tikzpicture}[scale=1.5]
...
For us a \define{graph} will be a pair of functions $s,t\maps E \to N$ where $E$ and $N$ are finite sets.   We call elements of $E$ \define{edges} and elements of $N$ \define{nodes}.  There is a category $\Graph$ with graphs as objects, where a morphism from the graph $s,t\maps E \to N$ to the graph $s',t' \maps E' \to N'$ is a pair of functions $f \maps E \to E', g \maps N \to N'$ such that these diagrams commute:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$E$};
\node (A') at (0,-1) {$E'$};
...
We can obtain a symmetric lax monoidal functor $F \maps (\Set, +) \to (\Cat,\times)$ by equipping $F$ with suitable functors $\phi_{N,N'} \maps F(N) \times F(N') \to F(N+N')$ for every pair of finite sets $N$ and $N'$, and also a functor $\phi_0 \maps F(\emptyset) \to 1 \in \Cat$.  The functor $\phi_0$ is uniquely determined since $1$ is the terminal category.   More interesting is $\phi_{N,N'}$.   This functor maps a pair of graph structures $s, t \maps E \to N$ and $s',t' \maps E' \to N'$ to the graph structure $s+s', t+t' \maps E+E' \to N+N'$.  In other words, it sends a pair of graph structures to their `disjoint union'.
...
Starting from the symmetric lax monoidal functor $F \maps (\Set, +) \to (\Cat,\times)$, \cref{DC} gives us a symmetric monoidal double category $F\lCsp$ in which:
\begin{itemize}
...
\begin{defn}
Given a set $\La$ of \define{labels}, an $\La$-\define{graph} is a graph $s,t\maps E\to N$
 equipped with a function $\ell \maps E \to \La$.
 A \define{morphism} from the $\La$-graph 
...
but following previous work \cite{BCR} we call an open $\La$-graph an $\La$-\define{circuit}.
...
$\La$-\define{circuit} is a cospan in $\Graph_\La$ of this form:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(S)$};
\node (B) at (1,0) {$G$};
...
\end{tikzpicture}
\]
and an \define{isomorphism} of $\La$-circuits is a commutative diagram in $\Graph_\La$ 
of this form:
\[
\begin{tikzpicture}[scale=1.5]
\node (E) at (3,-0.5) {$L(S)$};
...
% Replaced strict symmetric with symmetric strict three times below
Recall that a \define{prop} is a symmetric strict monoidal category whose objects are natural
numbers, with tensor product of objects given by addition.  An \define{algebra} of a prop $\T$ in a symmetric strict monoidal category $\C$ is a symmetric strict monoidal functor $A \maps \T \to \C$.   A \define{morphism} from the algebra $A \maps \T \to \C$ to the algebra
...
\end{center}
such that $S(\alpha), S(\lambda), S(\rho), T(\alpha), T(\lambda)$ and $T(\rho)$ are all identities and that the coherence axioms of a monoidal category are satisfied. Following the notation of Shulman, objects of $\lD_0$ are called \define{0-cells} or \define{objects} and morphisms of $\lD_0$ are called \define{vertical 1-morphisms}. Objects of $\lD_1$ are called \define{horizontal 1-cells} and morphisms of $\lD_1$ are called \define{2-morphisms}. The morphisms of $\lD_0$, which are vertical 1-morphisms, will be denoted $f \maps A \to C$ and we denote a 1-cell $M$ with $S(M)=A,T(M)=B$ by $M \maps A \to B$. Then a 2-morphism $a \maps M \to N$ of $\lD_1$ with $S(a)=f,T(a)=g$ would look like:
\[
\begin{tikzpicture}[scale=1]
\node (D) at (-4,0.5) {$A$};
\node (E) at (-2,0.5) {$B$};
