...
\begin{tikzcd}
 ...
 & F(\sigma_a)(Ff(y))\cong F(\sigma_n)(Fg(y))\ar[ul,"F(\sigma_a)k"]\ar[ur,"F(\sigma_c)\ell"'] &
 \end{tikzcd}
\end{displaymath}
In similar ways we can construct the initial object and coproducts in a Grothendieck category. 
...
\lE(M\ot M')&\;\stackrel{\cref{eq:tensorstuctured}}{=}\;\lE\Big((a+a',\bot_{a+a'})\to\underbrace{(m+m',\phi_{m,m'}(x,x'))}_{\cref{eq:explicitstructure2}}\leftarrow(b+b',\bot_{b+b'})\Big)%=a+a'\xrightarrow{i+i'}m+m'\xleftarrow{o+o'}b+b' \textrm{ with }\phi_{m,m'}(x,x')
\end{align*}
which are essentially the same. {\chris Well they look equal to me...thoughts?} Moreover, the monoidal unit for the category of arrows of $F\lCsp$ is $0_\A\xrightarrow{\raisebox{-3pt}[0pt][0pt]{$=$}}0_\A\xleftarrow{\raisebox{-3pt}[0pt][0pt]{$=$}}0_\A$ with the trivial decoration
$$\mathbf{1}\xrightarrow{\phi_0}F(0_A)\xrightarrow{F(\mathrm{id})}F(0_A)$$
which is essentially the same as $\lE((0_A,\phi_0)\xrightarrow{\raisebox{-3pt}[0pt][0pt]{$=$}}(0_A,\phi_0)\xleftarrow{\raisebox{-3pt}[0pt][0pt]{$=$}}(0_A,\phi_0))$ again by \cref{eq:explicitstructure2}.
...
We can make $F$ into a symmetric lax monoidal functor $F \maps (\Fin\Set, +) \to (\Cat,\times)$ by equipping it with suitable functors 
...
We can also use structured or decorated cospans to describe electrical circuits \cite{BC,BCR,BF}.  The key idea is to use graphs with labeled edges, where the labels can stand for resistors with any chosen resistance, capacitors with any chosen capacitance, or other circuit elements.   The whole theory of open graphs discussed in the previous section can be recapitulated for labeled graphs.  Since everything works the same way, we can be brief.

Fix a set $\La$ to serve as edge labels.  Define an $\La$-\define{graph} to be a graph $s,t\maps E\to N$ equipped with a function $\ell \maps E \to \La$.  There is a category $\Graph_\La$ with $\La$-graphs as objects, where a morphism from the $\La$-graph 
...
\]
The edges here can represent wires, with the positive real numbers labeling them serving to describe the resistance of a resistor on each wire.  The points in the boxes represent `terminals': that is, points where we allow ourselves to attach a wire from another circuit.
...
In a previous work, Master and the first author used structured cospans to obtain a symmetric monoidal double category of `open Petri nets' \cite{BM}.  A Petri net is given by a diagram in $\Set$ of the form:
\[
\begin{tikzpicture}[scale=1.5]
\node (B) at (1,0) {$T$};
...
\end{tikzpicture}
\]
Here $S$ is the set of \define{places}, $T$ is the set of \define{transitions}, and $\mathbb{N}[S]$ is the underlying set of the free commutative monoid on $S$. Each transition then has a formal linear combination of species given by an element of $\mathbb{N}[S]$ as its source and target as prescribed by the functions $s$ and $t$, respectively. An example of a Petri net is given by:
\[
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
Each set of species $S$ gives rise to a discrete Petri net $L(S)$ with $S$ as its set of species and no transitions. Master and the first author show the existence of a left adjoint $L \maps \Set \to \Petri$ where $\Petri$ is the category whose objects are Petri nets and whose `morphisms are morphisms of Petri nets'. They also show that $\Petri$ has finite colimits and thus using Theorem \cref{thm:SC} obtain a symmetric monoidal double category $\lOpen(\Petri)$ of open Petri nets which has:
\begin{enumerate}
