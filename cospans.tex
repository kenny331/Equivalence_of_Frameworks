...
From this symmetric lax monoidal functor $F \maps \A \to \mathsf{Set}$, Fong then constructs a symmetric monoidal category $F\textnormal{Cospan}(\A)$ which has:
\begin{enumerate}
...
In a previous work, the second author extended Fong's theory of decorated cospans to a symmetric monoidal bicategory of decorated cospans \cite{Cour}. Namely, under the same hypotheses used by Fong amounting to a category $\A$ with finite colimits and a symmetric lax monoidal functor $F \maps (\A,+,0) \to (\mathsf{Set}, \times, 1)$, there exists a symmetric monoidal bicategory $F\mathbf{Cospan}(\A)$ which has:
\begin{enumerate}
...
Let $\A$ be a category with finite colimits and $F \maps \A \to \Cat$ a lax monoidal pseudofunctor. Then there exists a double category $F\mathbb{C}$sp which has:
\begin{itemize}
\item{objects as those of $\A$,}
\item{vertical 1-morphisms as morphisms of $\A$,}
\item{horizontal 1-cells as $F$-decorated cospans in $\A$ which are pairs
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,0) {$m$};
...
\end{tikzpicture}
\]}
\item{2-morphisms as maps of $F$-decorated cospans in $\A$
...
\end{tikzpicture}
\]
where $!_a \in F(a)$ is the trivial decoration on $a$ given by the composite of the unique map $F(!) \maps F(0) \to F(a)$ and the morphism $\phi \maps 1 \to F(0)$  which comes from the structure of the lax monoidal pseudofunctor $F \maps \A \to \Cat$. For morphisms, the structure functor $U$ is defined as:
\[
\begin{tikzpicture}[scale=1.5]
\node (G) at (-1,0.5) {$a$};
...
is the source of the underlying map of cospans in $\A$, namely the morphism $f$ in $\A$; the target structure functor is defined similarly. These structure functors satisfy the equations $$SU(a)=\id_a=TU(a)$$for all objects $a$ of $\A$.
...
where $\psi \maps m + n \to m+_{b} n$ is the natural map from the coproduct to the pushout and $\phi_{m,n} \maps F(m) \times F(n) \to F(m+n)$ is the natural transformation coming from the structure of the lax monoidal pseudofunctor $F \maps \A \to \Cat$. Denoting the first and second of these horizontal 1-cells as $M$ and $N$, respectively, the source and target structure functors satisfy the equations $S(N \odot M)=S(M)$ and $T(N \odot M)=T(N)$.
...
\begin{thm}\label{DC}
Let $\A$ be a category with finite colimits and $F \maps \A \to \Cat$ a symmetric lax monoidal pseudofunctor. Then the double category $F\mathbb{C}$sp is symmetric monoidal.
\end{thm}
%!!!Kenny: We should try to make this theorem shorter.
...
\begin{proof}
First we note that the category of objects $F\mathbb{C}\textnormal{sp}_0=\A$ is symmetric monoidal under binary coproducts and the left and right unitors, associators and braidings are given as natural maps. The category of arrows $F\mathbb{C}\textnormal{sp}_1$ has:
\begin{enumerate}
...
\]
and}
\item{morphisms as maps of cospans in $\A$
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0.5) {$a$};
\node (A') at (0,-0.5) {$a^\prime$};
...
\end{tikzpicture}
\]
their tensor product $M_1 \otimes M_2$ is given by taking the coproducts of the cospans of $\A$
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a_1+a_2$};
...
\end{tikzpicture}
\]
where $0$ is the monoidal unit of $\A$ and $!_0 \maps 1 \to F(0)$ is the morphism which is part of the structure of the symmetric lax monoidal pseudofunctor $F \maps \A \to \Cat$. Tensoring an object with the monoidal unit, say, on the left:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$0$};
\node (B) at (1,0) {$0$};
...
\end{tikzpicture}
\]
then as $\mathbb{C}\textnormal{sp}(\A)$ is a symmetric monoidal double category, the following pentagon of underlying cospans and maps of cospans commutes:
\[
\begin{tikzpicture}[scale=1.5]
...
\]
and}
\item{2-morphisms as maps of cospans in $\A$ of the form:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,0.5) {$c$};
...
This symmetric monoidal bicategory $F\mathbf{Csp}$ is a superior version of the symmetric monoidal bicategory $F$Cospan$(\A)$ constructed earlier by the second author \cite{Cour} in that there is greater flexibility in what 2-morphisms are allowed. 
...
\begin{enumerate}
\item{objects as those of $\A$ and}
\item{morphisms as isomorphism classes of $F$-decorated cospans of $\A$, where an $F$-decorated cospan is given by a pair:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,0) {$c$};
...
%Let $F \maps \A \to \Cat$ be a symmetric lax monoidal pseudofunctor. Then by Theorem \cref{DC} of the previous section, we get a symmetric monoidal double category $F\mathbb{C}\textnormal{sp}$. This symmetric monoidal double category has:
%\begin{enumerate}
%\item{objects as those of $\A$,}
%\item{vertical 1-morphisms as morphisms of $\A$,}
...
\end{tikzpicture}
\]
where $\kappa$ is the isomorphism $$\kappa \maps H(c_1 +_b c_2) \xrightarrow{\sim} H(c_1) +_{H(b)} H(c_2)$$ which comes from the finite colimit preserving functor $H \maps \A \to \mathsf{A^\prime}$. The above diagram commutes by a similar argument to the one used in Theorem \cref{Equiv}. Similarly, for every object $c \in \A$, we have a unit comparison constraint $$\mathbb{H}_U \maps U_{\mathbb{H}(c)} \to \mathbb{H}(U_c)$$ given by the globular 2-isomorphism:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0.5) {$H(c)$};
\node (A') at (0,-0.5) {$H(c)$};
...
\end{tikzpicture}
\]
The diagrams involving the morphisms of decorations are similar to those in Theorem \cref{DC}. This shows that $\mathbb{H}=(H,E,\theta)$ is a double functor. Next we show that this double functor is symmetric monoidal. First, that the object component $\mathbb{H}_0=H$ is symmetric monoidal is clear as $H \maps \A \to \A^\prime$ preserves finite colimits. As for the arrow component $\mathbb{H}_1$, given two horizontal 1-cells $M_1$ and $M_2$ in $F\mathbb{C}\textnormal{sp}$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a_1$};
\node (B) at (1,0) {$c_1$};
...
\node (D) at (1,-0.5) {$\theta_{1_\A}E(!_{1_\A}) \phi \in F^\prime(H(1_\A))$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$1$} (B)
(C) edge node[above]{$1$} (B);
...
\node (F) at (5,-0.5) {$\theta_{1_\A} E(!_{1_\A}) \phi \in F^\prime(H(1_\A))$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$1$} (B)
(C) edge node[above]{$1$} (B)
(A) edge node[left]{$\kappa$} (A')
...
\end{tikzpicture}
\]
together with the morphism $\tau_\mu \maps F^\prime(\kappa)(!_{1_\mathsf{A^\prime}}) \to (\theta_{1_\A} E(!_{1_\A})\phi)$ in $F^\prime(H(1_\A))$. The following square then commutes for any horizontal 1-cell $M$ of $F \mathbb{C}\textnormal{sp}$: 
\[
\begin{tikzpicture}[scale=1.5]
...
\end{tikzpicture}
\]
where we have abbreviated the monoidal units of $F\mathbb{C}\textnormal{sp}_1$ and $F'\mathbb{C}\textnormal{sp}_1$ as $1_\A$ and $1_\mathsf{A^\prime}$, respectively. The diagram of corresponding decorations is given by:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0.5) {$F^\prime(\ell)(d_{!_{1_\mathsf{A^\prime}}} \otimes d_{\mathbb{H}(M)})$};
\node (B) at (0,-0.5) {$d_{\mathbb{H}(M)}$};
\node (A') at (4,0.5) {$F^\prime(H(\ell^\prime) \kappa)(d_{!_{\mathbb{H}(1_\A)}}\otimes d_{\mathbb{H}(M)})$};
\node (B') at (4,-0.5) {$F^\prime(H(\ell^\prime))(d_{\mathbb{H}(1_\A \otimes M)})$};
\path[->,font=\scriptsize,>=angle 90]
...
\begin{thm}\label{SC}
Given a category $\X$ with finite colimits and a category $\A$ with finite coproducts and a finite coproduct preserving functor $L \maps \A \to \X$ with $\A$ and $\X$, there exists a symmetric monoidal double category $_L \mathbb{C}\textnormal{sp}(\X)$ which has:
\begin{itemize}
\item{objects given by objects of $\A$,}
\item{vertical 1-morphisms given by morphisms of $\A$,}
...
\end{equation}
\end{itemize}
Composition of horizontal 1-cells and 2-morphisms is given by pushouts in $\X$ and tensoring of objects is under binary coproducts in $\A$.
\end{thm}
\begin{proof}
...
Then the symmetric monoidal double category $F\mathbb{C}\textnormal{sp}$ of decorated cospans given in \cref{thm:decoratedcospans} is equivalent to the symmetric monoidal double category $\scsp{L_F}{\inta F}$ of structured cospans given in \cref{SC}, where $L_F \maps \A \to \inta{F}$ is the left adjoint of the induced Grothendieck opfibration $\U_F\maps \inta{F} \to \A$. %by \cref{thm:Grothendieck}, 
...
\end{tikzpicture}
\]
where $R\maps \X \to \A$ is the right adjoint to the functor $L \maps \A \to \X$ and $\eta \maps 1_{\A} \to RL$ is the unit of the adjunction $L \dashv R$ which is an isomorphism since $L$ is fully faithful. \textbf{Hopefully...actually, it's a `lari'!} Similarly, the image of a 2-morphism $\alpha \maps M \to N$ in $_L \mathbb{C}\textnormal{sp}(\X)$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(c_1)$};
\node (B) at (1,0) {$x$};
...
\end{tikzpicture}
\]
Thus the set $$_f { _L \mathbb{C}\textnormal{sp}(\X)}_g(M,N)$$ consists of triples $$(f,\alpha,g)$$ rendering the above diagram commutative where $f$ and $g$ are morphisms of $\A$ and $\alpha$ is a morphism of $\X$. The image of the above 2-morphism under the double functor $\mathbb{E}$ is given by:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$c_1$};
\node (B) at (1,0) {$R(x)$};
...
\end{tikzpicture}
\]
where $!_{1_{\A}} = \phi \maps 1 \to F(1_\A)$ is the trivial decoration which comes from the structure of the symmetric lax monoidal pseudofunctor $F \maps \A \to \Cat$. The horizontal 1-cell $U_{1_{_L \mathbb{C}\textnormal{sp}(\X)}}$ is given by:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(1_\A)$};
\node (B) at (1,0) {$L(1_\A)$};
...
of $F\mathbb{C}\textnormal{sp}$. This is just the natural isomorphism $\mathbb{E}_{1_\A}$ from earlier.
...
\[
		\begin{tikzpicture}
			...
			\node (d4) at (7.5,-1) {$\mathbb{E}(1_{ _L \mathbb{C}\textnormal{sp}(\X)} \otimes M_1)$};
                                \node (a5) at (-6,-1) {$1_\A+c_1^\prime$};
			\node (b5) at (-0.5,-1) {$R(L(1_\A)+x_1)$};
			\node (c5) at (5,-1) {$1_\A+c_1^\prime$};
			...
			(c) edge node[above]{$\eta_{1_\A}+R(o_1)\eta_{c_1^\prime}$} (b)
...
(b2) edge node[right]{$\tau_1$} (b3)
			...
			(c5) edge node[above]{$(\mu_{L(1_\A),d_1})(\eta_{1_\A}+R(o_1)\eta_{c_1^\prime})$} (b5)
...
\end{tikzpicture}
\]
The above square commutes because $$F((\kappa)(\beta_\A))(x_1+x_2) = F((R(\beta_\X))(\kappa))(x_1+x_2)$$ as the corresponding braiding square for the finite colimit preserving functor $R \maps (\X,1_\X,+) \to (\A,1_\A,+)$ commutes. Thus the double functor $\mathbb{E} \maps _L\mathbb{C}\textnormal{sp}(\X) \to F\mathbb{C}\textnormal{sp}$ is symmetric monoidal.
\end{comment}
...
\begin{comment}
We can also define the part of the double equivalence $\mathbb{G} \maps F\mathbb{C}\textnormal{sp} \to {_L \mathbb{C} \textnormal{sp}(\X)}$ which goes in the other direction: again, the object component of this double functor will be $\mathbb{G}_0 = \id_{\A}$.
