...
\]
\end{itemize}
Many of the flawed applications of decorated cospans have been fixed using structured cospans \cite[Sec.\ 6]{BC2}, but not all decorated cospans can handled using structured cospans.
...
Finally, in \cref{Applications}, we describe applications to electrical circuits, Petri nets, chemistry and dynamical systems.
...
\end{comment}
Finally, the symmetric monoidal structure of $F\lCsp_1$ is inherited from that in $\A$ and $F$, with braiding the cospan isomorphism
...
a_1+a_2\ar[rr,"i_1+i_2"]\ar[d,"\stackrel{\beta}{\cong}"'] && m_1+m_2\ar[d,"\stackrel{\beta}{\cong}"'] && b_1+b_2\ar[ll,"o_1+o_2"']\ar[d,"\stackrel{\beta}{\cong}"] &&1\ar[r,"x_1\times x_2"] & F(m_1)\times F(m_2)\ar[r,"\phi_{m_1,m_2}"] & F(m_1+m_2) \\
a_2+a_1\ar[rr,"i_2+i_1"'] && m_2+m_1 && b_2+b_1\ar[ll,"o_2+o_1"] &&\one \ar[r,"x_2\times x_1"] & F(m_2)\times F(m_1)\ar[r,"\phi_{m_2,m_1}"] & F(m_2+m_1)
 \end{tikzcd}
\end{displaymath}
together with the decoration isomorphism formed by \cref{eq:braidedpseudofun} of the symmetric monoidal pseudofunctor.
...
\one \ar[ddrr,dashed,"{(x_1\oplus y_1)\odot(x_2\oplus y_2)}"']\ar[r,"x_1{\times}y_1{\times}x_2{\times}y_2"] \& F(m_1){\times}F(n_1){\times}F(m_2){\times}F(n_2)\ar[r,"\phi_{m_1,n_1}\times\phi_{m_2,n_2}"] \& F(m_1{+}n_1){\times}F(m_2{+}n_2) \ar[d,"\phi_{m_1+n_1,m_2+n_2}"] \\
...
\one \ar[ddrr,dashed,"{(x_1\odot x_2)\oplus(y_1\odot y_2)}"']\ar[r,"x_1{\times}x_2{\times}y_1{\times}y_2"] \& F(m_1){\times}F(m_2){\times}F(n_1){\times}F(n_2)\ar[r,"\phi_{m_1,m_2}\times\phi_{n_1,n_2}"] \& F(m_1{+}m_2){\times}F(n_1{+}n_2) \ar[d, "F\psi\times F\psi"] \\
\&\& F(m_1+_bm_2)\times F(n_1+_{b'}n_2)\ar[d,"\phi_{m_1+_bm_2,n_1+_{b'}n_2}"] \\
\&\& F((m_1+_bm_2)+F(n_1+_{b'}n_2))
\end{tikzcd}}
\end{displaymath}
where the legs of the cospan are the sums of the dashed ones of the appropriate diagrams \cref{eq:dcospanscomposition}.
Thus the (globular) cospan isomorphism is the canonical universal map between the colimit of the same diagram obtained in two different ways $(m_1+n_1)+_{b+b'}(m_2+n_2)\cong(m_1+_b m_2)+(n_1+_{b'}n_2)$, and the decoration isomorphism can be built in a similar way using coherent isomorphisms essentially due to pseudonaturality of $\phi$. {\chris Should we give more details here?}
...
\item an isomorphism $\theta \maps EF \Rightarrow F'H$: {\chris is this natural or pseudonatural? It should be clarified.}
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\A$};
\node (B) at (1,0) {$\Cat$};
...
\end{tikzpicture}
\]
where given a decoration morphism $\tau \colon F(h)(x) \to x'$, the induced decoration morphism is:
\[
\begin{tikzpicture}[scale=1.5]
\node (G) at (-3.5,-0.5) {$1$};
\node (H) at (-2,0) {$F(m)$};
...
\end{tikzpicture}
\]
{\chris There should be an iso on the right if we go with pseudonatural isomorphism}
\end{itemize}
...
\end{tikzpicture}
\]
where $\kappa$ is the isomorphism $$\kappa \maps H(c_1 +_b c_2) \xrightarrow{\sim} H(c_1) +_{H(b)} H(c_2)$$ which comes from the finite colimit preserving functor $H \maps \A \to \mathsf{A'}$. The above diagram commutes by a similar argument to the one used in \cref{thm:equiv}. Similarly, for every object $c \in \A$, we have a unit comparison constraint \\[ \H_U \maps U_{\H(c)} \to \H(U_c)\]
given by the globular 2-isomorphism:
where $\kappa$ is the isomorphism 
\[  \kappa \maps H(c_1 +_b c_2) \xrightarrow{\sim} H(c_1) +_{H(b)} H(c_2)\] 
which comes from the finite colimit preserving functor $H \maps \A \to \A'$. The above diagram commutes by a similar argument to the one used in \cref{thm:equiv}. Similarly, for every object $c \in \A$, we have a unit comparison constraint 
\[ \lH_U \maps U_{\lH(c)} \to \lH(U_c)\] 
given by the globular 2-isomorphism:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0.5) {$H(c)$};
...
\end{thm}
%%% Christina: note to self, now our functor factors through that specific Rex means that each category F(a) which later becomes the fibre comes with a specific choice of colimits. We do not know what this choice is, but of course such colimits in the fibres will be isomorphic to any other we may stumble upon!
...
Moreover in this situation the $\J$-colimits in $\X$ and $\J$ can be chosen so that $U$ strictly preserves them.   %%% CHECK AND MAYBE FIX THIS!!!
\end{enumerate}
\end{lem}
...
\& (b,y)\ar[ul,"{\begin{cases}f\maps b\to a &\textrm{in }\A \\k\maps F(f)y\to x &\textrm{in }F(a)\end{cases}}"description]\ar[ur,"{\begin{cases}g\maps b\to c &\textrm{in }\A \\\ell\maps F(g)y\to z &\textrm{in }F(c)\end{cases}}"description] \&
 \end{tikzcd}
\end{equation}
which on the first variable is given by the pushout in $\A$ on the left below, and the object $w$ is formed as the pushout in the fibre category $F(a+_bc)$ on the right below:
\begin{displaymath}
\begin{tikzcd}
...
\begin{tikzcd}
 ...
 & F(u_a)(Ff(y))\cong F(u_c)(Fg(y))\ar[ul,"F(u_a)k"]\ar[ur,"F(u_c)\ell"'] &
 \end{tikzcd}
\end{displaymath}
In a similar way we can construct the initial object and coproducts in a Grothendieck category from those in the fibres, namely
...
\end{equation}
where $\iota_a:a\to a+b\leftarrow b:\iota_b$ are the sum inclusions in $\A$ and the sum on the second variable is in the fibre $F(a+b)$.
...
We now have all the necessary background to formally construct an isomorphism between the double category of decorated cospans and the double category of structured cospans, starting from a symmetric lax monoidal pseudofunctor $F \maps (\A, +) \to (\Cat, \times)$ that factors through $\Rex$ as a pseudofunctor.
...
\end{equation}
describes the construction of the adjunction from the original $F$. Explicitly, the left adjoint picks the initial object in the fibre $L(a)=(a,\bot_a)$, also expressed as $F(!_a)\circ\phi_0(*)$ according to \cref{eq:explicitstructure1} for $(\phi,\phi_0)$ the monoidal structure of $F$. %Christina: If each adjoint picks one isomorphic initial object, can we choose the one with that choice? Well they are all isomorphic and that could help with the monoidal structure later...

As a left adjoint, this induced $L$ preserves all colimits that exist between the categories $\A$ and $\inta F$, which have finite colimits, so we can construct the double category of structured cospans $_L\lCsp(\inta F)$ of \cref{thm:SC}. Its objects and vertical morphisms are those of the category $\A$ (same as for $F\lCsp$), whereas horizontal 1-cells are now cospans of the form $\cspn{L(a)}{(m,x)}{L(b)}{}{}$ in the Grothendieck category $\inta F$ (\cref{def:GrothCat}). Explicitly, they consist of two pairs of morphisms
...
\end{gather}
in the fibre category $F(m')$, by composition in the Grothendieck category \cref{eq:compGrothcat}. Since both morphisms emanate from the mapped initial object in $F(m')$ under the finite-colimit-preserving reindexing functors, they are the unique such into $x'$ therefore this gives no extra conditions; similarly for the equality including $o,o'$.
...
We can also describe $\La$-circuits using decorated cospans.   There is a symmetric lax monoidal pseudofunctor $F \maps (\Fin\Set, +) \to (\Cat, \times)$ such that for any finite set $N$, the category $F(N)$ has:
\begin{itemize}
...
The machinery of structured cospans has been used to provide a semantics for open Petri nets \cite{BM}: a symmetric monoidal double functor from ${}_L \lCsp(\Petri)$ to  a symmetric monoidal double category of `open commutative monoidal categories'.  Presumably this double functor can equivalently be obtained using the machinery of decorated cospans, with the help of \cref{thm:functoriality}.  However, it should be clear by now that in cases where either structured or decorated cospans can be used, structured cospans are simpler.   We next turn to an example where decorated cospans are necessary.

\subsection{Petri nets with rates}

Chemists sometimes use Petri nets where the places represent `chemical species'---types of molecules, ions and the like---and the transitions represent chemical reactions.   In these Petri nets, which we shall call `Petri nets with rates', the transitions are labelled with positive real numbers called `rate constants'.  This allows one to systematically construct a dynamical system from any Petri net with rates, and mathematical chemists have proved deep theorems relating the topology of Petri nets with rates to the qualitative behavior of their dynamical systems.    

Pollard and the first author showed how to construct an `open dynamical system' from an `open' Petri net with rates, meaning one where chemicals can flow in and out at certain places \cite{BP}.  They used the formalism of decorated cospans to do this, and to define a functor from a category with open Petri nets with rates as morphisms to one with open dynamical systems as morphisms.  Later, structured cospans were used to promote the first of these categories to a double category \cite[Sec.\ 6.16]{BC}. Here we show that the second, the category with open dynamical systems as morphisms, \emph{cannot} be made into a double category using structured cospans---but we \emph{can} do it using decorated cospans!

A \define{Petri net with rates} is a Petri net $s,t \maps T \to \N[S]$ together with a function $r \maps T \to (0,\infty)$ assigning to each transition $\tau \in T$ a positive real number called its \define{rate constant}.  There is a category $\Petri_r$ whose objects are Petri nets with rates, where a morphism from 
...
\item objects are finite sets,
\item vertical 1-morphisms are functions,
\item horizontal 1-cells are \define{open Petri nets with rates}:
\[
\begin{tikzpicture}[scale=1.5]
\node (D) at (-3,0) {$L(X)$};
\node (E) at (-2,0) {$P$};
...
\end{itemize}
The functorality of $D$ is proved in \cite[Lemma 15]{BP} while the lax symmetric monoidal stucture is given in Lemma 16.

Since every set gives a discrete category with that set of objects, we can reinterpret $D$ as a lax monoidal pseudofunctor $D \maps (\Fin\Set, +) \to (\Cat, \times)$.  Applying \cref{DC} we obtain a symmetric monoidal double category $D \lCsp$ where:
\begin{itemize}
\item objects are finite sets,
\item vertical 1-morphisms are functions,
...
We now show that while open dynamical systems of this sort are morphisms in a decorated cospan category, they do not arise from a structured cospan category via \cref{thm:equiv}.
...
%%Christina: Or future directions or something? Unless we at some point conclude about something for the two frameworks, i.e they are close enough that we can choose which one to work with according to our application or something? I did something along those lines at the very end....

We have given conditions under which a decorated cospan double category is isomorphic to a structured cospan double category, in \cref{thm:equiv}. The converse question is also interesting: is every structured cospan double category equivalent to a decorated cospan double category? The answer is similar to the previous one: affirmative, under certain conditions that allow us to make the passage from an appropriate functor $L\colon \A\to \X$ to an appropriate pseudofunctor $F\colon \A\to\Cat$, namely in the reverse direction of the earlier \cref{eq:FtoL}.

Let us now give a sketch of the relevant story. Suppose $\A$ and $\X$ have finite colimits and $L\maps\A\to\X$ is a finite-colimit preserving functor -- which by \cref{thm:SC} are the required conditions for constructing the symmetric monoidal double category of structured cospans. If $L$ has a right adjoint `left inverse' (meaning the unit is the identity) $U\maps\X\to\A$, which moreover strictly preserves the choice of pushouts in $\A$ and $\X$, it can be shown\footnote{This will appear in the forthcoming paper \cite{CV}.} that $U$ is an opfibration. Consequently, $U$ corresponds to a pseudofunctor $F\colon \A\to\Cat$ by the reverse of the Grothendieck construction, i.e. the second part of \cref{thm:Grothendieck}. Furthermore, if $U$ preserves finite coproducts, by the special case of the cocartesian monoidal Grothendieck construction discussed under \cref{lem:MonGroth}, the corresponding pseudofunctor $F$ has a lax monoidal structure $(\A,+)\to(\Cat,\times)$. As a result, by \cref{DC}, $F$ has now enough structure to induce the symmetric monoidal double category of decorated cospans $F\lCsp$, and further it can then be verified that it is isomorphic to the one of structured cospans.

{\chris Give some sort of excuse why we don't do the full result here?? Because of missing - for us now - steps? Or other things?} 

To give a better sense of how this lax monoidal pseudofunctor $F\maps\A\to\Cat$ is constructed, $F(a)$ is defined to be the pre-image of $a$ under $U$, namely the category of all objects in $x\in\X$ such that $U(x)=a$ and morphisms $k\maps x\to y\in\X$ such that $U(k)=1_a$. Given a morphism $f\maps a\to b$, there is a functor $F(f)\maps F(a)\to F(b)$ that maps some $x$ to the following pushout
\begin{displaymath}
 \begin{tikzcd}
...
a\ar[r,"f"'] & b & \textrm{in }\A
 \end{tikzcd}
\end{displaymath}
where $\varepsilon_x\colon LU(x)=L(a)\to x$ is the counit of $L\dashv U$. The fact that $U$ strictly preserves pushouts is necessary to show that the pushout is mapped, via $U$, directly down to $b$.

{\chris I don't know if the part below should be included or not, perhaps it is too speculative and not dead important. 
%If we keep, I can add a bit more information about what these special opfibs and special laris are, if we think it helps -- or not if it's OK now. 
Bear in mind that me and Daniel are supposed to clarify the middle category isomorphism in this other paper too!}

Even though for both \cref{thm:equiv} and the above described setting, the conditions required are only sufficient, they provide certain indication that under a suitable ``condition intersection'' in a sense, we could establish a functorial picture
\begin{displaymath}
\begin{tikzpicture}
\node [align=center,draw] (A) { Lax monoidal pseudofunctors \\ $(\A,+)\to(\Cat,\times)$};
\node[below right=1.5 and 1.5 of A,align=center,draw] (C)  { Symmetric monoidal \\double categories };
\node[below left=1 and .5  of A,draw] (D)  {Special opfibrations};
\node[below left=2 and 1 of A,draw] (E)  {Special laris};
\node[below=3 of A,align=center,draw] (B)  {Finitely cocomplete \\ functors $A\to\X$};
\node[below=.01 of D] {\rotatebox{90}{$\cong$}};
\path[->,font=\scriptsize]
(A) edge node[above,sloped]{$F\mapsto F\lCsp$}  (C)
(D) edge (A)
(E) edge (B)
(B) edge node[below,sloped]{$L\mapsto {}_{L}\lCsp$} (C);
\end{tikzpicture}
\end{displaymath}
with an actual natural isomorphism in the middle. This would imply that starting from an appropriate middle ground, employing the structured or the decorated cospan formalism is essentially the same. We leave such considerations for future work.
...
\end{equation}
 natural in $f$ and $g$.
A \define{braided lax monoidal} pseudofunctor between braided monoidal bicategories comes with an appropriate invertible modification with components %$F(\beta_{a,b})\circ \phi_{a,b}\cong\phi_{b,a}\circ \beta_{Fa,Fb}$. 
\begin{equation}\label{eq:braidedpseudofun}
 \begin{tikzcd}
Fa\otimes Fb\ar[r,"\phi_{a,b}"]\ar[d,"\beta_{Fa,Fb}"']\ar[dr,phantom,"\scriptstyle\cong"] & F(a\ot b)\ar[d,"F(\beta_{a,b})"] \\
...
\end{displaymath}
The category $\X$ is called the \textbf{total} category and $\A$ is called the \textbf{base} category of the opfibration. For any $a\in\A$, the \textbf{fibre category} $\X_a$ consists of all objects that map to $a$ and \define{vertical} morphisms between them, i.e., morphisms mapping to $1_a$.   

Assuming the axiom of choice, we may select a cocartesian lifting of each morphism $f\maps a\to b$ in $\A$ to each $x\in\X _a$, denoted by $\mathrm{Cocart}(f,x)\maps x\to f_!(x)$, rendering $U$ a so-called \textbf{cloven} opfibration. This choice induces \textbf{reindexing functors} $f_!\maps\X _a\to\X _b$ between the fibre categories, which by the lifting's universal property come equipped with natural isomorphisms $(1_a)_!\cong 1_{\X _a}$ and $(f\circ g)_!\cong f_!\circ g_!$.   With the help of these, any cloven opfibration $U \maps \X \to \A$ gives a pseudofunctor $F \maps \A \to \Cat$, where $\A$ viewed as a 2-category with trivial 2-cells, $F(a) = \X_a$ for each object $a \in \A$, and $F(f) = f_!$ for each morphism $f$ in $\A$.
...
\begin{thm}{\cite[Thm.\ 7.8]{Shulman2008}}
\label{ShulDubEquiv}
...
\begin{itemize}
		...
		\begin{tikzpicture}\label{eq:braidingaxiom}
			\node (A) at (0,1.5) {\footnotesize{$(M_2 \odot M_1) \otimes (N_2 \odot N_1)$}};
			\node (A') at (0,0) {\footnotesize{$ (M_2\otimes N_2)\odot (M_1\otimes N_1)$}};
			\node (B) at (5,1.5) {\footnotesize{$(N_2\odot N_1) \otimes (M_2\odot M_1)$}};
			\node (B') at (5,0) {\footnotesize{$ (N_2 \otimes M_2)\odot (N_1 \otimes M_1)$}};
			%
			\path[->,font=\scriptsize]
...
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$(M_2 \odot M_1) \otimes (N_2 \odot N_1)$};
\node (B) at (3,0) {$(N_2 \odot N_1) \otimes (M_2 \odot M_1)$};
\node (C) at (0,-.75) {$(M_2 \otimes N_2) \odot (M_1 \otimes N_1)$};
\node (D) at (3,-.75) {$(N_2 \otimes M_2) \odot (N_1 \otimes M_1)$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$\beta$} (B)
...
\end{tikzpicture}
\]
{\chris $\chi$'s in the definition of a monoidal double category have a different direction. Since it is an isomorphism it doesn't really change anything, but consider changing to keep them right}
First of all, if we only consider the underlying cospans without their decorations, we obtain the following `flattened' commutative diagram in the symmetric monoidal double category $\lCsp(\A)$, where the top and the bottom cospans coincide:
\[
		\begin{tikzpicture}
			\node (a) at (-4,0) {$a+a'$};
...
%(b) edge node[right]{$\tau_4$} (b5)
 ...
 %(b5) edge node[right]{$\tau_5$} (b6)
		...
		\end{tikzpicture}
	\]
where $\beta$ is the braiding of the symmetric monoidal category $\A$, $\hat{\chi}$ is the globular 2-morphism interchanging the tensor product and horizontal composition of the symmetric monoidal double category $\lCsp(\A)$, and the inwards pointing morphisms are natural maps from the each cospan's feet to its apex.
...
%			\node (a) at (-4,0) {$a+a'$};
			\node (b) at (1,0) {$(m_1+_b m_2) + (n_1+_{b'} n_2)$};
			\node (c) at (8,0) {$(x_1 \odot x_2) \oplus (y_1 \odot y_2) \in F((m_1+_b m_2)+(n_1+_{b'} n_2))$};
%			\node (a2) at (-4,1) {$a'+a$};
			\node (b2) at (1,1) {$(n_1 +_{b'} n_2) + (m_1 +_b m_2)$};
			\node (c2) at (8,1) {$(y_1 \odot y_2) \oplus (x_1 \odot x_2) \in F((n_1+_{b'} n_2)+(m_1+_b m_2))$};
%                            \node (a3) at (-4,2) {$a'+a$};
			\node (b3) at (1,2) {$(n_1+m_1) +_{(b'+b)} (n_2+m_2)$};
			\node (c3) at (8,2) {$(y_1\oplus x_1) \odot (y_2\oplus x_2) \in F((n_1+m_1)+_{b'+b}(n_2+m_2))$};
%                                \node (a5) at (-4,-1) {$a+a'$};
			\node (b5) at (1,-1) {$(m_1+n_1) +_{(b+b')} (m_2+n_2)$};
			\node (c5) at (8,-1) {$(x_1\oplus y_1) \odot (x_2\oplus y_2) \in F((m_1+n_1)+_{b+b'}(m_2+n_2))$};
%                                \node (a6) at (-4,-2) {$a'+a$};
			\node (b6) at (1,-2) {$(n_1+m_1) +_{(b'+b)} (n_2+m_2)$};
			\node (c6) at (8,-2) {$(y_1\oplus x_1) \odot (y_2\oplus x_2) \in F((n_1+m_1)+_{b'+b}(n_2+m_2))$};
			\path[->,font=\scriptsize,>=angle 90]
%			(a) edge node[above]{$$} (b)
%			(c) edge node[above]{$$} (b)
%                                (a2) edge node[above]{$$} (b2)
...
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\tau_1 \colon F(\beta)((x_1 \odot x_2)\oplus(y_1 \odot y_2)) \to (y_1 \odot y_2)\oplus(x_1 \odot x_2)$};
\node (B) at (0,-.5) {$\tau_2 \colon F(\hat{\chi})((y_1 \odot y_2)\oplus(x_1 \odot x_2)) \to (y_1\oplus x_1) \odot (y_2\oplus x_2)$};
\node (C) at (0,-1) {$\tau_3 \colon F(\hat{\chi})((x_1 \odot x_2)\oplus(y_1 \odot y_2)) \to (x_1\oplus y_1) \odot (x_2\oplus y_2)$};
\node (D) at (0,-1.5) {$\tau_4 \colon F(\beta \odot \beta)((x_1\oplus y_1)\odot(x_2\oplus y_2)) \to (y_1\oplus x_1)\odot(y_2\oplus x_2)$};
\path[->,font=\scriptsize,>=angle 90]
...
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$F(\hat{\chi} \beta)((x_1 \odot x_2)\oplus(y_1 \odot y_2))$};
\node (B) at (4,0) {$F(\hat{\chi})((y_1 \odot y_2)\oplus(x_1 \odot x_2))$};
\node (C) at (0,-1) {$F(\beta \odot \beta)((x_1\oplus y_1) \odot (x_2 \oplus  y_2))$};
\node (D) at (4,-1) {$(y_1\oplus x_1) \odot (y_2\oplus x_2)$};
\path[->,font=\scriptsize,>=angle 90]
...
\end{tikzpicture}
\]
where in the top left corner, we note that $$F(\hat{\chi} \beta)((x_1 \odot x_2)\oplus (y_1 \odot y_2)) = F((\beta \odot \beta)\hat{\chi})((x_1 \odot x_2)\oplus (y_1 \odot y_2))$$
...
{\chris Well only up to isomorphism since F is a pseudofunctor. Does this matter?}
{\chris I would here expect big pasted diagrams of isomorphisms coming from the braided lax monoidal pseudofunctor and using its axioms to actually show that two such isomorphisms are equal...}
