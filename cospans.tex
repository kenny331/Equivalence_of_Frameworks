...
\item an object is an object of $\A$,
\item a vertical 1-morphism is a morphism of $\A$,
\item a horizontal 1-cell from $a$ to $b$ is a \define{$L$-structured cospan}, that is,
...
\end{tikzpicture}
\end{displaymath}
\item a 2-morphism is a \define{map of $L$-structured cospans}, that is, a commutative diagram in $\X$ of the form
\begin{displaymath}
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(a)$};
\node (B) at (1,0) {$x$};
...
\end{displaymath}
We set $\lE_0$ to be the identity on $\A$, and decree that $\lE_1$ sends any
