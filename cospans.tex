...
Notice that since the target decoration is part of the sum $\left(m+m',F(\iota_m)x+F(\iota_{m'})x'\right)$ in the Grothendieck category $\inta F$ by \cref{eq:fibrewisecoprod}, this isomorphism we constructured is the canonical such that it commutes with the inclusions into the sum.
This is useful when checking associativity for example: there is only one such canonical isomorphism $\lE(M)\otimes\lE(M')\otimes\lE(M'')\simrightarrow\lE(M\ot M'\ot M'')$ for any three structured cospans\footnote{A similar argument establishes that there is only one canonical isomorphism $\lE(M)\odot\lE(M')\odot\lE(M'')\to\lE(M\odot M'\odot M'')$ for any three composable structured cospans earlier, since the target decoration of \cref{eq:isomorphismdiag} forms a pushout in the Grothendieck category.}.
