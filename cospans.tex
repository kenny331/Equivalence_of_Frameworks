...
%Notice that in the above proof, when we compose two structured cospans, namely two horizontal 1-cells in the double category ${}_L\lCsp(\inta F)$ as in \cref{eq:pushoutissum}, the resulting composite cospan apex ends up being a sum on the second variable due to the definition of the left adjoint functor $L$ which forces the feet of all cospans to involve a fiberwise initial object.Thus in fact the assumptions of \cref{thm:equiv} could be weakened to state that the pseudofunctor $F\colon \A\to\SMC$ factors through the 2-category of categories with finite coproducts.   
