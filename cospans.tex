...
Suppose $\ca{A}$ has finite colimits and $(F,\phi,\phi_0)\maps(\ca{A},+) \to (\bicat{Cat},\times,1)$ is a symmetric lax monoidal pseudofunctor that factors through $\bicat{Rex}$ as an ordinary pseudofunctor. 
...
\begin{proof}
This was shown by Moeller and the third author \cite[Theorems~3.13\&4.1]{MV}. In summary, we have correspondences
\begin{gather}
...
\end{gather}
The second equivalence was also earlier observed by Shulman \cite{Shul2008}. In more detail, if $(\mu,\mu_0)$ is the lax monoidal structure of the pseudofunctor $F$ as recalled in \cref{sec:2cats}, each fibre category $\X_a=F(a)$ obtains a monoidal structure via
...
$(\X,+)\to(\A,+)$ that strictly preserve chosen coproducts and initial object, and such that $+$ preserves cocartesian liftings, bijectively correspond to pseudofunctors into the 2-category of cocartesian categories; for more details, see \cite[Corollary 4.7]{MV} and the related discussion. %!!! Christina: someone wrote Rex there, perhaps it was me. This is a subcategory, not all finite colimits are there.
...
Finally, if that pseudofunctor is lax monoidal $(F,\phi,\phi_0)\maps(\A,+)\to(\bicat{Cat},\times)$ to begin with, the monoidal Grothendieck construction in the cocartesian case discussed earlier \cref{eq:explicitstructure1} expresses $\bot_a$ as the image of the composite
