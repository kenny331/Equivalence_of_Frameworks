...
\end{itemize}
%\end{defn}
This is an opfibration over $\A$, with fibre categories $(\inta F)_a=F(a)$ and reindexing functors $f_!=F(f)$. In this way, the Grothendieck construction induces the following equivalence.
...
to construct a left adjoint $L_F\colon\A\to\inta F$ which is also a right inverse, namely $U_FL_F=\mathrm{id}_\A$. Explicitly, this left adjoint is given by $L(a)=F(f)\circ\phi_0(*)$ as in \cref{eq:initialinfibre}. 

We can now construct the double category of structured cospans $\scsp{L_F}{\inta F}$
