...
In this section we prove that two different frameworks which utilize symmetric monoidal double categories, namely decorated cospans and structured cospans described in \cref{sec:structuredcospans,DecCospansDoubleCat}, are equivalent. This is established by \cref{Equiv}, under some assumptions relevant to the existence of left adjoints for opfibrations. We begin by elaborating this underlying framework, using results mainly from \cite{CV} and \cite{MV}.
...
{\chris A little bit for monoidal Grothendieck here}

For our purposes, we are interested in the fact that under certain conditions, the induced monoidal opfibration by the Grothendieck construction $ \int{F} \to \A$ will be a right adjoint to some adjoint $L_F \colon \A \to \X$, and moreover the unit of the adjunction is the identity; this is called \emph{lari} in [Gray].

{\chris in progress, probably split up in two}
...
In the above described setting, a symmetric lax monoidal functor gives rise to a double category of decorated cospans as well as a double category of structured cospans. The result below establishes their equivalence.  
