...
We denote by $\opICat(\A)$ the 2-category 
%$[\mathbf{A},\mathbf{Cat}]_{\textnormal{pseudo}}$
...
In this section we prove that two different frameworks which utilize symmetric monoidal double categories, namely decorated cospans and structured cospans described in \cref{sec:structuredcospans,DecCospansDoubleCat}, are equivalent under certain assumptions. This is established by \cref{Equiv} which is based on a framework relevant to the existence of left adjoints for opfibrations. We begin by sketching this underlying framework, which is worked out in detail in \cite{CV} and \cite{MV}. Standard material on fibrations and the Grothendieck construction can be found for example in \cite{Herm}, \cite{Borc}.
...
First of all we recall some basics regarding opfibrations and their relation to pseudofunctors into $\bicat{Cat}$ via the Grothendieck construction.
{\chris Below I would suggest to change D to X because that's what we agreed on, and moreover the cocartesian universal property is a bit more general than that! However, do we really want to go into such detail? Also, more notation changes needed.}
\begin{defn}
...
Let $\OpFib(\A)$ denote the 2-subcategory of the slice $\mathbf{Cat}/ \A$ of opfibrations over $\A$, opfibrations morphisms and natural transformations with vertical components. In fact, there is a 2-equivalence between opfibrations over $\A$ and pseudofunctors $\A\to\ca{Cat}$ induced by the so-called Grothendieck construction.
...
The Grothendieck construction gives rise to a 2-functor $[\A,\mathbf{Cat}] \to \mathbf{Cat}/ \mathrm{A}$ which factors through the image of the embedding $\textnormal{Opfib}(\mathrm{A}) \hookrightarrow \mathbf{Cat} / \mathrm{A}$ as per the following theorem.
...
\begin{thm}\label{thm:Grothendieck}\hfill
    \begin{enumerate}
        \item Every opfibration $\U \maps \D \to \C$ gives rise to a pseudofunctor $F_U \maps \C \to \bicat{Cat}$.
...
\end{gather}
where the second equivalence was already observed by Shulman in \cite{Shul2}. Moreover, the above further restricts to the case when the Grothendieck category is specifically cocartesian monoidal itself, see \cite[Cor. 4.8]{MV} for more details. Then, the monoidal opfibration clauses for $U\colon(\X,+,0)\to(\A,+,0)$ in fact translate to the functor (strictly) preserving coproducts and the initial object, and bijectively correspond to pseudofunctors $F\colon\A\to\bicat{cocartCat}$ into the 2-category of cocartesian categories, functors that preserve coproducts and initial objects and natural transformations. 

Finally, using the following result we can further restrict to our current case of interest, namely opfibrations that also preserve pushouts, i.e. all finite colimits. Of course this is more general since it relates the existence of any class of colimits in the total category of an opfibration to their existence in the fibres; for more details, see \cite{Herm}.

\begin{lem}\label{lem:fibrewiselimits}
Suppose $\ca{J}$ is a small category and $\U\colon\X\to\A$ is an opfibration. If $\A$ has $\ca{J}$-colimits,
the following are equivalent:
\begin{enumerate}
 \item all fibres have $\ca{J}$-colimits, and the reindexing functors preserve them;
 \item $\X$ has $\ca{J}$-colimits, and $\U$ strictly preserves them.
\end{enumerate}
\end{lem}
...
\begin{prop}\cite[Prop.\ 4.4]{Gray} \label{prop:opfibtolari}
  Let $\U \colon\D \to \C$ be an opfibration. Then
...
Suppose $\mathsf{A}$ and $\mathsf{X}$ are finitely cocomplete categories and $F \colon(\mathsf{A},+,0) \to (\mathbf{Cat},\times,\mathbf{1})$ is a symmetric lax monoidal pseudofunctor. %such that as an ordinary pseudofunctor, it factors through $\bicat{fcocCat}$. 
The symmetric monoidal double category $F\mathbb{C}\textnormal{sp}$ of decorated cospans (\cref{thm:decoratedcospans}) is equivalent to the symmetric monoidal double category $_{L_F} \mathbb{C}\textnormal{sp}(\mathsf{X})$ of structured cospans (\cref{SC}), where $L_F \colon \mathsf{A} \to \int{F}$ is the left adjoint of the induced Grothendieck opfibration $\U_F\colon \int{F} \to \mathsf{A}$ by \cref{thm:Grothendieck}, under the assumptions of \cref{cor:fcocMonGroth}.
\end{thm}
\begin{proof}
{\chris in progress}
