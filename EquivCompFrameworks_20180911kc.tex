...
\begin{equation}\label{eqn:no2morphism}
\begin{tikzpicture}
  [scale=.8,auto=left]
%\node (D) at (9,11.5) {$1$};
...
This double category is in fact fibrant and so a result of Shulman \cite{Shul} allows us to extract from this symmetric monoidal double category $F \mathbb{C}$sp a symmetric monoidal bicategory $H(F \mathbb{C}\textnormal{sp})$ as the `horizontal edge bicategory' of the double category $F \mathbb{C}$sp. This bicategory $H(F \mathbb{C}\textnormal{sp})$ has:
\begin{enumerate}
\item{objects as those of $\mathrm{A}$,}
...
\]
and}
\item{2-morphisms as maps of cospans with the same feet together with a natrual transformation.
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a_1$};
\node (B) at (1,1) {$b$};
...
\end{enumerate}
This bicategory $H(F\mathbb{C}\textnormal{sp})$ is superior to the original bicategory $F$Cospan$(A)$ constructed by the second author \cite{Cour} in that it has the 2-morphisms that one would expect such a bicategory to have, such as \ref{eqn:no2morphism}. \textbf{marker}
...
In the present work, we construct a symmetric monoidal `double category' $\mathbb{F}\textnormal{Cospan}(\textrm{A})$. The double category $\mathbb{F}\textnormal{Cospan}(\textrm{A})$ that we construct in this paper has:
\begin{enumerate}
