...
Our approach is to instead view the functor $F$ as a pseudofunctor $F \colon \textrm{A} \to \mathbf{Cat}$ and then consider $F(a)$ not as a \emph{set} of decorations on the object $a$ but as a \emph{category} of decorations on the element $a$. Then we first construct a symmetric monoidal double category $F \mathbb{C}$sp. This symmetric monoidal double category will have:
\begin{enumerate}
\item{objects as those of $\mathrm{A}$ and}
