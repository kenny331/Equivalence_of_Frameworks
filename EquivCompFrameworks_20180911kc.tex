...
The outline for the paper is as follows: In the second section we present the definition of double category and symmetric monoidal double category, both of which can be found in earlier works \cite{Shul}. In the third section, we construct the symmetric monoidal double category $F\textnormal{Cospan}(\textrm{A})$ mentioned above as well as a result of Shulman that allows us to obtain a symmetric monoidal bicategory whose decategorification is an improved version of Fong's symmetric monoidal category. In the fourth section we investigate when a symmetric lax monoidal pseudofunctor gives rise to a left adjoint via the Grothendieck construction. In the fifth section we review another compositional framework which uses cospans equipped with extra structure due to the first two authors, namely `structured cospans' and prove that the double categorical versions of decorated cospans and structured cospans are equivalent, and in the final section, we present a few examples that can be realized with either framework and exhibit the equivalences.
...
%\begin{enumerate}
%\item{Need to build a symmetric monoidal decorated cospan category $F$Cospan$(\mathrm{A})$ from a lax symmetric monoidal pseudofunctor $F \colon \mathrm{A} \to \bold{Cat}$.}
%\item{Need to view $F$Cospan$(\mathrm{A})$ as a one object bicategory and then a one object pseudo double category.}
...
%\item{Define a symmetric monoidal lax double functor $\mathbb{F} \colon$ $F$Cospan$(\mathrm{A}) \to \mathbb{C} \bold{at}$.}
%\item{Define a symmetric monoidal oplax double functor $\mathbb{I} \colon 1 \to \mathbb{C} \bold{at}$ where $1$ is the one object double category.}
%\item{Obtain a symmetric monoidal double category of decorated cospans $\mathbb{F}$Cospan$(\mathrm{A})$ as a comma object from the cospan $1 \xrightarrow{ \mathbb{I} } \mathbb{C}\bold{at} \xleftarrow{ \mathbb{F} } F\textrm{Cospan}(\mathrm{A})$ in the 2-category $\mathbb{DBL}$ of monoidal double categories and lax functors. (Note that the functor $\mathbb{I} \colon 1 \to \mathbb{C}\bold{at}$ is actually pseudo, and hence both lax and oplax.)} 
%\item{Figure out what a (double?) equivalence of symmetric monoidal double categories is.}
%\item{Show that $_F \textrm{Csp}(\mathrm{A})$ is equivalent to $\mathbb{F}\textrm{Cospan}(\mathrm{A})$ as symmetric monoidal double categories, where $_F \textrm{Csp}(\mathrm{A})$ is the structured cospan double category that comes from the double category Csp$(\mathrm{A})$ and the functor $F \colon \mathrm{A} \to \bold{Cat}$.}
%\end{enumerate}
...
If $\mathrm{A}$ is a category with finite colimits, there is a well-known symmetric monoidal pseudo double category $\mathbb{C} \textnormal{ospan}(\mathrm{A})$ which has:
\begin{enumerate}
\item{objects as those of $\mathrm{A}$,}
\item{vertical 1-morphisms as morphisms of $\mathrm{A}$,}
\item{horizontal 1-cells as cospans in $\mathrm{A}$, and}
...
The symmetric monoidal category $(\bold{Cat},\times,1)$ consisting of categories and functors can naturally be viewed as a one object bicategory using a standard level shift. This one object bicategory, which we denote as $\textnormal{BD}(\bold{Cat})$, will have:
\begin{enumerate}
...
\node (C) at (2,0) {$\mathbb{C}\textnormal{ospan}(\mathrm{A})$};
\path[->,font=\scriptsize,>=angle 90]
...
The oplax double functor $\mathbb{G} \colon 1 \to \mathbb{B}\textnormal{D}(\bold{Cat})$ is trivially defined modulo $\mathbb{G}$ mapping the identity horizontal 1-cell to the monoidal unit of $\mathbb{B}\textnormal{D}(\bold{Cat})$ which is given by the terminal category $1$. The category $\mathrm{A}$ has finite colimits and can be viewed as a 2-category with only trivial 2-morphisms. Then from a symmetric lax monoidal pseudofunctor $F \colon \mathrm{A} \to \bold{Cat}$, we can obtain symmetric lax monoidal lax double functor $\mathbb{F} \colon \mathbb{C}\textnormal{ospan}(\mathrm{A}) \to \mathbb{B}\textnormal{D}(\bold{Cat})$ which is defined as the pseudofunctor $F$ on horizontal 1-cells and 2-morphisms; note that $\mathbb{B}\textnormal{D}(\bold{Cat})$ has a single object and an identity vertical 1-morphism. Thus $\mathbb{F}$ will map a cospan in $\mathrm{A}$: $$a \rightarrow c \leftarrow b$$ to $F(c) \in \bold{Cat}$ and 
\begin{thm}
Let $F \colon \mathrm{A} \to \bold{Cat}$ be a symmetric lax monoidal pseudofunctor. Then there exists a symmetric (lax) monoidal lax double functor $\mathbb{F} \colon \mathbb{C}\textnormal{ospan}(\mathrm{A}) \to \mathbb{B}\textnormal{D}(\bold{Cat})$ which is defined:
\begin{enumerate}
...
\item{as the pseudofunctor $F \colon \mathrm{A} \to \bold{Cat}$ on horizontal 1-cells, meaning that $$(c_1 \xrightarrow{i} c \xleftarrow{o} c_2) \mapsto F(c)$$ and}
\item{as the pseudofunctor $F \colon \mathrm{A} \to \bold{Cat}$ on 2-morphisms, meaning that 
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$c_1$};
\node (B) at (1,0) {$c$};
...
\end{proof}
\begin{prop}
Let $1$ denote the trivial double category with a single object and only identities. Then there exists a symmetric lax monoidal oplax double functor $\mathbb{G} \colon \mathbf{1} \to \mathbb{B}\textnormal{D}(\bold{Cat})$ which is defined:
\begin{enumerate}
\item{trivially on objects, as both $1$ and $\mathbb{B} \textnormal{D}(\bold{Cat})$ have a single object,}
\item{trivially on vertical 1-morphisms, as both $1$ and $\mathbb{B} \textnormal{D}(\bold{Cat})$ only have a single identity vertical 1-morphism,}
\item{the identity horizontal 1-cell of $1$ maps to the monoidal unit of $(\mathbb{B}\textnormal{D}(\bold{Cat}),\times, 1)$, which is the terminal category $1$ with a single object and identity morphism, and}
\item{the identity 2-morphism maps to the identity functor $! \colon 1 \to 1$ on the terminal category.}
\end{enumerate}
\end{prop}
...
\]
\end{defn}
Using this definition we want to construct a `pseudo comma double category' from the lax double functor $\mathbb{F} \colon \mathbb{C}\textnormal{ospan}(\mathrm{A}) \to \mathbb{B} \textnormal{D}(\bold{Cat})$ and the oplax double functor $\mathbb{G} \colon 1 \to \mathbb{B}\textnormal{D}(\bold{Cat})$. First, we define how this pseudo comma double category $(\mathbb{G} / \mathbb{F}) \coloneqq \mathbb{F}\textnormal{Cospan}(\mathrm{A})$ is obtained.
\textbf{Should this be a proposition or a definition? Ask Dr. Baez and Christina for their opinions.}
\begin{defn}
Let $\mathbb{G} \colon \lC \to \lE$ be an oplax double functor and $\mathbb{F} \colon \lD \to \lE$ be a lax double functor where $\lC = (\mathrm{A}_{0},\mathrm{A}_{1}), \lD = (\bold{D}_{0}, \bold{D}_{1})$ and $\lE = (\bold{E}_{0}, \bold{E}_{1})$ are pseudo  double categories with $\mathrm{A}_{0}, \mathrm{A}_{1}$ the category of objects and category of arrows of the double category $\lC$, respectively, and likewise for $\bold{D}_{0}, \bold{D}_{1}, \bold{E}_{0}$ and $\bold{E}_{1}$. Then the \define{pseudo comma double category} determined by the double functors $\mathbb{G}$ and $\mathbb{F}$ is the pseudo double category $(\mathbb{G} / \mathbb{F})$ consisting of a category of objects $(\mathbb{G} / \mathbb{F})_{0}$ and category of arrows $(\mathbb{G} / \mathbb{F})_{1}$ where the category of objects $(\mathbb{G} / \mathbb{F})_0$ is the comma category obtained from the functors $\mathbb{G}_0 \colon \mathrm{A}_{0} \to \bold{E}_{0}$ and $\mathbb{F}_0 \colon \bold{D}_{0} \to \bold{E}_{0}$ and $(\mathbb{G} / \mathbb{F})_1$ is the comma category obtained from the functors $\mathbb{G}_{1} \colon \mathrm{A}_{1} \to \bold{E}_{1}$ and $\mathbb{F}_1 \colon \bold{D}_{1} \to \bold{E}_{1}$.
\end{defn}
\begin{thm}
Let $\mathbb{G} \colon 1 \to \mathbb{B}\textnormal{D}(\bold{Cat})$ be the oplax double functor previously defined and let $\mathbb{F} \colon \mathbb{F}\textnormal{Cospan}(\mathrm{A}) \to \mathbb{B}(\bold{Cat})$ be the lax double functor previously defined. 
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$1$};
\node (B) at (1,1) {$\mathbb{B}\textnormal{D}(\bold{Cat})$};
...
\end{tikzpicture}
\]
Then there exists a category $(\mathbb{G} / \mathbb{F})_0 \coloneqq (\mathbb{G}_0 / \mathbb{F}_0)$ obtained as a comma category from the functors $\mathbb{F}_0 \colon \mathbb{C}\textnormal{ospan}(\mathrm{A})_0 \to \mathbb{B}\textnormal{D}(\bold{Cat})_0$ and $\mathbb{G}_0 \colon 1 \to \mathbb{B}\textnormal{D}(\bold{Cat})_0$ which has:
\begin{enumerate}
\item{objects as those of $\mathrm{A}$ and}
\item{morphisms as those of $\mathrm{A}$,}
\end{enumerate}
and there exists a category $(\mathbb{G} / \mathbb{F})_1 \coloneqq (\mathbb{G}_1 / \mathbb{F}_1)$ obtained as a comma category from the functors $\mathbb{F}_1 \colon \mathbb{C}\textnormal{ospan}(\mathrm{A})_1 \to \mathbb{B}\textnormal{D}(\bold{Cat})_1$ and $\mathbb{G}_1 \colon 1 \to \mathbb{B}\textnormal{D}(\bold{Cat})_1$ which has:
\begin{enumerate}
\item{objects as $F$-decorated cospans of $\mathrm{A}$, which are pairs:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,1) {$c$};
...
\]
and}
\item{morphisms as maps of cospans of $\mathrm{A}$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0.5) {$a$};
...
\node (C) at (2,0) {$\mathbb{C}\textnormal{ospan}(\mathrm{A})_0=\mathrm{A}$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$\mathbb{G}_0$} (B)
(C) edge node[above]{$\mathbb{F}_0$} (B);
\end{tikzpicture}
\]
Thus $(\mathbb{G} / \mathbb{F})_0$ will be the comma category which has objects given by triples $(\star,c,\alpha \colon \mathbb{G}(\star) \to \mathbb{F}(c))$ where $\star$ is the single object of $1$, $c$ is a object of $\mathrm{A}$ and $\alpha \colon \mathbb{G}(\star) \to \mathbb{F}(c)$ is a morphism in $\mathbb{B}\textnormal{D}(\bold{Cat})_0$. The pseudo double category $\mathbb{B}\textnormal{D}(\bold{Cat})$ only has a single object $\star$ and identity vertical 1-morphism, so the triple $(\star,c,\alpha \colon \mathbb{G}(\star) \to \mathbb{F}(c))$ amounts to an object of $\mathrm{A}$ together with trivial structure. Given another object $(\star, c^\prime, \alpha^\prime \colon \mathbb{G}(\star) \to \mathbb{F}(c^\prime))$, a morphism from the first triple to the second triple is a pair $(\beta, \gamma)$ where $\beta = \id_{\star}$ is a morphism in $1$ and $\gamma \colon c \to c^\prime$ is a morphism in $\mathrm{A}$ such that the following diagram commutes:
\[
\begin{tikzpicture}[scale=1.5]
...
\end{tikzpicture}
\]
Simplifying things a bit, $\mathbb{G}(\delta)$ must be the identity 2-morphism of $1$ and $h$ is a morphism of $\mathbb{C}\textnormal{ospan}(\mathrm{A})$, namely a triple of morphisms $(h_1,h,h_2)$ in $\mathrm{A}$ that makes the following diagram commute:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0.5) {$a$};
\node (A') at (0,-0.5) {$a^\prime$};
...
\end{tikzpicture}
\]
which is just a morphism $\iota \colon F(h)(d) \to d^\prime$ in $F(c^\prime)$. Thus a morphism of $(\mathbb{G} / \mathbb{F})_1$ is a map of cospans in $\mathrm{A}$ together with a natural transformation that that makes the above triangle commute.
\newline
...
\end{enumerate}
We thus have two symmetric monoidal double categories: $_L \mathbb{C}\textnormal{sp}(\mathrm{Graph})$ obtained from structured cospans and $\mathbb{F}\textnormal{Cospan}(\mathrm{FinSet})$ using Theorem \ref{main1}. These two double categories are in fact equivalent as symmetric monoidal double categories which we now show.

\textbf{Probably delete stuff after this line.}

Define a double functor $\mathbb{E} \colon _L \mathbb{C}\textnormal{sp}(\mathrm{Graph}) \to \mathbb{F}\textnormal{Cospan}(\mathrm{FinSet})$ as follows: $\mathbb{E}_0=\id_{\mathrm{FinSet}}$ as both double categories $_L \mathbb{C}\textnormal{sp}(\mathrm{Graph})$ and $\mathbb{F}\textnormal{Cospan}(\mathrm{FinSet})$ have $\mathrm{FinSet}$ as their category of objects. The identity functor on $\mathrm{FinSet}$ is trivially an equivalence. For the arrow component $\mathbb{E}_1$, given a horizontal 1-cell of $_L \mathbb{C}\textnormal{sp}(\mathrm{Graph})$, which is a cospan in $\mathrm{Graph}$ of the form:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(N)$};
\node (B) at (1,0) {$M$};
...
\end{tikzpicture}
\]
the functor $\mathbb{E}_1$ maps this horizontal 1-cell to the horizontal 1-cell of $\mathbb{F}\textnormal{Cospan}(\mathrm{FinSet})$ given by:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$N$};
\node (B) at (1,0) {$R(M)$};
...
\end{tikzpicture}
\]
where $R \colon \mathrm{Graph} \to \mathrm{FinSet}$ is the forgetful functor, right adjoint to $L \colon \mathrm{FinSet} \to \mathrm{Graph}$. The image of a 2-morphism in $_L \mathbb{C}\textnormal{sp}(\mathrm{Graph})$ given by:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(N_1)$};
\node (B) at (1,0) {$M$};
...
\end{tikzpicture}
\]
is the 2-morphism in $\mathbb{F}\textnormal{Cospan}(\mathrm{FinSet})$ given by:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$N_1$};
\node (B) at (1,0) {$R(M)$};
...
\end{tikzpicture}
\]
together with a graph morphism $\iota_h \colon F(R(h))(M) \to M^\prime$. Denoting a cospan by its apex, the map $$\mathbb{E}_1 \colon { _f { _L \mathbb{C}\textnormal{sp}(\mathrm{Graph})}_g} (M,M^\prime) \to { _{\mathbb{E}(f)} \mathbb{F}\textnormal{Cospan}(\mathrm{FinSet})_{\mathbb{E}(g)}}(\mathbb{E}(M),\mathbb{E}(M^\prime))$$ is bijective and thus the double functor $\mathbb{E}$ is full and faithful. To see that it is also essentially surjective, given a horizontal 1-cell in $\mathbb{F}\textnormal{Cospan}(\mathrm{FinSet})$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$N_1$};
\node (B) at (1,0) {$N$};
\node (C) at (2,0) {$N_2$};
\node (D) at (3,0) {$M \in F(N)$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$i$} (B)
(C) edge node[above]{$o$} (B);
\end{tikzpicture}
\]
there exists a horizontal 1-cell in $_L \mathbb{C}\textnormal{sp}(\mathrm{Graph})$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(N_1)$};
\node (B) at (1,0) {$M$};
...
\end{tikzpicture}
\]
and a 2-isomorphism in $\mathbb{F}\textnormal{Cospan}(\mathrm{FinSet})$ from the image of the latter horizontal 1-cell to the former given by:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$N_1$};
\node (B) at (1,0) {$R(M)$};
...
which trivially commutes as $R(M)=N$ and $R(i^\prime)$ and $R(o^\prime)$ are the underlying maps of vertices of the graph morphisms $i^\prime$ and $o^\prime$ which act as the functions $i$ and $o$, respectively. Given two composable horizontal 1-cells $\alpha$ and $\beta$ of $_L \mathbb{C}\textnormal{sp}(\mathrm{Graph})$: 
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(N_1)$};
\node (B) at (1,0) {$M$};
