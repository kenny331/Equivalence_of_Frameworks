...
\begin{thm}\label{thm:Grothendieck}\hfill
    \begin{enumerate}
        ...
        \item Every opindexed category $F \maps \A \to Cat$ gives rise to  an opfibration $\U_F \colon \inta F\to\A$.
...
Finally, if that pseudofunctor is lax monoidal $(F,\phi,\phi_0)\colon(\A,+,0)\to(\bicat{Cat},\times,\mathbf{1})$ to begin with, the monoidal Grothendieck construction in the cocartesian case discussed earlier (\cref{eq:explicitstructure1}) express $\bot_a$ as the image of the composite
...
to construct a left adjoint $L_F\colon\A\to\inta F$ which is also a right inverse, namely $U_FL_F=\mathrm{id}_\A$. Explicitly, this left adjoint is given by $L(a)=(a,\bot_a)$, picking the initial object in the finitely cocomplete fibre also expressed as $F(u_a)\circ\phi_0(*)$ as in \cref{eq:initialinfibre}. Diagrammatically,
\begin{displaymath}
...
\end{equation}
according to \cref{def:GrothCat}. The 2-morphisms in this double category are as in \cref{eq:2cellsStrCsp}.

In order to prove that there is a double equivalence between $\dcsp{F}$ and $\scsp{L_F}{\inta F}$, we define a %n identity-on-objects and vertical morphism
double functor
\begin{displaymath}
\mathbb{E}\colon\scsp{L_F}{\inta F} \longrightarrow\dcsp{F}
\end{displaymath}
as follows. The object component of $\mathbb{E}=(\mathbb{E}_0,\mathbb{E}_1)$ is $\mathbb{E}_0 = \id_{\mathsf{A}}$ since both double categories have $\mathsf{A}$ as their vertical category; trivially, $\mathbb{E}_0$ is an equivalence of categories. Given a horizontal 1-cell in $\scsp{L_F}{\inta F}$ like \cref{eq:scsphor1cell}, the arrow component $\mathbb{E}_1$ of the double functor maps it simply to the decorated cospan
\begin{displaymath}
 a\xrightarrow{\; i \;}c\xleftarrow{\; o \;}b\;\textrm{ with decoration }x\in F(c)
\end{displaymath}
which is really a bijective correspondence since the unique maps from the initial objects in the fibres are always there. {\chris maybe too informal explanation!}
