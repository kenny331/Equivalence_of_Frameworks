...
\item {\chris Where should we put some standard things we use? For exampe, lax monoidal pseudofunctor should be recalled early. Together with pseudonatural transformations I believe.}
...
In this section we prove that two different frameworks which utilize symmetric monoidal double categories, namely structured cospans and decorated cospans described respectively in \cref{DecCospansDoubleCat,sec:structuredcospans}, are equivalent under certain assumptions. This is established by \cref{Equiv}, which relies on conditions relevant to the existence of left adjoints for opfibrations. We begin by sketching this underlying framework, which is worked out in detail in \cite{MV} and \cite{CV}.
...
First of all, we recall some basics regarding opfibrations and their relation to pseudofunctors into $\bicat{Cat}$ via the Grothendieck construction. Standard material on fibration theory and the Grothendieck construction can be found for example in \cite{Herm}, \cite{Borc}.
...
%\end{defn}
The category $\X$ is called the \textbf{total} category, whereas $\A$ is the \textbf{base} category of the opfibration. For any $a\in\A$, the \textbf{fibre} $\X_a$ consists of all objects that map to $a$ and vertical morphisms between them, i.e. mapping to $1_a$.
Assuming the axiom of choice, we may select a cocartesian arrow over each $f\maps a\to b$ in $\A$ and $x\in\X _a$, denoted by $\mathrm{Cocart}(f,x)\maps x\to f_!(x)$; $U$ is then called a \textbf{cloven opfibration}. This choice induces the so-called \textbf{reindexing functors} $f_!\colon\X _a\to\X _b$ between the fibre categories, which by the liftings universal property adhere to $(1_a)_!\cong1_{\X _a}$ and $(f\circ g)_!\cong f_!\circ g_!$. If these isomorphisms are equalities, we have the notion of a \textbf{split} opfibration.
...
\end{itemize}
%\end{defn}
This is an opfibration over $\A$, with fibre categories $(\int F)_a=F(a)$ and reindexing functors $f_!=F(f)$. In this way, the Grothendieck construction induces the following equivalence.
...
%: $$\int F \colon [ \mathrm{A}, \mathbf{Cat} ] \to \textnormal{Opfib}(\mathrm{A}) \hookrightarrow \mathbf{Cat} / \mathrm{A}.$$
...
\begin{thm}\label{thm:Grothendieck}\hfill
    \begin{enumerate}
        ...
        \item Every opindexed category $F \maps \A \to Cat$ gives rise to  an opfibration $\U_F \colon \int F\to\A$.
...
In \cite{MV}, the Grothendieck construction is generalized to the monoidal setting, namely lax monoidal structures on a pseudofunctor $F\colon\A\to\bicat{Cat}$ bijectively correspond to monoidal structures on the total category $\int F$ such that the induced opfibration is a strict monoidal functor and $\otimes_{\int F}$ is cartesian, see \cite[Thm. 3.10]{MV}. If the monoidal category $\A$ is in fact cocartesian monoidal, there is a further correspondence of the two structures with an ordinary pseudofunctor $F\colon\A\to\ca{MonCat}$:
\begin{gather}
...
\end{gather}
where the second equivalence was already observed by Shulman in \cite{Shul2}. In more detail, if $(\phi,\phi_0)$ is the lax monoidal structure of $F$, each fibre category $\X_a=F(a)$ obtains a monoidal structure via
...
The first part formulates the existence of colimits \emph{locally} in each fiber, which can equivalently be expressed by the corresponding pseudofunctor $F\colon\A\to\bicat{Cat}$ landing on the sub-2-category $\bicat{fcocCat}$ of finitely cocomplete categories and functors that preserve colimits; the second part formulates the existence of colimits \emph{globally} in the total category $\int F$. Combining these, we can deduce the following.
...
\begin{cor}\label{cor:fcocMonGroth}
 ...
 factors through $\bicat{fcocCat}$, then the Grothendieck category $\int F$ has all finite colimits and the induced opfibration $\U_F\colon\int F\to\A$ preserves them. 
\end{cor}

On a different but related subject, for our purposes we are interested in the existence of a left adjoint $L_F \colon \A \to \X$ for the induced monoidal opfibration $ \int{F} \to \A$ by the Grothendieck construction as discussed above. The following result provides sufficient conditions for that, which go hand-in-hand with \cref{cor:fcocMonGroth} and \cref{lem:fibrewiselimits}.
...
\begin{proof}\emph{(Sketch)}
 The left adjoint $L\colon\A\to\X$ maps an object $a$ to the initial object in its fibre $\X_a$, denoted by $\bot_a$. By construction, $U(L(a))=U(\bot_a)=a$.
\end{proof}
...
In this case, $\bot_a$ is precisely the cocartesian lifting of $0_\X$ under the unique map $f\colon0_\A$ in the base category:
\begin{displaymath}
\xymatrix @C=.4in @R=.2in
{0_\X \ar @{.>}[d]\ar[rr]^-{\mathrm{Cocart}(0_\X,f)} && f_!(0_X)=:\bot_a\ar @{.>}[d] & \textrm{in }\X  \\
0_\A\ar[rr]_-{f}\ar[rr]^-{\exists!} && a & \textrm{in }\A}
\end{displaymath}
Moreover, if $U=U_F$ for a pseudofunctor $F\colon\A\to\bicat{Cat}$ under \cref{thm:Grothendieck}, the initial object in each fibre is precisely $F(f)(0_\X)$, for $f$ the unique map from $0_\A$ as above. Finally, if that pseudofunctor is lax monoidal $(F,\phi,\phi_0)\colon(\A,+,0)\to(\bicat{Cat},\times,\mathbf{1})$ to begin with, the monoidal Grothendieck construction in the cocartesian case discussed earlier (\cref{eq:explicitstructure1}) express $\bot_a$ as the image of the composite
\begin{equation}\label{eq:initialinfibre}
\mathbf{1}\xrightarrow{\phi_0}F(0_\A)\xrightarrow{F(f)}F(a) 
\end{equation}
\end{rmk}
...
We have now laid all the necessary theoretical background to formally establish an equivalence between the double category of decorated cospans and the double category of structured cospans constructed from a symmetric lax monoidal pseudofunctor into $\bicat{Cat}$.
...
Then the symmetric monoidal double category $F\mathbb{C}\textnormal{sp}$ of decorated cospans (\cref{thm:decoratedcospans}) is equivalent to the symmetric monoidal double category $_{L_F} \mathbb{C}\textnormal{sp}(\mathsf{X})$ of structured cospans (\cref{SC}), where $L_F \colon \mathsf{A} \to \int{F}$ is the left adjoint of the induced Grothendieck opfibration $\U_F\colon \int{F} \to \mathsf{A}$ %by \cref{thm:Grothendieck}, 
under the assumptions of \cref{cor:fcocMonGroth}.
\end{thm}

\begin{proof}
Recall that the double category of decorated cospans $\dcsp{F}$ has objects and vertical 1-morphisms those of $\A$, horizontal 1-cells $a\tickar b$ are cospans $\cspn{a}{c}{b}{}{}$ in $\A$ along with $d\in F(c)$, and 2-morphisms are cospan maps $k\colon c\to c'$ such that $(Fk)(d)=d'$, see \cref{eq:FCsp2morph}.

By \cref{cor:fcocMonGroth}, when $F$ as an ordinary pseudofunctor factors through $\bicat{fcocCat}$, the (monoidal) Gro\-the\-ndieck construction gives rise to a finitely cocomplete $\int F$ such that the corresponding opfibration $U_F\colon(\int{F},+,0)\to(\A,+,0)$ preserves all finite colimits. %Christina: maybe the cocartesian structures are redundant to write here, since U_F really preserves all finite colimits anyways.
...
to construct a left adjoint $L_F\colon\A\to\int F$ which is also a right inverse, namely $U_FL_F=\mathrm{id}_\A$. In fact, by \cref{rmk:important}, this left adjoint is given by $L(a)=F(f)\circ\phi_0(*)$, the initial object in the fibre above $a$.
