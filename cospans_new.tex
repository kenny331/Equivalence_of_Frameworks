...
One can also check that $\Phi$ is an opfibration. Given a morphism $f \maps a \to a'$ in $\A$ and an object $(a \xrightarrow{i}b, x) \in \A/U$ over $a$, we shall show that a cocartesian lifting of $f$ is given by 
\[ (f,g,1) \maps (a \xrightarrow{i} b, x) \, \to \, (a' \xrightarrow{i'} b+_a a', F(g)(x)). \]
where $b +_a a'$, $i'$ and $g$ arise from the following pushout square in $\A$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$a'$};
...
\end{tikzpicture}
\]
To show that $(f,g,1)$ is an cocartesian lifting of $f$, suppose we are given a morphism 
\[    (f'',g'',h'') \maps (a \xrightarrow{i} b, x) \; \to \; (a'' \xrightarrow{i''} b'', x'')   \]
in $\A/U$ and a morphism $k \maps a \to a''$ in $\A$ such that $\Phi(f'',g'',h'') = k \circ f$.   To
show $\Phi$ is an opfibration, we need to show there exists a unique morphism
...
To achieve this set $\tilde{f} = k$, $\tilde{h} = h''$, and define $\tilde{g}$ using
...
(D) edge [dashed] node[left]{$\exists! \, \tilde{g} \phantom{a}$} (E);
\end{tikzpicture}
\]
...
\end{tikzpicture}
\]
and consequently a natural isomorphism between our two objects of $(\A/U)_c$, showing that $\Phi$ satisfies the Beck--Chevalley condition.   We then define $F\lCsp$ to be $\Fr(\Phi)$.
\end{proof}
