...
We start with decorated cospans, proving Theorems \ref{thm:decorated_cospans_1} and \ref{thm:decorated_cospans_2} by applying Shulman's result to a particular monoidal bifibration built
from a lax monoidal pseudofunctor $F \maps (A,+) \to (\Cat,\times)$.    The Grothendieck construction gives a bijection between pseudofunctors $F \maps \A \to \Cat$ and opfibrations $U \maps \inta F \to \A$.  We need two refinements of this construction: one that gives monoidal ofibrations, and one that gives symmetric monoidal opfibrations.  We explain the Grothendieck construction and these related concepts in \cref{subsec:fibrations}; here we simply recall the result we need.
