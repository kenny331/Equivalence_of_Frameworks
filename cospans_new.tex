...
\item a morphism in $\inta F$ from $(b,p)$ to $(b',p')$ is a pair $(f,\zeta)$
where $g \maps b \to b'$ is a morphism in $\A$ and $\zeta \maps F(g)(p) \to p'$ is
a morphism in $F(b')$.
\end{itemize}
The functor $U$ maps objects and morphisms to their first component, so in the comma
category $\A/U$, 
\begin{itemize}
\item an object is a triple $(a,x,a \xrightarrow{i} U(x))$ with $a \in \A$ and $x=(b,p) \in \inta F$;
\item a morphism from $(a,x,a \xrightarrow{i} U(x))$ to $(a',x',a' \xrightarrow{i'} U(x'))$ is a pair $(f,\theta)$ where $f \maps a \to a'$ is a morphism in $\A$ and $\theta \maps x \to x'$ is a morphism in $\inta F$ such that this square commutes:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$a'$};
...
\]
\end{itemize}
We define the functor $\Phi \maps \A/U \to \A$ on objects by $\Phi(a,x,a \xrightarrow{i} U(x))=a$ and on morphisms by $\Phi(f,\theta)=f$. 

There is a monoidal structure on $\A/U$ where the tensor product of two objects $(a_1,x_1,a_1 \xrightarrow{i_1} U(x_1))$ and $(a_2,x_2,a_2 \xrightarrow{i_2} U(x_2))$ is given by
...
One can check that $\Phi$ is a fibration.  Given a morphism $f \maps a \to a'$ in $\A$ and an object $(a',x',a' \xrightarrow{i'} U(x')) \in \A/U$ over $a'$, a cartesian lifting of $f$ is given by 
\[ (f,\id) \maps (a,x',a \xrightarrow{i' \circ f} U(x')) \, \to \, (a',x',a' \xrightarrow{i'} U(x')). \]
...
One can also check that the functor $\Phi$ is an opfibration, but this takes a bit more work. Given a morphism $f \maps a \to a'$ in $\A$ and an object $(a,x,a \xrightarrow{i} U(x)) \in \A/U$ over $a$, we shall show that a cocartesian lifting of $f$ is given by 
\[ (f,\theta) \maps (a,x,a \xrightarrow{i} U(x)) \, \to \, (a',x', a' \xrightarrow{i'} U(x')). \]
where $i'$ arises from the following pushout square in $\A$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$a'$};
...
$i' \maps a' \to U(x')$ as desired.  Finally, define $\theta \maps x \to x'$ to be 
the morphism in $\inta F$ given by the pair $(g,\id)$ where $g \maps b \to b'$ is
given as above and $\id \maps F(g)(p) \to p'$ is the identity.

To show that $(f,\theta)$ is an cocartesian lifting of $f$, suppose 
