...
<<<<<<< Updated upstream
<<<<<<< Updated upstream
The functor $U_1$ is defined on objects by $U_1(a,x,a \xrightarrow{i} U(x))=a$ and on morphisms by $U_1(f,\alpha)=f$. We have a monoidal structure on the comma category $(\A/U)$ where the tensor product of two objects $(a_1,x_1,a_1 \xrightarrow{i_1} U(x_1))$ and $(a_2,x_2,a_2 \xrightarrow{i_2} U(x_2))$ is given by $$(a_1,x_1,a_1 \xrightarrow{i_1} U(x_1)) \otimes (a_2,x_2,a_2 \xrightarrow{i_2} U(x_2)) = (a_1+a_2,x_1+x_2,a_1+a_2 \xrightarrow{i_1+i_2} U(x_1+x_2))$$ where the natural  isomorphism $U(x_1)+U(x_2) \cong U(x_1+x_2)$ is implicit in the above composite, for reasons of space. The monoidal unit is given by $(0_\A,0_\X,0_\A \to U(0_\X))$. It is easy to see that the functor $U_1$ can be made into a strict monoidal functor using this tensor product.
=======
=======
>>>>>>> Stashed changes
The functor $\Phi$ is defined on objects by $U_1(a,x,a \xrightarrow{i} U(x))=a$ and on morphisms by $U_1(f,\alpha)=f$. We have a monoidal structure on the comma category $(\A/U)$ where the tensor product of two objects $(a_1,x_1,a_1 \xrightarrow{i_1} U(x_1))$ and $(a_2,x_2,a_2 \xrightarrow{i_2} U(x_2))$ is given by
...
\end{tikzpicture}
\]
Let $(b,x,b \xrightarrow{i} U(x))$ be an object of $(\A/U)_b$. Going right, we precompose with $h \colon a \to b$ to obtain the object $(a,x,a \xrightarrow{i \circ h} U(x))$ in $(\A/U)_a$. Then going down, we push forward along $k$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$U(x)$};
...
\end{tikzpicture}
\]
to obtain the object $(c,z,c \to U(z))$ of $(\A/U)_c$, where $z \in \int F$ has $U(z)=c+_a U(x)$. In particular, $z=L(c)+_{L(a)}x$ and as $U$ preserves finite colimits and $UL=1_\A$, we have that 
\[  U(z)=U(L(c)+_{L(a)}x)=U(L(c))+_{U(L(a))}U(x)=c+_a U(x).\]
...
\end{tikzpicture}
\]
which yields the object $(d,z',d \to U(z'))$ of $(\A/U)_d$ where $z' \in \int F$ such that $U(z')=d+_b U(x)$. In particular, $z'$ is given by $z'=L(d)+_{L(b)}x$. We then precompose with $f \colon c \to d$ to obtain the object $(c,z',c \to U(z'))$ of $(\A/U)_c$.

Now we seek a natural isomorphism between the two objects $(c,z,c \to c+_a U(x))$ and $(c,z',c \to d+_b U(x))$ in $(\A/U)_c$. It suffices to obtain a natural isomorphism between the two objects $z=L(c)+_{L(a)} x$ and $z'=L(d)+_{L(b)} x$ in $\X$. This pushout square:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(a)$};
\node (B) at (1.5,0) {$x$};
...
\end{tikzpicture}
\]
exhibits $z$ as a pushout of the span 
\[  L(c) \xleftarrow{L(k)} L(a) \xrightarrow{I \circ L(h)} x 
\]
in $\X$. As $L$ preserves pushouts, we may apply it to the original pushout square in $\A$
and paste on another pushout square:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(b)$};
\node (B) at (1.5,0) {$x$};
...
which exhibits $z'$ as a pushout of the same span in $\X$.  The resulting natural isomorphism
between $z$ and $z'$ gives the desired natural isomorphism between our two objects of $(\A/U)_c$, showing that $U_1$ satisfies the Beck--Chevalley condition.
\end{proof}
