...
\begin{lem}
\label{lem:MonGroth}
There is a 2-equivalence between the 2-categories of monoidal opfibrations $U: (\mathsf{X}, \otimes) \to (\mathsf{A}, \otimes)$ and monoidal pseudofunctors $F: (\mathsf{A},\otimes) \to (\mathsf{Cat}, \times)$ , and if $\mathsf{A}$ cocartesian monoidal, there is a 2-equivalence between these and pseudofunctors from $\A$ into $\MonCat$. Similarly there is a 2-equivalence between the 2-categories of symmetric monoidal opfibrations $U: (\mathsf{X}, \otimes) \to (\mathsf{A}, \otimes)$ and symmetric monoidal pseudofunctors $F: (\mathsf{A},\otimes) \to (\mathsf{Cat}, \times)$ , and if $\mathsf{A}$ cocartesian monoidal, there is a 2-equivalence between these and pseudofunctors from $\A$ into $\SMC$.
\end{lem}
...
\end{equation}
where $\iota_a \maps a\to a+b$ and $\iota_b \maps b \to a+b$ are the coproduct inclusions in $\A$ and the coproduct in the second component is in the fiber $F(a+b)$. {\chris Which parts of this section are actually used in the current proof of 4.1? Keep only those!}
...
\end{tikzpicture}
\]
Here $L(a)$ is the initial object in the fiber of $\X = \inta F$ over $a \in A$, namely $(a,t_a)$ 
where $t_a \in F(a)$ is the trivial decoration
\[    \one \xrightarrow{\phi_0} F(0_\A) \xrightarrow{F(!_a)} F(a). \]
{\chris Right, but $i$ and $o$ are also specific pairs of maps in the Grothendieck category, see equation 26 in earlier version.}
...
\end{tikzpicture}
\]
{\chris ... to that and that map of structured cospans!}
...
\begin{proof}
{\john Here we are using $\bot_b$ to mean trivial decoration.  Sometimes we do this, and
sometimes we use it to mean initial object in the fiber of $b$.  This leads to the strange
result $\bot_b = (b,\bot_b)$.}  
