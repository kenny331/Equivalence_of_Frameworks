...
Now we are ready to use Shulman's result to prove  \cref{thm:decorated_cospans_1} and  \cref{thm:decorated_cospans_2}.

\begin{proof}[Proof of \cref{thm:decorated_cospans_1}] 
To obtain the desired double category we apply \cref{lem:shulman} to a bifibration $\Phi \maps \A/U \to \A$ which we now define.   Let $U \maps \inta F \to \A$ be the monoidal opfibration associated to the lax monoidal pseudofunctor $F \maps (\A,+) \to (\Cat, \times)$.  As in \cref{def:GrothCat},
\begin{itemize}
\item an object of $\inta F$ is a pair $(b,x)$ where $b \in \A$ and $x \in F(b)$;
\item a morphism in $\inta F$ from $(b,x)$ to $(b',x')$ is a pair $(g,h)$
where $g \maps b \to b'$ is a morphism in $\A$ and $h \maps F(g)(x) \to x'$ is
a morphism in $F(b')$.
\end{itemize}
The functor $U$ maps objects and morphisms to their first component, so one can describe
the comma category $\A/U$ as follows:
\begin{itemize}
...
There is a monoidal structure on $\A/U$ coming from the monoidality of $U$.  On objects
this is given by
\[  (a \xrightarrow{i} b, x) \otimes (a' \xrightarrow{i'} b', x') = (a+a' \xrightarrow{i+i'} b+b', 
\phi_{a,b}(d,d') )\]
where $\phi$ is the laxator for $F$.  The monoidal unit is given by $(0_\A \xrightarrow{!} 0_\A, x)$
where $x \in F(0_\A)$ is the element given by $\phi_0 \maps \one \to F(0_\A)$.   With this monoidal structure it is easy to check that the functor $\Phi$ is strict monoidal.

One can check that $\Phi$ is a fibration.  Given a morphism $f \maps a \to a'$ in $\A$ and an object $(a' \xrightarrow{i'} b', x') \in \A/U$ over $a'$, a cartesian lifting of $f$ is given by 
...
Lastly, we need to check that $\Phi$ satisfies the Beck--Chevalley condition. Given a pushout square in $\A$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$b$};
...
\end{tikzpicture}
\]
we need to show that the natural transformation $\theta$, defined as in \cref{subsec:fibrations}, is an isomorphism:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$(\A/U)_a$};
...
\end{tikzpicture}
\]
Let $(b \xrightarrow{i} e, x)$ be an object of $(\A/U)_b$. Going left, we precompose with $h \maps a \to b$ to obtain the object $(a \xrightarrow{i \circ h} e, z)$ of $(\A/U)_a$. Then going down, we push forward along $k$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$e$};
...
\end{tikzpicture}
\]
to obtain the object $(c \xrightarrow{\psi} c+_a e, F(\psi)(x))$ of $(\A/U)_c$.
...
Having checked the hypotheses of \cref{lem:shulman}, we then define $F\lCsp$ to be $\Fr(\Phi)$.
\end{proof}
