...
if $\Phi\op \maps \C\op \to \A\op$ is an opfibration.   A functor $\Phi \maps \C \to \A$ is a \define{bifibration} if it is both a fibration and an opfibration.  If $\Phi \maps \C \to \A$ an opfibration we have for any morphism $f \maps a \to b$ in $\A$ a covariant reindexing functor
\[    f_! \maps \C_a \to \C_b \]
between the fibers of $\Phi$, as recalled in \cref{subsec:fibrations}.  Similarly, if $\Phi$ is a fibration we have a contravariant reindexing functor
...
\end{tikzpicture}
\]
the following square commutes up to a natural transformation:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\C_a$};
\node (B) at (1,0) {$\C_b$};
...
\end{tikzpicture}
\]
If $\theta$ is a natural isomorphism whenever the original square in $\A$ was
...
Suppose $\A$ has finite colimits and $\Phi \maps \C \to \A$ is a Beck--Chevalley bifibration.  Then there is a double category $\Fr(\Phi)$ in which
...
\begin{proof}[Proof of \cref{thm:decorated_cospans_1}] We apply \cref{lem:shulman} to the Beck-Chevalley monoidal bifibration $U_1 \colon (\A/U) \to \A$ which we now define. Let $(\A/U)$ be the comma category in which:
\begin{enumerate}
...
The functor $U_1$ is a fibration where given a morphism $f \colon a \to b$ in $\A$ with $(b,y,b \xrightarrow{i} U(y))$ over $b$ in $\int F$, the cartesian lifting of $f$ is given by $$(f,\id) \colon (a,y,a \xrightarrow{i \circ f} U(y)) \to (b,y,b \xrightarrow{i} U(y)).$$Denoting the cartesian lifting of a morphism $f$ by $\textnormal{cart}(f)$, the tensor product of $(\A/U)$ strictly preserves cartesian liftings in the sense that $\textnormal{cart}(f_1) \otimes \textnormal{cart}(f_2)=\textnormal{cart}(f_1+f_2)$.

The functor $U_1$ is also an opfibration where given a morphism $f \colon a \to b$ with $(a,x,a \xrightarrow{i} U(x))$ over $a$, the cocartesian lifting of $f$ is given by $$(f,\theta) \colon (a,x,a \to U(x)) \to (b,z,b \xrightarrow{\phi} U(z))$$where $\theta$ and $\phi$ arise from the following pushout square in $\A$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$b$};
...
\end{tikzpicture}
\]
Denoting the cocartesian lifting of a morphism $f$ by $\textnormal{cocart}(f)$, we have that $\textnormal{cocart}(f_1) \otimes \textnormal{cocart}(f_2) \cong \textnormal{cocart}(f_1+f_2)$ by the universal map between two colimits of the same diagram: $$(U(x_1)+_{a_1}b_1)+(U(x_2)+_{a_2}b_2) \cong U(x_1+x_2)+_{a_1+a_2}(b_1+b_2).$$
...
\end{tikzpicture}
\]
one can show that the following square commutes up to a natural isomorphism:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$(\A/U)_a$};
\node (B) at (2,0) {$(\A/U)_b$};
...
\end{tikzpicture}
\]
Let $(b,x,b \xrightarrow{i} U(x))$ be an object of $(\A/U)_b$. Going right, we precompose with $h \colon a \to b$ to obtain the object $(a,x,a \xrightarrow{i \circ h} U(x))$ in $(\A/U)_a$. Then, going down, we pushforward along $k \colon a \to c$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (2,0) {$U(x)$};
\node (C) at (0,-1) {$c$};
\node (D) at (2,-1) {$c+_{a}U(x)$};
\path[->,font=\scriptsize,>=angle 90]
...
\end{tikzpicture}
\]
to obtain the object $(c,z,c \to U(z))$ of $(\A/U)_c$ where $z \in \int F$ such that $U(z)=c+_a U(x)$. In particular, $z=L(c)+_{L(a)}x$ and as $U$ preserves finite colimits and $UL=1_\A$, we have that $$U(z)=U(L(c)+_{L(a)}x)=U(L(c))+_{U(L(a))}U(x)=c+_a U(x).$$ For the other route, first going down, we pushforward the object $(b,x,b \xrightarrow{i} U(x))$ along $g \colon b \to d$ by taking the following pushout:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$b$};
\node (B) at (2,0) {$U(x)$};
\node (C) at (0,-1) {$d$};
\node (D) at (2,-1) {$d+_{b}U(x)$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$i$} (B)
...
Now we seek a natural isomorphism between the two objects $(c,z,c \to c+_a U(x))$ and $(c,z',c \to d+_b U(x))$ in $(\A/U)_c$. It suffices to obtain a natural isomorphism between the two objects $z=L(c)+_{L(a)} x$ and $z'=L(d)+_{L(b)} x$ in $\X$. The following two squares are pushouts:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(a)$};
\node (B) at (2,0) {$x$};
...
\end{tikzpicture}
\]
the left of which exhibits $L(c)+_{L(a)}x$ as a colimit of the span $$L(c) \xleftarrow{L(k)} L(a) \xrightarrow{I \circ L(h)} x$$in $\X$. As $L$ preserves pushouts, the following square is also a pushout:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(a)$};
\node (B) at (1,0) {$L(b)$};
...
\end{tikzpicture}
\]
which we may then paste to the left of the above pushout square on the right to obtain another pushout square:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(b)$};
\node (B) at (2,0) {$x$};
...
\end{tikzpicture}
\]
which exhibits $L(d)+_{L(b)} x$ also as a colimit of the same span in $\X$, and hence the desired natural isomorphism between our two objects of $(\A/U)_c$ showing that $U_1$ satisfies the Beck-Chevalley condition.
\end{proof}
