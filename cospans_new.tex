...
Now we are ready to use Shulman's result to prove  \cref{thm:decorated_cospans_1} and  \cref{thm:decorated_cospans_2}.  An $F$-decorated cospan from $a \in \A$ to $b \in \A$
is a cospan 
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
...
\phi_{a,b}(d,d') )\]
where $\phi$ is the laxator for $F$.  The monoidal unit is $(0_\A \xrightarrow{!} 0_\A, x)$
where $x \in F(0_\A)$ is the element given by $\phi_0 \maps \one \to F(0_\A)$.   With this monoidal structure it is easy to see that $\Phi$ is strict monoidal.
...
\end{tikzpicture}
\]
to obtain the object $(c \xrightarrow{\psi} c+_a e, F(\psi)(x))$ of $(\A/U)_c$.  For the other route, first going down, we push the object $(b,x,b \xrightarrow{i} U(x))$ forward along $g \maps b \to d$ by taking the following pushout:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$b$};
\node (B) at (1.5,0) {$U(x)$};
...
\end{tikzpicture}
\]
is the following map of $F'$-decorated cospans $\lH(\alpha)$ in $F'\lCsp$:
\[
\begin{tikzpicture}[scale=1.5]
...
\end{tikzpicture}
\]
where the decoration morphism $\tau$ is mapped to a decoration morphism as follows:
\[
\begin{tikzpicture}[scale=1.5]
\node (G) at (-3.5,-0.5) {$1$};
\node (H) at (-2,0) {$F(m)$};
...
Given two categories $\A$ and $\A'$ with finite colimits, two lax monoidal pseudofunctors $F \maps \A \to \Cat$ and $F' \maps \A' \to \Cat$, a finite colimit preserving functor $H \maps \A \to \A'$, a lax monoidal pseudofunctor $E \maps \Cat \to \Cat$ and a natural isomorphism $\theta \maps EF \Rightarrow F' H$ as in the following diagram:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\A$};
\node (B) at (1,0) {$\Cat$};
...
\end{tikzpicture}
\] 
the triple $(H,E,\theta)$ induces a double functor $\lH \maps F\lCsp \to F'\lCsp$ as defined above.
\end{thm}

\begin{proof}   From the lax monoidal pseudofunctor $F \maps \A \to \Cat$, we obtain a Beck--Chevalley monoidal bifibration $\Phi \maps (\A/U) \to \A$ as in \cref{thm:decorated_cospans_1}.  We do the same for the lax monoidal pseudofunctor $F' \maps \A' \to \Cat$ to obtain a Beck--Chevalley monoidal bifibration $\Phi' \maps (\A'/U') \to \A'$.
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$(\A/U)$};
\node (B) at (2,0) {$(\A'/U')$};
...
\end{tikzpicture}
\]
Next we construct a strong monoidal functor $G \maps \A/U \to \A'/U'$  that makes the resulting square commute.  Such a commutating square is a morphism in Shulman's 2-category $\mathscr{MF}^{\textrm{fr}}$ of Beck--Chevalley monoidal bifibrations.  Appying Shulman's 2-functor $\mathbb{F}\mathbf{r} \maps \mathscr{MF}^{\textrm{fr}} \to \mathscr{F}r\mathscr{B}i$ to this morphism, \cite[Theorem 14.11]{Shulman2008}, we obtain the desired double functor
\[ \mathbb{H} \maps  F\lCsp \to F'\lCsp .\]  

We described $\A/U$ and its monoidal structure in the proof of \cref{thm:decorated_cospans_1}, 
and of course the case of $\A'/U'$ is analogous.   We define a strong monoidal functor $G \maps \A/U \to \A'/U'$ as follows.  First suppose we are given a object $(a \xrightarrow{i} b, x)$ in $(\A/U)$, where recall $x \in F(x)$.    Then we define
\[  G(a \xrightarrow{i} b, x) = (H(a) \xrightarrow{H(i)} H(b), x') \]
where the object $x' \in F'(H(b))$ is picked out by the composite
\[  1 \xrightarrow{\phi} E(1) \xrightarrow{E(p)} E(F(m)) \xrightarrow{\theta_m} F'(H(m)) \]
so that $U'(\theta_m E(p) \phi)=H(m)$, 
Given a pair of morphisms $(f,\tau)$ that make the following square commute:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,0) {$b$};
