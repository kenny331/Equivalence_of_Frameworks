...
We prove both these theorems using the work of Shulman \cite{Shulman2008}.  Shulman gives a general way to construct double categories from `Beck--Chevalley monoidal bifibrations'---a concept we explain in \cref{subsec:fibrations}.   
...
domain, an object of $\Fr(\Phi)$ simply amounts to a cospan in $\C$.  Thus, in this case $\Fr(\Phi)$ is the double category of cospans in $\C$.    We shall use variations on this idea to define double categories of decorated and structured cospans, and also prove that under certain conditions these double categories are equivalent.    

We start with decorated cospans, proving Theorem \ref{thm:decorated_cospans_1} and 
\ref{thm:decorated_cospans_2} by applying Shulman's result to a particular monoidal bifibration built from a lax monoidal pseudofunctor $F \maps (A,+) \to (\Cat,\times)$.    The Grothendieck construction gives a bijection between pseudofunctors $F \maps \A \to \Cat$ and opfibrations $U \maps \inta F \to \A$.  We need two refinements of this construction: one that gives monoidal opfibrations, and one that gives symmetric monoidal opfibrations.  We recall the Grothendieck construction and these related concepts in \cref{subsec:fibrations}; here we simply state the result we need.
...
Now we are ready to use Shulman's result to prove  \cref{thm:decorated_cospans_1} and  \cref{thm:decorated_cospans_2}.  An $F$-decorated cospan from $a$ to $b$ 
is a cospan 
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
...
\end{tikzpicture}
\] 
the triple $(H,E,\theta)$ induces a double functor $\lH \maps F\lCsp \to F'\lCsp$ defined as above.
\end{thm}

\begin{proof}   
From the lax monoidal pseudofunctor $F$ we obtain a Beck--Chevalley monoidal bifibration $\Phi \maps \A/U \to \A$ as in \cref{thm:decorated_cospans_1}.  Similarly the lax monoidal pseudofunctor $F'$ gives a Beck--Chevalley monoidal bifibration $\Phi' \maps \A'/U' \to \A'$.  Next we construct a strong monoidal functor $G \maps \A/U \to \A'/U'$  that makes this square commute:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\A/U$};
\node (B) at (1,0) {$\A'/U'$};
...
and of course the case of $\A'/U'$ is analogous.   We define $G \maps \A/U \to \A'/U'$ as follows.  First we define $G$ on any object $(a \xrightarrow{i} b, x)$ of $\A/U$ as follows:
\[  G(a \xrightarrow{i} b, x) \; = \; (H(a) \xrightarrow{H(i)} H(b), \underline{x}). \]
To define $\underline{x}$, we treat the object $x \in F(b)$ as a functor 
$x \maps 1 \to F(b)$, and let $\underline{x} \in F'(H(b))$ be given by this functor:
\[  1 \xrightarrow{\phi_0} E(1) \xrightarrow{E(x)} E(F(b)) \xrightarrow{\;\theta_b\;} F'(H(b)) .\]
Next we define $G$ on any morphism
\[   (f,g,h) \maps (a \xrightarrow{i} b, x) \; \to \; (a' \xrightarrow{i'} b', x') \]
in $\A/U$.   For this we set
\[   G(f,g,h) = (H(f),H(g),\underline{h}) .\]
To define $\underline{h}$, we treat $h \maps F(g)(x) \to x'$ as a natural transformation:
\[
\begin{tikzpicture}[scale=1.5]
\node (G) at (-3.5,-0.5) {$1$};
\node (H) at (-2,0) {$F(b)$};
...
\end{tikzpicture}
\]
and let $\underline{h} \maps F'(H(g))(\underline{x}) \to \underline{x}'$ be given by this
