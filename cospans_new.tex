...
We thank Daniel Cicala, Brendan Fong and Joe Moeller for helpful conversations.  We especially thank Michael Shulman for suggesting a better proof strategy that greatly improved this paper.  The third author would like to thank the General Secretariat for Research  and Technology (GSRT) and the Hellenic Foundation for Research and Innovation (HFRI).
...
{\chris There was a paragraph here (under (21) in old version) which I think is used in what follows, since that is the version of the theorem we actually use, the cocartesian total category one!}
...
\end{tikzpicture}
\]
{\chris I believe these should now be $\bot_a$ rather than $I_a$ although under these conditions those things are isomorphic I think! Since $F$ factors through Rex, each fibre has an initial object $\bot_a$ which can now be used.}

Here $I_a \in F(a)$ is the \define{trivial decoration} given by the composite
\[  \one\xrightarrow{\phi_0}F(0)\xrightarrow{F(!_a)}F(a), \]
while $! \maps F(i)(I_a) \to s$ is the unique morphism with this domain and
codomain (recall that $F(i)$ preserves initial objects), and similarly for
$! \maps F(o)(I_b) \to s$.   Finally, this isomorphism sends the map of decorated cospans:
\[
\begin{tikzpicture}[scale=1.5,baseline=(current bounding box.center)]
\node (A) at (0,0.5) {$a$};
\node (A') at (0,-0.5) {$a'$};
...
\begin{lem}
{\chris For $(F,\phi,\phi_0)\colon(\A,+)\to(\Cat,\times)$ a lax monoidal pseudofunctor, t}he double category $F\lCsp$ \textcolor{blue}{Kenny: of \cref{thm:decorated_cospans_1}}  is fibrant.
\end{lem}
...
\begin{displaymath}%\label{eq:compGrothcat}
