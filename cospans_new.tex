...
is the composite of their underlying cospans, shown in dashed arrows here:
\[
...
\end{tikzpicture}
\]
together with the decoration in $F(m +_b n)$ specified by this functor:
\[  \one\cong\one \times \one \xrightarrow{x \times y} F(m) \times F(n) \xrightarrow{\phi_{m,n}} F(m+n) \xrightarrow{F(\psi)} F(m+_{b}n). \]
