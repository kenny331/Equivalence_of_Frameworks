...
In what follows, we define a \define{cocartesian monoidal category} $(\A,+)$ to be a symmetric
monoidal category where the monoidal structure is given by chosen binary coproducts, with a chosen initial object serving as the unit for the monoidal structure.  Any category with finite coproducts can be made into a cocartesian monoidal category by arbitrarily choosing finite coproduct and a unit object.

%In all that follows, when we say a category `has finite colimits' we mean it is equipped with a choice of colimit for every finite diagram.   Thus, if $\A$ has coproducts it gives a cocartesian monoidal category $(\A,+)$: that is, a symmetric monoidal category where the monoidal structure is given by the chosen binary coproducts and initial object.   However, when we say a functor `preserves finite colimits', it need only do this up to canonical isomorphism, unless otherwise specified.   MAY NEED THIS LATER, BUT NOT HERE!!!

\begin{thm}\label{thm:decorated_cospans_1}
Let $(\A,+)$ be cocartesian monoidal and $F \maps (\A,+) \to (\Cat,\times)$ a lax monoidal pseudofunctor. Then there exists a double category $F\lCsp$ in which
\begin{itemize}
\item an object is an object of $\A$,
\item a vertical 1-morphism is a morphism of $\A$,
...
We prove both these theorems using the work of Shulman \cite{Shulman2008}, which we recall here.   First we need a few concepts.   Opfibrations are recalled in \cref{subsec:fibrations}, 
and fibrations are defined dually, with a functor $\Phi \maps \C \to \A$ being a fibration if and only
if $\Phi\op \maps \C\op \to \A\op$ is an opfibration.   A functor $\Phi \maps \C \to \A$ is a \define{bifibration} if it is both a fibration and an opfibration.      If $\C$ and $\A$ are monoidal categories, a \define{monoidal} bifibration $\Phi \colon \C \to \A$ is a bifibration that is also a strict monoidal functor.   Similarly, if $\C$ and $\A$ are symmetric monoidal, a \define{symmetric monoidal} bifibration is a bifibration $\Phi \colon \C \to \A$ that is a symmetric strict monoidal functor.
...
\end{tikzpicture}
\]
the following square commutes up to a specified natural transformation, built
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\C_a$};
\node (B) at (1.5,0) {$\C_b$};
...
a pushout, we say $\Phi$ is \define{Beck--Chevalley}.  (Shulman says `strongly co-Beck--Chevalley'.)
...
We apply \cref{lem:shulman} to a bifibration $\Phi \maps (\A/U) \to \A$ which we now define. Let $(\A/U)$ be the comma category in which:
\begin{enumerate}
\item{an object is a triple $(a,x,a \xrightarrow{i} U(x))$, and}
\item{a morphism from $(a,x,a \xrightarrow{i} U(x))$ to $(a',x',a' \xrightarrow{i'} U(x'))$ is a pair of morphisms $f \colon a \to a'$ and $\alpha \colon x \to x'$ such that the following square commutes:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$a'$};
...
(C) edge node[below]{$U(\alpha)$} (D);
\end{tikzpicture}
\]}
\end{enumerate}
The functor $\Phi$ is defined on objects by $\Phi(a,x,a \xrightarrow{i} U(x))=a$ and on morphisms by $\Phi(f,\alpha)=f$. We have a monoidal structure on the comma category $(\A/U)$ where the tensor product of two objects $(a_1,x_1,a_1 \xrightarrow{i_1} U(x_1))$ and $(a_2,x_2,a_2 \xrightarrow{i_2} U(x_2))$ is given by
\[  (a_1,x_1,a_1 \xrightarrow{i_1} U(x_1)) \otimes (a_2,x_2,a_2 \xrightarrow{i_2} U(x_2)) = (a_1+a_2,x_1+x_2,a_1+a_2 \xrightarrow{i_1+i_2} U(x_1+x_2)) \]
where we have left the natural isomorphism $U(x_1)+U(x_2) \cong U(x_1+x_2)$  implicit
 in the above composite, for reasons of space. The monoidal unit is given by $(0_\A,0_\X,0_\A \to U(0_\X))$. It is easy to see that the functor $U_1$ can be made into a strict monoidal functor using this tensor product.

The functor $\Phi$ is a fibration where given a morphism $f \maps a \to b$ in $\A$ with $(b,y,b \xrightarrow{i} U(y))$ over $b$ in $\int F$, the cartesian lifting of $f$ is given by 
\[ (f,\id) \maps (a,y,a \xrightarrow{i \circ f} U(y)) \to (b,y,b \xrightarrow{i} U(y)). \]
Denoting the cartesian lifting of a morphism $f$ by $\textnormal{cart}(f)$, the tensor product of $(\A/U)$ strictly preserves cartesian liftings in the sense that $\textnormal{cart}(f_1) \otimes \textnormal{cart}(f_2)=\textnormal{cart}(f_1+f_2)$.

The functor $\Phi$ is also an opfibration where given a morphism $f \maps a \to b$ with $(a,x,a \xrightarrow{i} U(x))$ over $a$, the cocartesian lifting of $f$ is given by 
\[ (f,\theta) \maps (a,x,a \to U(x)) \to (b,z,b \xrightarrow{\phi} U(z)) \]
where $\theta$ and $\phi$ arise from the following pushout square in $\A$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$b$};
\node (C) at (0,-1) {$U(x)$};
\node (D) at (1.5,-1) {$U(x)+_a b$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$f$} (B)
(A) edge node[left]{$i$} (C)
(B) edge [dashed] node[right]{$\phi$} (D)
(C) edge [dashed] node[above]{$\theta$} (D);
\end{tikzpicture}
\]
and the object $z$ arises from the cocartesian lifting of $\theta$ through $U$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (2.5,0) {$c$};
...
\[  (U(x_1)+_{a_1}b_1)+(U(x_2)+_{a_2}b_2) \cong U(x_1+x_2)+_{a_1+a_2}(b_1+b_2). \]

Lastly, the monoidal bifibration $U_1$ satisfies the Beck--Chevalley condition. Given a pushout square in $\A$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$b$};
...
\end{tikzpicture}
\]
Let $(b,x,b \xrightarrow{i} U(x))$ be an object of $(\A/U)_b$. Going left, we precompose with $h \colon a \to b$ to obtain the object $(a,x,a \xrightarrow{i \circ h} U(x))$ of $(\A/U)_a$. Then going down, we push forward along $k$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$U(x)$};
...
\end{tikzpicture}
\]
to obtain the object $(c,z,c \xrightarrow{\psi} U(z))$ of $(\A/U)_c$, where $z \in \int F$ is given by $z=\bot_c+_{\bot_a}x$. Explicitly, $z$ is given by
\[ 1 \xrightarrow{x} F(U(x)) \xrightarrow{F(\zeta)} F(0+_0 U(x)) \xrightarrow{F(!_c +_{!_a} 1)} F(c+_a U(x)) \]
...
{\john Need to refer to a proposition where we show $U$ preserves pushouts strictly.}
 For the other route, first going down, we push the object $(b,x,b \xrightarrow{i} U(x))$ forward along $g \colon b \to d$ by taking the following pushout:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$b$};
\node (B) at (1.5,0) {$U(x)$};
...
\end{tikzpicture}
\]
which yields the object $(d,z',d \xrightarrow{\theta} U(z'))$ of $(\A/U)_d$ where $z' \in \int F$ is given by $z'=\bot_d+_{\bot_b}x$. We then precompose with $f \colon c \to d$ to obtain the object $(c,z',c \xrightarrow{\theta \circ f} U(z'))$ of $(\A/U)_c$.

Now we seek a natural isomorphism between the two objects $(c,z,c \xrightarrow{\psi} c+_a U(x))$ and $(c,z',c \xrightarrow{\theta \circ f} d+_b U(x))$ in $(\A/U)_c$. It suffices to obtain a natural isomorphism between the two objects $z=\bot_c+_{\bot_a} x$ and $z'=\bot_d+_{\bot_b} x$ in $\X$. This pushout square:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$U(x)$};
...
which exhibits $d+_b U(x)$ as a pushout of the same span in $\A$.  The resulting natural isomorphism $\kappa$ between $c+_a U(x)$ and $d+_b U(x)$ then extends to a natural isomorphism between $z=\bot_c +_{\bot_a} x$ and $z'=\bot_d +_{\bot_b} x$ in $\X$:
\[
\begin{tikzpicture}[scale=1.5]
\node () at (5,0) {$\cong$};
\node (A) at (0,0) {$1$};
...
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[fill=white]{$x$} (A1)
(A) edge node[fill=white]{$x$} (B1)
...
\end{tikzpicture}
\]
and consequently a natural isomorphism between our two objects of $(\A/U)_c$, showing that $U_1$ satisfies the Beck--Chevalley condition.
\end{proof}
...
From the symmetric lax monoidal pseudofunctor $F \colon \A \to \Cat$, we can obtain a Beck-Chevalley symmetric monoidal opfibration $\Phi \colon (\A/U) \to \A$ as in \cref{thm:decorated_cospans_1}, where $U \colon \int F \to \A$ is the associated monoidal opfibration of the pseudofunctor $F$. We can do the same for the other symmetric lax monoidal pseudofunctor $F' \colon \A' \to \Cat$ to obtain another Beck-Chevalley symmetric monoidal opfibration $\Phi' \colon (\A'/U') \to \A'$.
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$(\A/U)$};
\node (B) at (2,0) {$(\A'/U')$};
...
\end{tikzpicture}
\]
We construct a strong monoidal functor $G \colon (\A/U) \to (\A'/U')$ between the monoidal comma categories $(\A/U)$ and $(\A'/U')$ that will make the induced square commute. Such a commutative square is a morphism in the 2-category $\mathbf{FMF}$ of monoidal bifibrations satisfying the Beck-Chevalley condition which then by a 2-functor $\mathbf{Fr} \colon \mathbf{FMF} \to \mathbf{FDbl}$ due to Shulman \cite[Theorem 14.11]{Shulman2008} will yield a map $$\mathbf{Fr}(G,H) \colon \mathbf{Fr}(\Phi) \to \mathbf{Fr}(\Phi')$$between the fibrant symmetric monoidal double categories $\mathbf{Fr}(\Phi)=F\lCsp$ and $\mathbf{Fr}(\Phi')= F'\lCsp$.

The monoidal comma category $(\A/U)$ has:
\begin{enumerate}
\item{objects as triples $(a,x,a \to U(x))$, and}
\item{a morphism from $(a,x,a \to U(x))$ to $(b,y,b \to U(y))$ is a pair of morphisms $f \colon a \to b$ and $\tau \colon x \to y$ that make the following square commute:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,0) {$b$};
...
\end{enumerate}
The tensor product of two objects $(a_1,x_1,a_1 \to U(x_1))$ and $(a_2,x_2,a_2 \to U(x_2))$ is given by $$(a_1,x_1,a_1 \to U(x_1)) \otimes (a_2,x_2,a_2 \to U(x_2)) = (a_1+a_2,x_1+x_2,a_1+a_2 \to U(x_1+x_2)).$$The monoidal unit of $(\A/U)$ is given by $(0_\A,0_\X,0_\A \to U(0_\X))$. Those of $(\A'/U')$ are analogous.

On objects, the strong monoidal functor $F \colon (\A/U) \to (\A'/U')$ is defined as follows:
\begin{enumerate}
\item{given $(a,x,a \to U(x))$ in $(\A/U)$, $$G(a,x,a \to U(x)) = (H(a),\theta_m E(x) \phi,H(a) \to U'(\theta_m E(x) \phi))$$where if $1 \xrightarrow{x} F(m)$, then $U(x)=m$ and $\theta_m E(x) \phi$ is given by 
$$1 \xrightarrow{\phi} E(1) \xrightarrow{E(x)} E(F(m)) \xrightarrow{\theta_m} F'(H(m))$$
so that $U'(\theta_m E(x) \phi)=H(m)$, }
...
\node (A) at (0,0) {$H(a)$};
\node (B) at (3,0) {$H(b)$};
\node (C) at (0,-1) {$U'(\theta_m E(x) \phi)$};
\node (D) at (3,-1) {$U'(\theta_n E(y) \phi))$};
\path[->,font=\scriptsize,>=angle 90]
...
(A) edge node [above] {$\phi$} (L)
(G) edge node [above] {$x$} (H)
(G) edge node [below] {$y$} (I)
(H) edge node [right] {$F(h)$} (I)
(B) edge node [above] {$\theta_m$} (E)
(D) edge node [above] {$\theta_n$} (F)
(E) edge node [right] {$F'(H(h))$} (F)
(L) edge node[above]{$E(x)$} (B)
(L) edge node[below]{$E(y)$} (D)
(B) edge node[right]{$E(F(h))$} (D);
\end{tikzpicture}
\]
}
\end{enumerate}
The laxator is given by the following natural isomorphism between $\theta_m E(x) \phi + \theta_n E(y) \phi$ and $\theta_{m+n} E(x+y) \phi$:
\[
\begin{tikzpicture}[scale=1.5]
\hspace{-.6in}
\node () at (3,-0.5) {$\scriptscriptstyle{\stackrel{E_{x_1,x_2}} \cong}$};
...
(B) edge node [above] {$E(x) \times E(y)$} (E)
(D) edge node [above] {$E(x \times y)$} (F)
...
(A2) edge node [above] {$\scriptscriptstyle{E(F(!_{0_\A}))}$} (A3)
...
The proof is analogous to the proof of \cref{thm:decorated_cospans_1} in that we apply \cref{lem:shulman} to the Beck-Chevalley monoidal bifibration $U_2 \colon (L/\X) \to \A$ which we now define. Let $(L/\X)$ be the comma category in which:
\begin{enumerate}
\item{an object is a triple $(a,x,L(a) \xrightarrow{I} x)$, and}
\item{a morphism from $(a,x,L(a) \xrightarrow{I} x)$ to $(a',x',L(a') \xrightarrow{I'} x')$ is a pair of morphisms $f \colon a \to a'$ and $\alpha \colon x \to x'$ such that the following square commutes:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(a)$};
\node (B) at (1,0) {$L(a')$};
...
\]}
\end{enumerate}
The functor $U_2$ is defined on objects by $U_2(a,x,L(a) \xrightarrow{I} x)=a$ and on morphisms by $U_2(f,\alpha)=f$. The monoidal structure of $(L/\X)$ is similar to the monoidal structure of $(\A/U)$ of \cref{thm:decorated_cospans_1} in that the tensor product of two objects $(a_1,x_1,L(a_1) \xrightarrow{I_1} x_1)$ and $(a_2,x_2,L(a_2) \xrightarrow{I_2} x_2)$ is given by 
...
where again the natural isomorphism $L(a_1)+L(a_2) \cong L(a_1+a_2)$ is implicit. The monoidal unit is ${(0_\A,0_\X,{L(0_\A) \to 0_\X})}$, and it is clear that $U_2$ is a strict monoidal functor using this tensor product.

The functor $U_2$ is both a fibration and an opfibration. Given a morphism $f \colon a \to b$ in $\A$ with ${(b,x,L(b) \xrightarrow{I} x)}$ over $b$ in $\X$, the cartesian lifting of $f$ is given by
$$(f,\id_x) \colon (a,x,L(a) \xrightarrow{I \circ L(f)} x) \to (b,x,L(b) \xrightarrow{I} x).$$
With $(a,x,L(a) \xrightarrow{I} x)$ over $a$ in $\X$, the cocartesian lifting of $f$ is given by
$$(f,\kappa) \colon (a,x,L(a) \xrightarrow{I} x) \to (b,x+_{L(a)} L(b),L(b) \xrightarrow{\theta} x+_{L(a)} L(b))$$
...
Lastly, we show that $U_2 \colon (L/ \X) \to \A$ satisfies the Beck-Chevalley condition. Given a pushout square in $\A$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,0) {$b$};
...
\end{tikzpicture}
\]
Let $(b,x,L(b) \xrightarrow{I} x)$ be an object of $(L/\X)_b$. Going right, we precompose with $L(h) \colon L(a) \to L(b)$ to obtain the object $(a,x,L(a) \xrightarrow{I \circ L(h)} x)$ in $(L/\X)_a$. Then, going down, we pushforward along $L(k) \colon L(a) \to L(c)$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(a)$};
\node (B) at (2,0) {$x$};
...
\end{tikzpicture}
\]
to obtain the object $(c,L(c)+_{L(a)} x,L(c) \xrightarrow{\psi} L(c)+_{L(a)} x)$ in $(L/ \X)_c$. For the other route, first going down, we pushforward the object $(b,x,L(b) \xrightarrow{I} x)$ along $L(g) \colon L(b) \to L(d)$ by taking the following pushout:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(b)$};
\node (B) at (2,0) {$x$};
...
\end{tikzpicture}
\]
which yields the object $(d,L(d)+_{L(b)}x,L(d) \xrightarrow{\theta} L(d)+_{L(b)} x)$ of $(L/\X)_d$. We then precompose with $L(f) \colon L(c) \to L(d)$ to obtain the object $(c,L(d)+_{L(b)} x,L(c) \xrightarrow{\theta \circ L(f)} L(d)+_{L(b)} x)$ of $(L/\X)_c$.
...
\end{tikzpicture}
\]
which exhibits $L(d)+_{L(b)} x$ also as a colimit of the same span in $\X$, and hence the desired natural isomorphism between our two objects of $(L /\X)_c$. Thus $U_2$ satisfies the Beck-Chevalley condition.
\end{proof}
...
\end{enumerate}
and $\mathbf{FDbl}$ is the 2-category of fibrant double categories, double functors and double transformations. Given two monoidal bifibrations $\Phi \colon \A \to \mathsf{B}$ and $\Phi' \colon \A' \to \mathsf{B}'$, a \define{strong monoidal morphism of bifibrations} from $\Phi$ to $\Phi'$ consists of a pair of strong monoidal functors $F_1$ and $F_2$ making the following square commute:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\A$};
\node (B) at (1,0) {$\A'$};
...
\end{tikzpicture}
\]
and such that the identity $\Phi' F_1 = F_2 \Phi$ is a monoidal transformation. The two monoidal fibrations in question are the functors $U_1$ and $U_2$ of Theorems \ref{thm:decorated_cospans_2} and \ref{thm:SC}, respectively, which share $\A$ as a common base, and thus we only need a single strong monoidal functor $F \colon (\A/U) \to (L/\X)$ making the following diagram commute:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$(\A/U)$};
\node (B) at (2,0) {$(L/ \inta F)$};
\node (C) at (1,-1.5) {$\A$};
%\node (D) at (2,-1) {$\A$};
...
(A) edge node[left]{$U_1$} (C)
(B) edge node[right]{$U_2$} (C);
%(C) edge [bend left] node[above]{$G_1$} (C)
%(C) edge [bend right] node[above]{$G_2$} (C);
\end{tikzpicture}
\]
with $U_2 F=U_1$ a monoidal transformation. The functor $F \colon (\A/U) \to (L/\inta F)$ is expressed by the natural isomorphism of an adjunction, namely:
$$F(a,x,a \to U(x))=(a,x,L(a) \to x)$$
and is the identity on pairs of morphisms making the required squares commute. It is clear that $F$ is strong monoidal via component-wise binary coproducts which are preserved by both $L$ and $U$, and that the identity $U_2 F=U_1$ is a monoidal transformation. As $F$ is an isomorphism, we obtain an isomorphism $$\mathbf{Fr}(F) \colon \mathbf{Fr}(U_1) \to \mathbf{Fr}(U_2)$$ between the two symmetric monoidal double categories $\mathbb{F}\mathbf{r}(U_1)=F\lCsp$ and $\mathbf{F}\mathbf{r}(U_2)= _L \lCsp(\inta F)$ of Theorems \ref{thm:decorated_cospans_2} and \ref{thm:SC}, respectively.
\end{proof}
...
A \define{monoidal double category} is a pseudomonoid in $\Dbl$, namely it is equipped with double functors $\otimes\colon\lD\times\lD\to\lD$, $I\colon\one\to\lD$ and invertible double transformations $\otimes\circ(1\times\otimes)\cong\otimes\circ(\otimes\times1)$, $\otimes\circ(1\times I)\cong1\cong\otimes\circ(I\times1)$ satisfying standard axioms.
\end{defn} 
\noindent
