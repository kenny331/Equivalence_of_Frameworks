...
\end{tikzpicture}
\]
We construct a strong monoidal functor $F \colon (\A/U) \to (\A'/U')$ between the monoidal comma categories $(\A/U)$ and $(\A'/U')$ that will make the induced square commute. Such a commutative square is a morphism in the 2-category $\mathbf{FMF}$ of monoidal bifibrations satisfying the Beck-Chevalley condition which then by a 2-functor $\mathbf{Fr} \colon \mathbf{FMF} \to \mathbf{FDbl}$ due to Shulman \cite[Theorem 14.11]{Shulman2008} will yield a map $$\mathbf{Fr}(F,H) \colon \mathbf{Fr}(\Phi) \to \mathbf{Fr}(\Phi')$$between the fibrant symmetric monoidal double categories $\mathbf{Fr}(\Phi)=F\lCsp$ and $\mathbf{Fr}(\Phi')= F'\lCsp$.
...
\begin{enumerate}
\item{given $(a,x,a \to U(x))$ in $(\A/U)$, $$F(a,x,a \to U(x)) = (H(a),\theta_m E(x) \phi,H(a) \to U'(\theta_m E(x) \phi))$$where if $1 \xrightarrow{x} F(m)$, then $U(x)=m$ and $\theta_m E(x) \phi$ is given by 
...
\end{tikzpicture}
\]
we define $$F(f,\tau)=(H(f),\theta_h E(\tau) \phi).$$
...
\end{enumerate}
One can show that the induced laxator and unit morphism given by these prescribed tensor products satisfy the four required coherence laws of a symmetric strong monoidal functor.
\end{proof}
