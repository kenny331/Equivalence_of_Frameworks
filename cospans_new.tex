...
\[  \one\cong\one \times \one \xrightarrow{s \, \times \, t} F(n) \times F(m) \xrightarrow{\phi_{n,m}} F(m+n) \xrightarrow{F(\psi)} F(m +_{b}n). \]
...
\item of two horizontal 1-cells $(a \to m \leftarrow b, s \in F(m))$ and $(c \to n \leftarrow d, t \in F(n))$ is
\[
\begin{tikzcd}
& m+n & \\
...
domain, an object of $\Fr(\Phi)$ simply amounts to a cospan in $\A$.  In this case $\Fr(\Phi)$ is the double category of cospans in $\A$.    We shall use variations on this idea to construct double categories of decorated and structured cospans, and to prove that under certain conditions these double categories are equivalent.    We begin with decorated cospans, proving Theorem \ref{thm:decorated_cospans_1} and \ref{thm:decorated_cospans_2} by applying Shulman's result to a particular monoidal bifibration built from a lax monoidal pseudofunctor $F \maps (A,+) \to (\Cat,\times)$.   
...
Denoting the cartesian lifting of $f$ to an object $z \in \A/U$ by $\mathrm{Cart}(f,z)$, the tensor product of $\A/U$ strictly preserves such cartesian liftings in the sense that $\mathrm{Cart}(f_1,z_1) \otimes \mathrm{Cart}(f_2,z_2)=\mathrm{Cart}(f_1 \otimes f_2, z_1 \otimes z_2)$.  
...
in $\A/U$ and a morphism $k \maps a \to a''$ in $\A$ such that $\Phi(f'',g'',h'') = k f$.   To show that $(f,g,1)$ is a cocartesian lifting of $f$, we need to show there exists a unique morphism
...
We now compare structured and decorated cospans.   In \cref{thm:decorated_cospans_1} we built a double category of decorated cospans from a functor $\Phi \maps \A/U \to \A$.    In \cref{thm:structured_cospans} we built a double category of structured cospans from a functor $\Psi \maps L/\X \to \A$.   We now show that under certain conditions $L$ is left adjoint to $U$.   Whenever this happens, $\A/U$ is isomorphic to $L/\X$, by a general fact about arrow categories.    The bifibrations $\Phi$ and $\Psi$ are then also isomorphic.   By the functoriality
of Shulman's construction in \cref{lem:shulman}, it follows that the double category of decorated cospans is isomorphic to the double category of structured cospans.
...
\[    \one \xrightarrow{\phi_0} F(0_\A) \xrightarrow{F(!)} F(b) , \]
