...
Given two categories $\A$ and $\A'$ with finite colimits, two lax monoidal pseudofunctors $F, F' \maps (\A',+) \to (\Cat,\times)$, a finite colimit preserving functor $H \maps \A \to \A'$, a lax monoidal pseudofunctor $E \maps (\Cat,\times) \to (\Cat,\times)$ and a {\john monoidal???} natural isomorphism $\theta$ as in the following diagram:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\A$};
\node (B) at (1,0) {$\Cat$};
...
and of course the case of $\A'/U'$ is analogous.   We define the strong monoidal functor $G \maps \A/U \to \A'/U'$ as follows.  First we define $G$ on an object $(a \xrightarrow{i} b, x)$ of $\A/U$.
We can treat $x \in F(b)$ as a morphism $x \maps 1 \to F(b)$.    Then we define
...
Then we define $G$ on a morphism
\[   (f,g,h) \maps (a \xrightarrow{i} b, x) \; \to \; (a' \xrightarrow{i'} b', x') \]
