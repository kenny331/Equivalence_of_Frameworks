...
We apply \cref{lem:shulman} to a bifibration $\Phi \maps (\A/U) \to \A$ which we now define. The functor $U \colon \inta F \to \A$ is the associated monoidal opfibration of the symmetric lax monoidal pseudofunctor $F \colon \A \to \Cat$. Let $(\A/U)$ be the comma category in which:
\begin{enumerate}
\item{an object is a triple $(a,x,a \xrightarrow{i} U(x))$ with $a \in \A$ and $x=(a',p) \in \A \times F(a')$, and}
\item{a morphism from $(a,x,a \xrightarrow{i} U(x))$ to $(b,x',b \xrightarrow{i'} U(x'))$ is a pair of morphisms $f \colon a \to b$ and $\alpha \colon x \to x'$ such that the following square commutes:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$b$};
...
where we have left the equality $U(x_1)+U(x_2) = U(x_1 \otimes x_2)$ implicit in the above composite, for reasons of space. The monoidal unit is given by $(0_\A,0_\X,0_\A \to U(0_\X))$. It is easy to see that the functor $\Phi$ can be made into a strict monoidal functor using this tensor product.
...
\end{tikzpicture}
\]
Let $(b,x,b \xrightarrow{i} U(x))$ be an object of $(\A/U)_b$ where $x=(b',p) \in \A \times F(b')$ so that $U(x)=b'$. Going left, we precompose with $h \colon a \to b$ to obtain the object $(a,x,a \xrightarrow{i \circ h} U(x))$ of $(\A/U)_a$. Then going down, we push forward along $k$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$U(x)$};
...
We then precompose with $f \colon c \to d$ to obtain the object $(c,z',c \xrightarrow{\theta \circ f} U(z'))$ of $(\A/U)_c$.
...
\end{tikzpicture}
\]
We construct a strong monoidal functor $G \colon (\A/U) \to (\A'/U')$ between the monoidal comma categories $(\A/U)$ and $(\A'/U')$ that will make the induced square commute. Such a commutative square is a morphism in the 2-category $\mathbf{FMF}$ of monoidal bifibrations satisfying the Beck-Chevalley condition which then, by a 2-functor $\mathbb{F}\mathbf{r} \colon \mathbf{FMF} \to \mathbf{FDbl}$ due to Shulman \cite[Theorem 14.11]{Shulman2008}, will yield a map $$\mathbb{F}\mathbf{r}(G,H) \colon \mathbb{F}\mathbf{r}(\Phi) \to \mathbb{F}\mathbf{r}(\Phi')$$between the fibrant symmetric monoidal double categories $\mathbb{F}\mathbf{r}(\Phi)=F\lCsp$ and $\mathbb{F}\mathbf{r}(\Phi')= F'\lCsp$.
...
The proof is analogous to the proof of \cref{thm:decorated_cospans_1} in that we apply \cref{lem:shulman} to the Beck-Chevalley symmetric monoidal bifibration $\Psi \colon (L/\X) \to \A$ which we now define. Let $(L/\X)$ be the comma category in which:
\begin{enumerate}
...
The functor $\Psi$ is both a fibration and an opfibration. Given a morphism $f \colon a \to b$ in $\A$ with ${(b,x,L(b) \xrightarrow{I} x)}$ in $\X$ over $b$, the cartesian lifting of $f$ is given by
...
Lastly, we show that $\Psi \colon (L/ \X) \to \A$ satisfies the Beck-Chevalley condition. Given a pushout square in $\A$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1,0) {$b$};
...
\end{tikzpicture}
\]
with $\Psi F=\Phi$ a monoidal transformation. The functor $F \colon (\A/U) \to (L/\X)$ is expressed by the natural isomorphism of an adjunction, namely:
$$F(a,x,a \to U(x))=(a,x,L(a) \to x)$$
and is the identity on pairs of morphisms making the required squares commute. It is clear that $F$ is strong monoidal via component-wise binary coproducts which are preserved by both $L$ and $U$, and that the identity $\Psi F=\Phi$ is a monoidal transformation. As $F$ is an isomorphism, we obtain an isomorphism $$\mathbb{F}\mathbf{r}(F) \colon \mathbb{F}\mathbf{r}(\Phi) \to \mathbb{F}\mathbf{r}(\Psi)$$ between the two symmetric monoidal double categories $\mathbb{F}\mathbf{r}(\Phi)=F\lCsp$ and $\mathbb{F}\mathbf{r}(\Psi)= _L \lCsp(\inta F)$ of Theorems \ref{thm:decorated_cospans_2} and \ref{thm:SC}, respectively.
\end{proof}
