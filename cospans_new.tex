...
\[  (a,L(a) \xrightarrow{i} x) + (a',L(a') \xrightarrow{i'} x')=(a+a',x+x',L(a+a')  \xrightarrow{\sim} L(a)+L(a') \xrightarrow{i+i'} x+x'). \]
The monoidal unit is ${(0_\A,0_\X,{L(0_\A) \to 0_\X})}$.  It is clear that $\Psi \maps (L/\X, +) \to (\A,+)$ is a strict monoidal functor.  
...
The `corresponding pseudofunctor'   $F \maps \A \to \SMC$  comes from \cref{lem:MonGroth}.
