...
\end{tikzpicture}
\] 
we obtain a double functor $\lH \maps F\lCsp \to F'\lCsp$ defined as above.
\end{thm}
...
To make $G$ into a strong monoidal functor, we need to equip it with a natural isomorphism
\[  \gamma \maps G(a \xrightarrow{i} b, x) \otimes G(a' \xrightarrow{i'} b', x') \; 
\xrightarrow{\sim} \; G((a \xrightarrow{i} b, x) \otimes (a' \xrightarrow{i'} b', x')).\]
Note that
...
\]
where $\phi$ is the laxator for $F$.  Since $H$ preserves finite colimits, the isomorphism
$\gamma$ is obvious except 
