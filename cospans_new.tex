...
$U \maps \inta F \to \A$.   Moeller and the third author \cite{MV} showed that
...
To obtain the desired double category we apply \cref{lem:shulman} to a bifibration $\Phi \maps \A/U \to \A$ which we now define.   Let $U \maps \inta F \to \A$ be the monoidal opfibration associated to the lax monoidal pseudofunctor $F \maps \A \to \Cat$.   Thus:
\begin{itemize}
\item an object of $\inta F$ is a pair $(b,p)$ where $b \in \A$ and $p \in F(A)$;
...
a morphism in $F(b')$.
\end{itemize}
The functor $U$ maps objects and morphisms to their first component.   We thus obtain an explicit description of the comma category $\A/U$:
\begin{itemize}
...
\]
\end{itemize}
The functor $\Phi \maps \A/U \to \A$ is defined on objects by $\Phi(a,x,a \xrightarrow{i} U(x))=a$ and on morphisms by $\Phi(f,\alpha)=f$. We have a monoidal structure on $\A/U$ where the tensor product of two objects $(a_1,x_1,a_1 \xrightarrow{i_1} U(x_1))$ and $(a_2,x_2,a_2 \xrightarrow{i_2} U(x_2))$ is given by
...
One can also check that the functor $\Phi$ is also opfibration, but this takes a bit more work.  Given a morphism $f \maps a \to a'$ in $\A$ and an object $(a,x,a \xrightarrow{i} U(x)) \in \A/U$ over $a$, we shall show that a cocartesian lifting of $f$ is given by 
...
given in the above diagram and $\id \maps F(f)(p) \to p'$ is the identity.
