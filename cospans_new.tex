...
\[ \left(a\to m\leftarrow b,x\in F(m)\right), \qquad \left(b\to n\leftarrow c,y\in F(n)\right)\] 
...
\end{tikzpicture}
\]
together with the decoration in $y \odot x \in F(m +_b n)$ specified by this functor:
\[  \one\cong\one \times \one \xrightarrow{x \times y} F(n) \times F(m) \xrightarrow{\phi_{n,m}} F(m+n) \xrightarrow{F(\psi)} F(m +_{b}n). \]
...
\end{tikzpicture}
\]
together with the decoration morphism $\tau_{\beta \odot \alpha} \maps F(h_1 +_g h_2)(y \odot x) \to (y' \odot x')$ specified by this natural transformation:
\[
...
\end{tikzpicture}
\]
where the middle isomorphism is \cref{eq:pseudonaturality} from pseudonaturality of $\phi$ and the right-hand isomorphism comes from the pseudofunctoriality of $F$.
...
We prove both these theorems using the work of Shulman \cite{Shulman2008}, who gives a general way to construct double categories from `Beck--Chevalley monoidal bifibrations'---a concept we explain in \cref{subsec:fibrations}.   
...
Note that when $\C$ is the arrow category of $\A$ and $\Phi$ maps any arrow in $\A$ to its
domain, an object of $\Fr(\Phi)$ simply amounts to a cospan in $\C$.  In this case $\Fr(\Phi)$ is the double category of cospans in $\C$.    We shall use variations on this idea to construct double categories of decorated and structured cospans, and to prove that under certain conditions these double categories are equivalent.    Here we consider decorated cospans, proving Theorem \ref{thm:decorated_cospans_1} and \ref{thm:decorated_cospans_2} by applying Shulman's result to a particular monoidal bifibration built from a lax monoidal pseudofunctor $F \maps (A,+) \to (\Cat,\times)$.   

 The Grothendieck construction gives a bijection between pseudofunctors $F \maps \A \to \Cat$ and opfibrations $U \maps \inta F \to \A$.  We need two refinements of this construction: one that gives monoidal opfibrations, and one that gives symmetric monoidal opfibrations.  We recall these ideas in \cref{subsec:fibrations}; here we simply state the result we need.

\begin{lem}
\label{lem:MonGroth}
There is a 2-equivalence between the 2-categories of monoidal opfibrations and monoidal opindexed categories with a given base, and if the base is cocartesian monoidal, there is a 2-equivalence between these and pseudofunctors into $\MonCat$.   Similarly there is a 2-equivalence between symmetric monoidal opfibrations and symmetric monoidal opindexed categories, and if the base is cocartesian monoidal, also a 2-equivalence between these and pseudofunctors into $\SMC$.   
\end{lem}
...
is a cospan $a \to m \leftarrow b$ in $\A$ together with an object $x \in F(m)$.   This can be seen as a pair $(e \to m, x \in F(m))$ such that $e$ equals $a + b$.   But $(m,x \in F(m))$ is precisely an object of the Grothendieck category $\inta F$, and the opfibration $U \maps \inta F \to \A$ sends this object to $m$.  Thus, $(e \to m, x \in F(m))$ can be seen as an object of the comma category $\A/U$.   Let $\Phi \maps \A/U \to \A$ be the functor sending any object $(e \to m,x \in F(m)) \in \A/U$ to $e$.  Then an $F$-decorated cospan from $a$ to $b$ amounts to an object $c \in \A/U$ such that $\Phi(c) = a + b$.    With this choice of $\Phi$, Shulman's result gives the double category of $F$-decorated cospans.

\begin{proof}[Proof of \cref{thm:decorated_cospans_1}] 
Let $U \maps \inta F \to \A$ be the monoidal opfibration associated to the lax monoidal pseudofunctor $F \maps (\A,+) \to (\Cat, \times)$.  To obtain the desired double category we apply Shulman's \cref{lem:shulman} to the functor $\Phi \maps \A/U \to \A$, which we now describe in more detail.  In the comma category $\A/U$:
\begin{itemize}
\item an object is a pair $(a \xrightarrow{i} b, x)$ consisting of a morphism in $\A$ and an
object $x \in F(b)$;
\item a morphism from $(a \xrightarrow{i} b, x)$ to $(a' \xrightarrow{i'} b', x')$ is a triple $(f,g,h)$ where $f \maps a \to a'$ and $g \maps b \to b'$ are morphisms in $\A$ such that this square commutes:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$a'$};
...
\end{tikzpicture}
\]
and $h \maps F(g)(x) \to x'$ is a morphism in $F(b')$.
\end{itemize}
As in any comma category we have a functor $\Phi \maps \A/U \to \A$; this is given on objects by $\Phi(a \xrightarrow{i} b, x)=a$ and on morphisms by $\Phi(f,g,h)=f$. 
...
\[  (a \xrightarrow{i} b, x) \otimes (a' \xrightarrow{i'} b', x') = (a+a' \xrightarrow{i+i'} b+b', 
\phi_{b,b'}(x,x') )\]
where $\phi$ is the laxator for $F$.  The monoidal unit is $(0_\A \xrightarrow{!} 0_\A, 0)$
where $0 \in F(0_\A)$ is the object given by $\phi_0 \maps \one \to F(0_\A)$.   With this monoidal structure it is easy to see that $\Phi$ is strict monoidal.

Next, one can check that $\Phi$ is a fibration.  Given a morphism $f \maps a \to a'$ in $\A$ and an object $(a' \xrightarrow{i'} b', x') \in \A/U$ over $a'$, a cartesian lifting of $f$ to this object is given by 
\[ (f,1,1) \maps (a \xrightarrow{i' f} b' , x')\; \to \; (a' \xrightarrow{i'} b', x'). \]
Denoting the cartesian lifting of $f$ to the object $z \in \A/U$ by $\mathrm{Cart}(f,z)$, the tensor product of $\A/U$ strictly preserves such cartesian liftings in the sense that $\mathrm{Cart}(f_1,z_1) \otimes \mathrm{Cart}(f_2,z_2)=\mathrm{Cart}(f_1 \otimes f_2, z_1 \otimes z_2)$.  

One can also check that $\Phi$ is an opfibration. Given a morphism $f \maps a \to a'$ in $\A$ and an object $(a \xrightarrow{i}b, x) \in \A/U$ over $a$, we claim that a cocartesian lifting of $f$ is given by 
\[ (f,g,1) \maps (a \xrightarrow{i} b, x) \, \to \, (a' \xrightarrow{i'} b+_a a', F(g)(x)). \]
...
\[    (f'',g'',h'') \maps (a \xrightarrow{i} b, x) \; \to \; (a'' \xrightarrow{i''} b'', x'')   \]
in $\A/U$ and a morphism $k \maps a \to a''$ in $\A$ such that $\Phi(f'',g'',h'') = k \circ f$.   To show that $(f,g,1)$ is a cocartesian lifting of $f$, we need to show there exists a unique morphism
\[     (\tilde{f}, \tilde{g}, \tilde{h}) \maps (a' \xrightarrow{i'} b +_a a', F(g)(x)) \; \to \; (a'' \xrightarrow{i''} b'', x'') \]
...
\end{tikzpicture}
\]
after noting that $g'' i = i' f'' = i'' k f $ by the commuting square condition for the morphisms
...
\end{tikzpicture}
\]
defined in Equation \eqref{theta}.   We need to prove that when the square in $\A$ is 
a pushout, $\theta$ is a natural isomorphism.   

We start by describing $\theta$ more concretely.  Let $(b \xrightarrow{i} e, x)$ be an object of $(\A/U)_b$. Going left, we precompose with $h \maps a \to b$ to obtain the object $(a \xrightarrow{i h} e, z)$ of $(\A/U)_a$. Then going down, we push forward along $k$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$a$};
\node (B) at (1.5,0) {$e$};
...
\end{tikzpicture}
\]
to obtain the object $(c \xrightarrow{\psi} c+_a e, F(\psi)(x))$ of $(\A/U)_c$.  For the other route, first going down, we push the object $(b \xrightarrow{i} e,x)$ forward along $g \maps b \to d$ by taking the following pushout:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$b$};
\node (B) at (1.5,0) {$U(x)$};
...
\end{tikzpicture}
\]
which yields the object $(d \xrightarrow{\phi} d+_b e, F(\phi)(x))$ of $(\A/U)_d$.  Then going left, we precompose with $f \maps c \to d$ to obtain the object $(c \xrightarrow{\phi f} d+_b e, F(\phi)(x))$ of $(\A/U)_c$.

The natural transformation $\theta$ gives a morphism
\[    \theta \maps   (c \xrightarrow{\psi} c+_a e, F(\psi)(x)) \; \to \; (c \xrightarrow{\phi f} d+_b e, F(\phi)(x))  \]
...
in the diagram above) is a pushout, its pasting with the other pushout gives a pushout, so $\zeta$ and hence $\theta$ is isomorphism.

This shows that $\Phi$ obeys the Beck--Chevalley condition.  Having checked all the hypotheses of \cref{lem:shulman}, we can define $F\lCsp$ to be $\Fr(\Phi)$.
\end{proof}
...
\[ M= (a\xrightarrow{i}m\xleftarrow{o}b, \; x\in F(m)) \]
is the following $F'$-decorated cospan:
\[ \lH(M)= (H(a)\xrightarrow{H(i)}H(m)\xleftarrow{H(o)}H(b), \;\bar{x}\in F'(H(m))) \]
 where
\[
...
\node (G) at (5,0) {$\tau \maps F(h)(x) \to x'$};
\path[->,font=\scriptsize,>=angle 90]
(A) edge node[above]{$i$} (B)
(C) edge node[above]{$o$} (B)
(A) edge node[left]{$f$} (A')
...
\end{tikzpicture}
\]
where the decoration morphism $\tau$ is mapped to a decoration morphism $\lH(\tau)$ as follows.  We can think of $\tau$ as a natural transformation
\[
\begin{tikzpicture}[scale=1.5]
\node (G) at (-3.5,-0.5) {$1$};
\node (H) at (-2,0) {$F(m)$};
...
\]
\end{itemize}
The square at right commutes strictly because $\theta$ is a natural isomorphism.   Perhaps we could generalize our construction to handle the case where $\theta$ is a pseudonatural equivalence, making this square commute up to a natural isomorphism.  However, the following result suffices 
for the application in \cref{subsec:petrirates}.

\begin{thm}
\label{thm:functoriality}
Given categories $\A$ and $\A'$ with finite colimits, lax monoidal pseudofunctors $F, F' \maps (\A',+) \to (\Cat,\times)$, a finite colimit preserving functor $H \maps \A \to \A'$, a lax monoidal pseudofunctor $E \maps (\Cat,\times) \to (\Cat,\times)$ and a natural isomorphism $\theta$ as in the following diagram:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\A$};
\node (B) at (1,0) {$\Cat$};
...
\end{tikzpicture}
\] 
we obtain a double functor $\lH \maps F\lCsp \to F'\lCsp$ defined as above.  If $F, F'$ and $E$ are symmetric, then $\|H \maps F\lCsp \to F'\lCsp$ is a symmetric monoidal double functor. 
\end{thm}
...
\end{tikzpicture}
\]
Such a commuting square is a morphism in Shulman's 2-category of Beck--Chevalley monoidal bifibrations.  Appying Shulman's 2-functor $\mathbb{F}\mathbf{r}$ to this morphism \cite[Theorem 14.11]{Shulman2008}, we obtain the desired double functor
...
\[  G(a \xrightarrow{i} b, x) \; = \; (H(a) \xrightarrow{H(i)} H(b), \underline{x}), \]
where to define $\underline{x}$ we treat $x \in F(b)$ as a functor 
$x \maps 1 \to F(b)$ and let $\underline{x} \in F'(H(b))$ be the composite functor
\[  1 \xrightarrow{\phi_0} E(1) \xrightarrow{E(x)} E(F(b)) \xrightarrow{\;\theta_b\;} F'(H(b)) .\]
On morphisms $(f,g,h) \maps (a \xrightarrow{i} b, x) \; \to \; 
(a' \xrightarrow{i'} b', x')$, $G$ is given by
\[   G(f,g,h) = (H(f),H(g),\underline{h}) \]
where to define $\underline{h}$ we treat $h \maps F(g)(x) \to x'$ as a natural transformation
\[
\begin{tikzpicture}[scale=1.5]
\node (G) at (-3.5,-0.5) {$1$};
\node (H) at (-2,0) {$F(b)$};
...
\end{tikzpicture}
\]
and let $\underline{h} \maps F'(H(g))(\underline{x}) \to \underline{x}'$ be this
...
\[  \gamma \maps G(a \xrightarrow{i} b, x) \otimes G(a' \xrightarrow{i'} b', x') \; 
\to \; G((a \xrightarrow{i} b, x) \otimes (a' \xrightarrow{i'} b', x')).\]
Note that
\[   G(a \xrightarrow{i} b, x) \otimes G(a' \xrightarrow{i'} b', x') = 
\left(H(a) + H(a') \xrightarrow{H(i) + H(i')} H(b) + H(b') , \; \phi'_{H(b), H(b')}(\underline{x},
\underline{x}')\right) \]
where $\phi'$ is the laxator for $F'$, while
\[    G((a \xrightarrow{i} b, x) \otimes (a' \xrightarrow{i'} b', x')) =
\left(H(a+a') \xrightarrow{H(i+i')} H(b+b'), \; \underline{\phi_{b,b'}(x,x')} \right)
\]
where $\phi$ is the laxator for $F$.  Since $H$ preserves finite colimits, the laxator
$\gamma$ is obvious except for the morphism from $\phi'_{H(b), H(b')}(\underline{x},
\underline{x}')$ to $\underline{\phi_{b,b'}(x,x')}$.   This is given by the following natural
transformation:
\[
\begin{tikzpicture}[scale=1.5]
\node () at (3,-0.5) {$\scriptscriptstyle{\stackrel{E_{x,x'}} \cong}$};
...
(B) edge node [above] {$E(x) \times E(x')$} (E)
(D) edge node [below] {$E(x \times x')$} (F)
...
is defined similarly.   One can show that this laxator and unit morphism satisfy the coherence laws of a lax monoidal functor, and they are invertible, so $G$ is strong monoidal functor.   \john{If furthermore $F, F'$ and $E$ are symmetric, one can check that $G$ is symmetric monoidal, and that $\lH$ is a symmetric monoidal double functor.}
\end{proof}
...
The monoidal structure of $L/\X$ is similar to the monoidal structure of $\A/U$ of \cref{thm:decorated_cospans_1} in that the tensor product of two objects $(a,L(a) \xrightarrow{i} x)$ and $(a,L(a) \xrightarrow{i'} x')$ is given by 
\[  (a,L(a) \xrightarrow{i} x) \otimes (a',L(a') \xrightarrow{i'} x')=(a+a',x+x',L(a+a') \xrightarrow{i+i'} x+x') \]
where again the natural isomorphism $L(a)+L(a') \cong L(a+a')$ is implicit. The monoidal unit is ${(0_\A,0_\X,{L(0_\A) \to 0_\X})}$.  It is clear that $\Psi \maps (L/\X, \otimes) \maps (\A,+)$ is a strict monoidal functor.   {\john Is the tensor product on $L/\X$ also just $+$?  If so, let's say that and call it $+$.}
...
\end{tikzpicture}
\]
defined in Equation \eqref{theta} is an isomorphism.  Let $(b,L(b) \xrightarrow{i} x)$ be an object of $(L/\X)_b$.  Going left, we precompose with $L(h) \maps L(a) \to L(b)$ to obtain the object $(a,L(a) \xrightarrow{i  L(h)} x)$ in $(L/\X)_a$.  Then going down, we push forward along $L(k) \maps L(a) \to L(c)$:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(a)$};
\node (B) at (1.5,0) {$x$};
...
\end{tikzpicture}
\]
to obtain the object $(c,L(c) \xrightarrow{\psi} L(c)+_{L(a)} x)$ of $(L/ \X)_c$. For the other route, first going down, we push the object $(b,x,L(b) \xrightarrow{I} x)$ forward along $L(g) \maps L(b) \to L(d)$ by taking the following pushout:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(b)$};
\node (B) at (1.5,0) {$x$};
...
\end{tikzpicture}
\]
which yields the object $(d,L(d) \xrightarrow{\theta} L(d)+_{L(b)} x)$ of $(L/\X)_d$.  Then going left, we precompose with $L(f) \maps L(c) \to L(d)$ to obtain the object $(c,L(c) \xrightarrow{\phi L(f)} L(d)+_{L(b)} x)$ of $(L/\X)_c$.

To establish a natural isomorphism between the two objects $(c,L(c)+_{L(a)} x,L(c) \xrightarrow{\psi} L(c)+_{L(a)} x)$ and $(c,L(d)+_{L(b)} x,L(c) \xrightarrow{\theta \circ L(f)} L(d)+_{L(b)} x)$ in $(L/\X)_c$, it suffices to establish a natural isomorphism between the objects $L(c)+_{L(a)} x$ and $L(d)+_{L(b)} x$ in $\X$. The first pushout square above exhibits $L(c)+_{L(a)}x$ as a colimit of the span $$L(c) \xleftarrow{L(k)} L(a) \xrightarrow{I \circ L(h)} x$$in $\X$. On the other hand, as $L$ preserves finite colimits, $L$ preserves pushout squares, and so the following square is also a pushout:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$L(a)$};
\node (B) at (1,0) {$L(b)$};
...
All the above concepts and results have analogues for fibrations.   A functor $\Phi \maps \X \to \A$ a \define{fibration} if and only if $\Phi\op \maps \X\op \to \A\op$ is an opfibration.  Equivalently, $\Phi$ is a fibration if and only if for every $y \in \X$ with $U(y) = a$ and $f \maps a \to b$ in $\A$ there exists a cartesian lifting of $f$ to $x$, where this concept is defined dually to cocartesian lifting.     Furthermore, if $\Phi$ is a fibration there is a contravariant reindexing functor
\[    f^\ast \maps \X_b \to \X_a \]
for each morphism $f \maps a \to b$.  

\begin{defn}
A functor $\Phi \maps \X \to \A$ is a \define{bifibration} if it is both a fibration and an opfibration.     
\end{defn}

\begin{defn}
 If $\X$ and $\A$ are monoidal categories, a \define{monoidal} bifibration $\Phi \maps \X \to \A$ is a bifibration that is also a strict monoidal functor, such that the tensor product preserves cartesian and cocartesian liftings.   Similarly, if $\C$ and $\A$ are symmetric monoidal, a \define{symmetric monoidal} bifibration is a bifibration $\Phi \maps \C \to \A$ that is a symmetric strict monoidal functor.
\end{defn}
...
a pushout, we say $\Phi$ is \define{Beck--Chevalley}.  (Shulman  uses the term `strongly co-BC' \cite[Definition 13.21] {Shulman2008}.)
