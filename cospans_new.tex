...
In all that follows, when we say a category `has finite colimits' we mean it is equipped with a choice of colimit for every finite diagram.   Thus, if $\A$ has finite colimits it gives a cocartesian monoidal category $(\A,+)$: that is, a symmetric monoidal category where the monoidal structure is given by the chosen binary coproducts and initial object.   However, when we say a functor `preserves finite colimits', it need only do this up to canonical isomorphism, unless otherwise specified.  {\chris Shouldn't we say that the definition of a lax monoidal pseudofunctor is recalled there in the appendix?}
...
\end{tikzpicture}
\]
together with a \define{decoration morphism} $\tau \maps F(h)(s) \to s'$ in $F(s')$.
\end{itemize}
\end{thm}
...
domain, a horizontal 1-cell $\Fr(\Phi)$ simply amounts to a cospan in $\A$. {\chris Perhaps a bit more spelling out here --using specific names of objects and arrows--could help the reader go through that important statement more easily. Optional. } In this case $\Fr(\Phi)$ is the double category of cospans in $\A$.    We shall use variations on this idea to construct double categories of decorated and structured cospans, and to prove that under certain conditions these double categories are equivalent.    We begin with decorated cospans, proving Theorem \ref{thm:decorated_cospans_1} and \ref{thm:decorated_cospans_2} by applying Shulman's result to a particular monoidal bifibration built from a lax monoidal pseudofunctor $F \maps (\A,+) \to (\Cat,\times)$.   

 The Grothendieck construction gives a bijection between pseudofunctors $F \maps \A \to \Cat$ and opfibrations $U \maps \inta F \to \A$.  We need two refinements of this construction: one that gives monoidal opfibrations, and one that gives symmetric monoidal opfibrations.  We recall these ideas in \cref{subsec:fibrations}; here we simply state the result we need. {\chris Well we don't recall the monoidal Grothendieck construction in the appendix, just the ordinary one, and the definition of a monoidal fibration. Slightly rephrase? And also add the definition of a monoidal indexed category in the appendix?}
...
Then an $F$-decorated cospan from $a$ to $b$ amounts to an object $c \in \A/U$ such that $\Phi(c) = a + b$.    With this choice of $\Phi$, Shulman's result gives the double category of $F$-decorated cospans.

\begin{proof}[Proof of \cref{thm:decorated_cospans_1}] 
Let $U \maps \inta F \to \A$ be the monoidal opfibration associated to the lax monoidal pseudofunctor $F \maps (\A,+) \to (\Cat, \times)$ {\chris via \cref{lem:MonGroth}}.  To obtain the desired double category we apply Shulman's {\chris it's OK to skip ``Shulman's'' here?} \cref{lem:shulman} to the functor $\Phi \maps \A/U \to \A$, which we now describe in more detail.  In the comma category $\A/U$:
\begin{itemize}
...
Denoting the cartesian lifting of $f$ to an object $z \in \A/U$ by $\mathrm{Cart}(f,z)$, the tensor product of $\A/U$ strictly preserves such cartesian liftings in the sense that $\mathrm{Cart}(f_1,z_1) \otimes \mathrm{Cart}(f_2,z_2)=\mathrm{Cart}(f_1 + f_2, z_1 \otimes z_2)$.  {\chris We could put ``strictly'' in a parenthesis since even if it is true, it is not required for the definition of a monoidal fibration. In fact it is for the definition of monoidal split fibration but this is not relevant here.}
...
\end{tikzpicture}
\]
defined in \eqref{theta}. {\chris I changed the two cell inside the square to match others in the paper.}  We need to prove that when the square in $\A$ is 
...
\end{tikzpicture}
\]
Namely, we have $\theta = (1, \zeta, 1)$. {\chris Should this really be a triple?}   But when the original square in $\A$ (at upper left 
...
Our construction {\chris it might not be a completely clear what we mean by ``our construction'' here.} gives not only decorated cospan double categories, but  also maps between these.   For simplicity we only consider the monoidal case. {\chris we mean ``non-symmetric''? Although it seems we also talk about the symmetric case below!} Suppose we have two categories $\A,\A'$ with finite colimits and two lax monoidal pseudofunctors $F \maps \A \to \Cat$ and $F' \maps \A' \to \Cat$.   Then we can obtain a map between their decorated cospan double categories, namely a double functor $\lH \maps F\lCsp \to F' \lCsp$, from:
\begin{itemize}
...
extra data \cite{BC}.    Here we recall this formalism and give a new construction of the double category of structured cospans that clarifies their relation to decorated cospans.  {\chris Do we want to briefly mention that in the abstact?} This new construction again uses Shulman's \cref{lem:shulman}.  
...
As a left adjoint, $L$ preserves all colimits that exist between the categories $\A$ and $\inta F$, which have finite colimits, so we can construct the double category of structured cospans $_L\lCsp(\inta F)$ of \cref{thm:structured_cospans}.

To show that the symmetric monoidal double categories $F\lCsp$ and $_L \lCsp(\inta F)$ of Theorems \ref{thm:decorated_cospans_2} and \ref{thm:structured_cospans} are isomorphic, we make use of a result of Shulman \cite[Theorem~14.11]{Shulman2010}.   {\chris Shouldn't this material be moved outside the proof environment, probably earlier?} Namely, he showed that the $\mathbb{F}\mathbf{r}$ construction of \cref{lem:shulman} extends to a 2-functor 
...
\end{itemize}
Given two monoidal bifibrations $\Phi \maps \A \to \mathsf{B}$ and $\Phi' \maps \A' \to \mathsf{B}'$, a \define{strong monoidal morphism of bifibrations} from $\Phi$ to $\Phi'$ consists of a pair of strong monoidal functors $F_1$ and $F_2$ making the following square commute:
\[
\begin{tikzpicture}[scale=1.5]
\node (A) at (0,0) {$\A$};
\node (B) at (1,0) {$\A'$};
...
\end{tikzpicture}
\]
and such that the identity $\Phi' F_1 = F_2 \Phi$ is a monoidal transformation. The two monoidal bifibrations in question are the functors $\Phi$ and $\Psi$ of Theorems \ref{thm:decorated_cospans_2} and \ref{thm:structured_cospans}, respectively, which share $\A$ as a common base, and thus we only need a single strong monoidal functor $F \maps \A/U \to L/\X$ making this diagram commute:
\[
...
\end{tikzpicture}
\]
{\john with $\Psi F=\Phi$ a monoidal transformation.}  There is an isomorphism $F \maps \A/U \to L/\X$ arising from the fact that $U$ is right adjoint to $L$.   It is clear that $F$ is strong monoidal via componentwise binary coproducts which are preserved by both $L$ and $U$, {\john and that the identity $\Psi F=\Phi$ is a monoidal transformation.} As $F$ is an isomorphism, we obtain an isomorphism 
\[  \mathbb{F}\mathbf{r}(F) \maps \mathbb{F}\mathbf{r}(\Phi) \to \mathbb{F}\mathbf{r}(\Psi)\] between the two monoidal double categories $\mathbb{F}\mathbf{r}(\Phi)=F\lCsp$ and $\mathbb{F}\mathbf{r}(\Psi)= {}_L \lCsp(\inta F)$ of Theorems \ref{thm:decorated_cospans_2} and \ref{thm:structured_cospans}, respectively.  {\john One can check by hand that this isomorphism 
is symmetric monoidal.}
{\chris Even so, shouldn't we also add some of the explicit isomorphism mapping to make the isomorphism between those two double categories more concrete?}
\end{proof}
...
\end{tikzpicture}
\]
where $\bot_b$ is the trivial decoration given by
...
{\chris Since a fibration and an opfibration were not in a definition environment, I think bifibration does not need to be in one as well.}
...
{\chris Since we still use monoidal opfibrations in the main text, it could be better to say what a monoidal opfibration and fibration is first, and then the monoidal bifibration. Suggestion:

If $\X$ and $\A$ are (symmetric) monoidal categories, a \define{(symmetric) monoidal fibration} $\Phi \maps \X \to \A$ is a fibration that is also a (symmetric) strict monoidal functor, such that the tensor product of $\X$ preserves cartesian liftings. Similarly, a \define{(symmetric) monoidal opfibration} is an opfibration that is also a (symmetric) strict monoidal functor, such that the tensor product of the total category preserves cocartesian liftings, whereas for a \define{(symmetric) monoidal bifibration} the tensor product preserves both cartesian and cocartesian liftings.
}
...
\begin{equation}
\label{theta}
   k_! h^\ast \To k_! h^\ast g^\ast g_! = k_! k^\ast f^\ast g_! \To f^\ast g_! . 
\end{equation}
{\chris Shouldn't the middle equality be an isomorphism?}
